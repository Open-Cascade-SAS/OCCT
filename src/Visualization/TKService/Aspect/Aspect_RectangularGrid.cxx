                                     ...
                                     double&      gridY) const
{
  ...
  int    n1      = static_cast<int>(std::abs(D1) / myXStep + 0.5);
  int    n2      = static_cast<int>(std::abs(D2) / myYStep + 0.5);
  double offset1 = c1 + static_cast<double>(n1) * std::copysign(myXStep, D1);
  double offset2 = c2 + static_cast<double>(n2) * std::copysign(myYStep, D2);
