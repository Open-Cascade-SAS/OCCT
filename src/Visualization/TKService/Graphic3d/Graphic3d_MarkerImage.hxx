...
public:
  ...
  Standard_EXPORT Graphic3d_MarkerImage(const occ::handle<TColStd_HArray1OfByte>& theBitMap,
                                        const int               theWidth,
                                        const int               theHeight);
  ...
  Standard_EXPORT occ::handle<TColStd_HArray1OfByte> GetBitMapArray(
...
private:
  ...
  TCollection_AsciiString myImageAlphaId; //!< resource identifier
                                          // clang-format off
  occ::handle<TColStd_HArray1OfByte> myBitMap;       //!< bytes array with bitmap definition (for compatibility with old code)
