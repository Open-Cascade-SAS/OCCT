...
static occ::handle<TColStd_HArray1OfByte> fillPointBitmap(const int theSize)
{
  const int        aNbBytes = (theSize / 8 + (theSize % 8 ? 1 : 0)) * theSize;
  occ::handle<TColStd_HArray1OfByte> aBitMap  = new TColStd_HArray1OfByte(0, aNbBytes - 1);
...
Graphic3d_MarkerImage::Graphic3d_MarkerImage(const occ::handle<TColStd_HArray1OfByte>& theBitMap,
                                             const int               theWidth,
                                             const int               theHeight)
    : myBitMap(theBitMap),
      myMargin(1),
...
occ::handle<TColStd_HArray1OfByte> Graphic3d_MarkerImage::GetBitMapArray(
  ...
  const bool theIsTopDown) const
{
  ...
  occ::handle<TColStd_HArray1OfByte> aBitMap = new TColStd_HArray1OfByte(0, aNumOfBytes - 1);
  aBitMap->Init(0);
  ...
  switch (theMarkerType)
  {
    ...
    case Aspect_TOM_O_X: {
      ...
      if (theMarkerType == Aspect_TOM_O_POINT)
      {
        ...
        occ::handle<TColStd_HArray1OfByte> aBitMap = fillPointBitmap(aSize);
