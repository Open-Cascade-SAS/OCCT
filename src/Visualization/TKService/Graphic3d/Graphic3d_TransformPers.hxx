...
public:
                                         ...
                                         const int theViewportHeight) const
  {
    ...
    const int aVPSizeY = theCamera->Tile().IsValid() ? theCamera->Tile().TotalSize.y() : theViewportHeight;
    ...
    return Abs(aViewDim.Y()) / double(aVPSizeY);
  }
  ...
  //! @param theViewportWidth [in] the width of viewport
  //! @param theViewportHeight [in] the height of viewport
  virtual NCollection_Mat3<double> persistentRotationMatrix (const occ::handle<Graphic3d_Camera>& theCamera,
                                                                    const int theViewportWidth,
                                                                    const int theViewportHeight) const
  {
    (void )theCamera;
                                     ...
                                     const bool theToApplyProjPers) const
{
  ...
  occ::handle<Graphic3d_Camera> aCamera = theCamera;
  ...
  NCollection_Mat4<double> aWorldView = aCamera->OrientationMatrix();

  // use total size when tiling is active
  const int aVPSizeY = aCamera->Tile().IsValid() ? aCamera->Tile().TotalSize.y() : theViewportHeight;
  ...
  if ((myMode & Graphic3d_TMF_TriedronPers) != 0)
  {
    ...
    const double aFocus = aCamera->IsOrthographic()
                               ? aCamera->Distance()
                               : (aCamera->ZFocusType() == Graphic3d_Camera::FocusType_Relative
                                ? double(aCamera->ZFocus() * aCamera->Distance())
                                : double(aCamera->ZFocus()));

    // scale factor to pixels
    const gp_XYZ aViewDim = aCamera->ViewDimensions (aFocus);
    const double aScale = Abs(aViewDim.Y()) / double(aVPSizeY);
  ...
  else if ((myMode & Graphic3d_TMF_2d) != 0)
  {
    ...
    if ((myParams.Params2d.Corner & (Aspect_TOTP_LEFT | Aspect_TOTP_RIGHT)) != 0)
    {
      aCenter.SetX (-aViewDim.X() * aCamera->NDC2dOffsetX() + (double(myParams.Params2d.OffsetX) + aJitterComp) * aScale);
      if ((myParams.Params2d.Corner & Aspect_TOTP_RIGHT) != 0)
      {
    ...
    if ((myParams.Params2d.Corner & (Aspect_TOTP_TOP | Aspect_TOTP_BOTTOM)) != 0)
    {
      aCenter.SetY (-aViewDim.Y() * aCamera->NDC2dOffsetY() + (double(myParams.Params2d.OffsetY) + aJitterComp) * aScale);
      if ((myParams.Params2d.Corner & Aspect_TOTP_TOP) != 0)
      {
  ...
  else
  {
    ...
    {
      NCollection_Mat3<double> aRotMat = persistentRotationMatrix (theCamera, theViewportWidth, theViewportHeight);
    ...
    {
      // lock zooming
      double aScale = persistentScale (aCamera, theViewportWidth, theViewportHeight);
      Graphic3d_TransformUtils::Scale (aWorldView, aScale, aScale, aScale);
    }
  }
