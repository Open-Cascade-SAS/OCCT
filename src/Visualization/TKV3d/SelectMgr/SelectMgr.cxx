...
namespace
{
  ...
  static occ::handle<TColgp_HSequenceOfPnt> shrunkTriangle (const gp_Pnt* thePnts,
                                                       const gp_XYZ& theCenter)
  {
    ...
    occ::handle<TColgp_HSequenceOfPnt> aPoints = new TColgp_HSequenceOfPnt();
    aPoints->Append (aV1);
    aPoints->Append (aV2);
    aPoints->Append (aV3);
    aPoints->Append (aV1);
  ...
  static void addCylinder (Prs3d_NListOfSequenceOfPnt& theSeqLines,
                           ...
                           const gp_Trsf& theLoc)
  {
    occ::handle<TColgp_HSequenceOfPnt> aVertLine1 = new TColgp_HSequenceOfPnt();
    occ::handle<TColgp_HSequenceOfPnt> aVertLine2 = new TColgp_HSequenceOfPnt();
                                     ...
                                     const occ::handle<Graphic3d_TransformPers>& theTrsfPers)
{
  ...
  for (NCollection_Vector<occ::handle<SelectMgr_SensitiveEntity>>::Iterator aSelEntIter (theSel->Entities()); aSelEntIter.More(); aSelEntIter.Next())
  {
    ...
    if (occ::handle<Select3D_SensitiveBox> aSensBox = Handle(Select3D_SensitiveBox)::DownCast (anEnt))
    {
      addBoundingBox (aSeqLines, aSensBox, theLoc);
    }
    else if (occ::handle<Select3D_SensitiveCylinder> aSensCyl = Handle(Select3D_SensitiveCylinder)::DownCast (anEnt))
    {
      addCylinder (aSeqLines, aSensCyl, theLoc);
    }
    else if (occ::handle<Select3D_SensitiveCircle> aSensCircle = Handle(Select3D_SensitiveCircle)::DownCast (anEnt))
    {
    ...
    {
      occ::handle<TColgp_HArray1OfPnt> aSensPnts;
    ...
    else if (occ::handle<Select3D_SensitivePoly> aSensPoly = Handle(Select3D_SensitivePoly)::DownCast (anEnt))
    {
      int aFrom = 0, aTo = 0;
    ...
    else if (occ::handle<Select3D_SensitiveWire> aWire = occ::down_cast<Select3D_SensitiveWire>(anEnt))
    {
      ...
      for (NCollection_Vector<occ::handle<Select3D_SensitiveEntity>>::Iterator aSubIter (anEntities); aSubIter.More(); aSubIter.Next())
      {
        const occ::handle<Select3D_SensitiveEntity>& aSubEnt = aSubIter.Value();
        if (occ::handle<Select3D_SensitiveSegment> aSensSeg = Handle(Select3D_SensitiveSegment)::DownCast (aSubEnt))
        {
          occ::handle<TColgp_HSequenceOfPnt> aPoints = new TColgp_HSequenceOfPnt();
        ...
        else if (occ::handle<Select3D_SensitivePoly> aSubSensPoly = Handle(Select3D_SensitivePoly)::DownCast (aSubEnt))
        {
          int aFrom = 0, aTo = 0;
    ...
    else if (occ::handle<Select3D_SensitiveSegment> aSensSeg = occ::down_cast<Select3D_SensitiveSegment>(anEnt))
    {
      occ::handle<TColgp_HSequenceOfPnt> aPoints = new TColgp_HSequenceOfPnt();
      aPoints->Append (aSensSeg->StartPoint().Transformed (theLoc));
      aPoints->Append (aSensSeg->EndPoint()  .Transformed (theLoc));
    ...
    else if (occ::handle<Select3D_SensitivePoint> aSensPnt = occ::down_cast<Select3D_SensitivePoint>(anEnt))
    {
      aSeqPoints.Append (aSensPnt->Point().Transformed (theLoc));
    }
    else if (occ::handle<Select3D_SensitiveTriangulation> aSensTri = Handle(Select3D_SensitiveTriangulation)::DownCast (anEnt))
    {
      addTriangulation (aSeqLines, aSeqFree, aSensTri, theLoc);
    }
    else if (occ::handle<Select3D_SensitiveTriangle> aSensTri1 = occ::down_cast<Select3D_SensitiveTriangle>(anEnt))
    {
      gp_Pnt aPnts[3];
  ...
  {
    occ::handle<Graphic3d_ArrayOfPoints> anArrayOfPoints = new Graphic3d_ArrayOfPoints (aSeqPoints.Size());
