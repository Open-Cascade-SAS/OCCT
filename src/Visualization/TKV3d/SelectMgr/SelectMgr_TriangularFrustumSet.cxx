...
void SelectMgr_TriangularFrustumSet::Build()
{
  ...
  occ::handle<BRepMesh_DataStructureOfDelaun> aMeshStructure = new BRepMesh_DataStructureOfDelaun (anAllocator);
...
// purpose  :
// =======================================================================
bool SelectMgr_TriangularFrustumSet::OverlapsBox (const SelectMgr_Vec3& theMinPnt,
...
// purpose  :
// =======================================================================
bool SelectMgr_TriangularFrustumSet::OverlapsPolygon (const TColgp_Array1OfPnt& theArrayOfPts,
                                                                 ...
                                                                 bool* /*theInside*/) const
{
  ...
  for (NCollection_List<occ::handle<SelectMgr_TriangularFrustum>>::Iterator anIter (myFrustums); anIter.More(); anIter.Next())
  {
    if (anIter.Value()->OverlapsSphere (theCenter, theRadius, NULL))
    {
      ...
      for (int anIdx = myBoundaryPoints.Lower(); anIdx < myBoundaryPoints.Size() / 2 + myBoundaryPoints.Lower(); anIdx++)
      {
      ...
      bool isCenterInside = Abs (anAngleSum - 2 * M_PI) < Precision::Confusion();
      bool isBoundaryInside = false;
      bool isIntersectSphereBoundaries = IsBoundaryIntersectSphere (aCenterProj, theRadius, aNorm, aBoundaries, isBoundaryInside);
                                                                   ...
                                                                   bool* theInside) const
{
  ...
  const double aCosAngle = Cos (anAngle);
  ...
  const double aTBottom = -(aBottomCenter.XYZ().Dot (aDirNorm.XYZ()) + aCoefD) / aDirNorm.Dot (aDirNorm);
                                                                      ...
                                                                      const bool theIsFilled) const
{
  ...
  for (int anIdx = myBoundaryPoints.Lower(); anIdx < aFacesNb + myBoundaryPoints.Lower(); anIdx++)
  {
    ...
    const occ::handle<Geom_Line>& anInterLine = Handle(Geom_Line)::DownCast (anInterSS.Line(1));
    ...
    double anOffset = Sqrt (theRadius * theRadius - aDistance * aDistance);
...
// purpose  :
//=======================================================================
bool SelectMgr_TriangularFrustumSet::isIntersectBoundary (const gp_Pnt& thePnt1, const gp_Pnt& thePnt2) const
{
  int aFacesNb = myBoundaryPoints.Size() / 2;
...
//=======================================================================
bool SelectMgr_TriangularFrustumSet::segmentTriangleIntersection (const gp_Pnt& theOrig, const gp_Vec& theDir,
                                                                              const gp_Pnt& theV1, const gp_Pnt& theV2, const gp_Pnt& theV3)
{
...
bool SelectMgr_TriangularFrustumSet::IsInsideByProjection (const gp_Pnt& thePnt) const
{
  ...
  gp_Pnt aProj = thePnt;
  double aA, aB, aC, aD; aPln.Coefficients (aA, aB, aC, aD);
  ...
  for (int i = 0, j = aFacesNb - 1; i < aFacesNb; j = i++)
  {
    ...
    const bool intersect = ((Ai.Y() > P.Y()) != (Aj.Y() > P.Y()))
