                             ...
                             const OptionsForAttach&              theOptions)
{
  ...
  occ::handle<AIS_ManipulatorObjectSequence> aSeq = new AIS_ManipulatorObjectSequence();
...
void AIS_Manipulator::Attach(const occ::handle<AIS_ManipulatorObjectSequence>& theObjects,
                             const OptionsForAttach&                      theOptions)
{
...
occ::handle<AIS_ManipulatorObjectSequence> AIS_Manipulator::Objects() const
{
  return occ::down_cast<AIS_ManipulatorObjectSequence>(GetOwner());
}
...
{
  occ::handle<AIS_ManipulatorObjectSequence> anOwner =
    occ::down_cast<AIS_ManipulatorObjectSequence>(GetOwner());
                                                       ...
                                                       gp_Trsf&                theTrsf)
{
  ...
  {
    myStartTrsfs.Clear();
    occ::handle<AIS_ManipulatorObjectSequence> anObjects = Objects();
                                            ...
                                            const occ::handle<SelectMgr_EntityOwner>&      theOwner)
{
  ...
  for (NCollection_Sequence<occ::handle<Graphic3d_Group>>::Iterator aGroupIter(aPresentation->Groups()); aGroupIter.More();
       aGroupIter.Next())
  {
      ...
      for (int aU = 0; aU <= 1; ++aU)
      {
        for (int aV = 0; aV <= aStripsNb; ++aV)
        {
          ...
          if (aU != 0 && aV != 0)
          {
            ...
            occ::handle<Graphic3d_ArrayOfTriangles> aSquares = aV % 2 == 0 ? aColorlessArr : aColoredArr;
