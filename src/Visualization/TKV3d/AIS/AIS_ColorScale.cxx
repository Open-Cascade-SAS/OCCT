...
namespace
{
  ...
  static void addColoredQuad (const occ::handle<Graphic3d_ArrayOfTriangles>& theTris,
                              const int theXLeft, const int theYBottom,
                              const int theSizeX, const int theSizeY,
                                                 ...
                                                 int theNbIntervals)
  {
    ...
    int anInterval = 1 + (int )Floor (double (theNbIntervals) * (theValue - theMin) / (theMax - theMin));
...
//purpose  :
//=======================================================================
Aspect_SequenceOfColor AIS_ColorScale::MakeUniformColors (int theNbColors, 
                                                          double theLightness,
                                                          double theHueFrom,
                                                          double theHueTo)
{
  ...
  const double aHueEps = Precision::Angular() * 180. / M_PI;
...
//purpose  :
//=======================================================================
int AIS_ColorScale::computeMaxLabelWidth (const TColStd_SequenceOfExtendedString& theLabels) const
{
  int aWidthMax = 0;
                              ...
                              const int theMode)
{
  ...
  int aColorBreadth = Max (5, Min (20, myBreadth - aTextWidth - 3 * mySpacing));
                                   ...
                                   const int theColorBreadth)
{
  ...
  // Draw colors
  const int anXLeft = myLabelPos == Aspect_TOCSP_LEFT
                                 ...
                                 const int theColorBreadth)
{
  ...
  int aFilter = 0;
  {
    ...
    const int aSpc         = myHeight - aTitleHeight - ((Min (aNbLabels, 2) + Abs (aNbLabels - aNbIntervals - 1)) * myTextHeight);
    if (aSpc <= 0)
    {
      return;
    }
  ...
  int aLast1 = i1;
  int aLast2 = i2;
  const int anYBottom = myIsLabelAtBorder
                                   ? theBarBottom
                                   : theBarBottom + int(aStepY / 2);
  while (i2 - i1 >= aFilter || ( i2 == 0 && i1 == 0 ))
  {
    ...
    if (aFilter && !(aPos1 % aFilter))
    {
      drawText (theGroup, theLabels.Value (i1 + 1),
                anXLeft, anYBottom + int(i1 * aStepY + anAscent),
    ...
    if (aFilter && !(aPos2 % aFilter))
    {
      drawText (theGroup, theLabels.Value (i2 + 1),
                anXLeft, anYBottom + int(i2 * aStepY + anAscent),
                Graphic3d_VTA_CENTER);
      aLast2 = i2;
    }
    i1++;
  ...
  if (i0 != -1)
  {
    drawText (theGroup, theLabels.Value (i0 + 1),
              anXLeft, anYBottom + int(i0 * aStepY + anAscent),
              Graphic3d_VTA_CENTER);
  }
}
                                ...
                                const Quantity_Color& theColor)
{
  ...
  occ::handle<Graphic3d_AspectLine3d> anAspect = new Graphic3d_AspectLine3d (theColor, Aspect_TOL_SOLID, 1.0);
  occ::handle<Graphic3d_Group> aGroup = thePrs->NewGroup();
                               ...
                               int& theDescent) const
{
  ...
  if (HasInteractiveContext())
  {
    ...
    aViewer->Driver()->TextSize (aView, aText.ToCString(), (float)theHeight, aWidth, anAscent, aDescent);
  }
  theWidth   = (int)aWidth;
