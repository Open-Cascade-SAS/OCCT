...
public:
  ...
  Standard_EXPORT void SetColors(const occ::handle<TColStd_HArray1OfInteger>& aColor);

  //! Get the color for each node.
  //! Each 32-bit color is Alpha << 24 + Blue << 16 + Green << 8 + Red
  Standard_EXPORT occ::handle<TColStd_HArray1OfInteger> GetColors() const;
...
private:
  ...
  occ::handle<TColStd_HArray1OfInteger> myColor;
  int                 myFlagColor;
  int                 myNbNodes;
  int                 myNbTriangles;
};
