    ...
    : Select3D_SensitiveSet(theOwnerId)
{
  ...
  for (int aPntIter = aLowerIdx; aPntIter <= anUpperIdx; ++aPntIter)
  {
    ...
    if (aPntIter - aStartIdx == 2 && !aPlane.IsValid())
    {
      ...
      {
        occ::handle<TColgp_HArray1OfPnt> aPointsArray = new TColgp_HArray1OfPnt(0, anEndIdx - aStartIdx);
    ...
    else if (aPlane.IsValid())
    {
      ...
      if (!aPlane.Contains(thePoints.Value(aPntIter)) || anAngle > Precision::Confusion())
      {
        ...
        occ::handle<TColgp_HArray1OfPnt> aPointsArray = new TColgp_HArray1OfPnt(0, anUpperBound);
      ...
      else
      {
        ...
        if (anEndIdx == anUpperIdx)
        {
          occ::handle<TColgp_HArray1OfPnt> aPointsArray =
...
// =======================================================================
void Select3D_InteriorSensitivePointSet::GetPoints(occ::handle<TColgp_HArray1OfPnt>& theHArrayOfPnt)
{
  int aSize = 0;
  ...
  for (int aPolygIdx = 0; aPolygIdx < myPlanarPolygons.Length(); ++aPolygIdx)
  {
    ...
    occ::handle<TColgp_HArray1OfPnt>           aPoints;
