...
public:
  ...
  Standard_EXPORT Select3D_SensitiveTriangulation(
    ...
    const occ::handle<Poly_Triangulation>&       theTrg,
    const TopLoc_Location&                  theInitLoc,
    const occ::handle<TColStd_HArray1OfInteger>& theFreeEdges,
    const gp_Pnt&                           theCOG,
...
protected:
  ...
  occ::handle<TColStd_HArray1OfInteger> myFreeEdges;
  // clang-format off
  ...
  occ::handle<TColStd_HArray1OfInteger> myBVHPrimIndexes;     //!< Indexes of edges or triangles for BVH build
