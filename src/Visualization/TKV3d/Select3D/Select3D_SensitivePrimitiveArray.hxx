...
public:
  ...
  bool InitTriangulation (const occ::handle<Graphic3d_Buffer>&      theVerts,
                          ...
                          const int               theNbGroups = 1)
  {
    const int anUpper = !theIndices.IsNull() ? (theIndices->NbElements - 1)
                   ...
                   const int          theNbGroups = 1)
  {
    ...
    return InitPoints (theVerts, occ::handle<Graphic3d_IndexBuffer>(), theInitLoc, 0, anUpper, theToEvalMinMax, theNbGroups);
  }
  ...
  const occ::handle<TColStd_HPackedMapOfInteger>& LastDetectedElementMap() const { return myDetectedElemMap; }
  ...
  const occ::handle<TColStd_HPackedMapOfInteger>& LastDetectedNodeMap() const { return myDetectedNodeMap; }
  ...
  Standard_EXPORT std::vector<Graphic3d_Vec3> GetVertex (const int theIndex) const;
...
public:
  ...
  virtual int NbSubElements() const override
  {
...
protected:
  ...
  //! Auxiliary getter.
  const Graphic3d_Vec3& getPosVec3 (const int theIndex) const
  {
    return *reinterpret_cast<const Graphic3d_Vec3* >(myPosData + myPosStride * theIndex);
  }

  //! Auxiliary getter.
  const Graphic3d_Vec2& getPosVec2 (const int theIndex) const
  {
  ...
  Standard_EXPORT virtual double distanceToCOG (SelectBasics_SelectingVolumeManager& theMgr) override;
...
private:
  ...
  typedef NCollection_Shared<NCollection_Array1<occ::handle<Select3D_SensitivePrimitiveArray>> > Select3D_PrimArraySubGroupArray;
