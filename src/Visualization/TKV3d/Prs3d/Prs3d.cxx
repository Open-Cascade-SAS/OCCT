...
occ::handle<Graphic3d_ArrayOfPrimitives> Prs3d::PrimitivesFromPolylines(
  const NCollection_List<occ::handle<TColgp_HSequenceOfPnt>>& thePoints)
{
  ...
  int aNbVertices = 0;
  for (NCollection_List<occ::handle<TColgp_HSequenceOfPnt>>::Iterator anIt(thePoints); anIt.More(); anIt.Next())
  {
  ...
  for (NCollection_List<occ::handle<TColgp_HSequenceOfPnt>>::Iterator anIt(thePoints); anIt.More(); anIt.Next())
  {
    const occ::handle<TColgp_HSequenceOfPnt>& aPoints = anIt.Value();
...
void Prs3d::AddPrimitivesGroup(const occ::handle<Prs3d_Presentation>& thePrs,
