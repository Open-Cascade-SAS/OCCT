...
public:
  //! Main constructor.
  RectangularGridStructure (const occ::handle<Graphic3d_StructureManager>& theManager, V3d_RectangularGrid* theGrid)
...
void V3d_RectangularGrid::UpdateDisplay ()
{
  ...
  if (MakeTransform) {
    const double CosAlpha = Cos (RotationAngle ());
    const double SinAlpha = Sin (RotationAngle ());
...
void V3d_RectangularGrid::DefineLines ()
{
  ...
  const bool toUpdate = !myCurAreDefined
                                 || myCurDrawMode != Aspect_GDM_Lines
                                 || aXStep != myCurXStep
                                 || aYStep != myCurYStep;
  if (!toUpdate
  ...
  if (aSeqLines.Length())
  {
    occ::handle<Graphic3d_AspectLine3d> aLineAspect = new Graphic3d_AspectLine3d (myColor, Aspect_TOL_SOLID, 1.0);
  ...
  if (aSeqTenth.Length())
  {
    occ::handle<Graphic3d_AspectLine3d> aLineAspect = new Graphic3d_AspectLine3d (myTenthColor, Aspect_TOL_SOLID, 1.0);
    myGroup->SetPrimitivesAspect (aLineAspect);
...
void V3d_RectangularGrid::DefinePoints ()
{
  ...
  if (aSeqPnts.Length())
  {
    ...
    occ::handle<Graphic3d_AspectMarker3d> aMarkerAspect = new Graphic3d_AspectMarker3d (Aspect_TOM_POINT, myColor, 3.0);
...
void V3d_RectangularGrid::SetGraphicValues (const double theXSize, const double theYSize, const double theOffSet)
{
  if (! myCurAreDefined) {
    myXSize = theXSize;
    myYSize = theYSize;
