...
namespace
{
  static const int THE_NB_BOUND_POINTS = 8;
}
                          ...
                          const Graphic3d_Vec2i& theMargins)
{
  ...
  occ::handle<V3d_View> aParentView = !theParentView->IsSubview()
                               ? theParentView
...
{
  for (NCollection_Sequence<occ::handle<V3d_View>>::Iterator aViewIter (mySubviews); aViewIter.More(); aViewIter.Next())
  {
...
// purpose  :
// =============================================================================
occ::handle<V3d_View> V3d_View::PickSubview (const Graphic3d_Vec2i& thePnt) const
{
  if (thePnt.x() < 0
...
void V3d_View::SetBackgroundColor (const Quantity_TypeOfColor theType,
                                   ...
                                   const double theV3)
{
  double aV1 = Max (Min (theV1, 1.0), 0.0);
  double aV2 = Max (Min (theV2, 1.0), 0.0);
  double aV3 = Max (Min (theV3, 1.0), 0.0);
...
//purpose  :
//=============================================================================
void V3d_View::SetBgGradientStyle (const Aspect_GradientFillMethod theFillStyle, const bool theToUpdate)
{
...
//purpose  :
//=============================================================================
void V3d_View::SetBgImageStyle (const Aspect_FillMethod theFillStyle, const bool theToUpdate)
{
  myView->SetBackgroundImageStyle (theFillStyle);
...
//purpose  :
//=============================================================================
void V3d_View::SetAxis (const double theX,  const double theY,  const double theZ,
                        const double theVx, const double theVy, const double theVz)
{
...
//purpose  :
//=============================================================================
void V3d_View::Rotate(const double ax, const double ay, const double az,
                      const double X, const double Y, const double Z, const bool Start)
{
...
//purpose  :
//=============================================================================
void V3d_View::Rotate(const V3d_TypeOfAxe Axe, const double angle, const bool Start)
{
  switch (Axe) {
  case V3d_X :
...
//purpose  :
//=============================================================================
void V3d_View::Rotate (const V3d_TypeOfAxe theAxe, const double theAngle,
                       const double theX, const double theY, const double theZ, const bool theStart)
{
...
//purpose  :
//=============================================================================
void V3d_View::Turn(const double ax, const double ay, const double az, const bool Start)
{
  double Ax = ax;
  double Ay = ay;
  double Az = az;
...
//purpose  :
//=============================================================================
void V3d_View::Turn(const V3d_TypeOfAxe Axe, const double angle, const bool Start)
{
  switch (Axe) {
  case V3d_X :
...
//purpose  :
//=============================================================================
void V3d_View::SetUp (const double theVx, const double theVy, const double theVz)
{
  occ::handle<Graphic3d_Camera> aCamera = Camera();
...
void V3d_View::SetZSize (const double theSize)
{
  ...
  const double aPrecision = 1.0 / Pow (10.0, ShortRealDigits() - 1);
...
void V3d_View::SetRatio()
{
  ...
  {
    double aRatio = static_cast<double> (aWidth) /
                           static_cast<double> (aHeight);
...
//purpose  :
//=============================================================================
void V3d_View::FitAll (const Bnd_Box& theBox, const double theMargin, const bool theToUpdate)
{
...
//purpose  :
//=============================================================================
void V3d_View::DepthFitAll(const double Aspect,
                           const double Margin)
{
...
double V3d_View::Twist() const
{
  ...
  // compute Angle
  double anAngle = ASin (Max (Min (aP.Modulus(), 1.0), -1.0));
...
void V3d_View::SetFocale( const double focale )
{
  ...
  double aFOVyRad = ATan (focale / (aCamera->Distance() * 2.0));
...
//purpose  :
//=============================================================================
bool V3d_View::screenAxis (const gp_Dir& theVpn, const gp_Dir& theVup,
...
gp_XYZ V3d_View::TrsPoint (const Graphic3d_Vertex& thePnt, const TColStd_Array2OfReal& theMat)
{
  ...
  double X, Y, Z;
  thePnt.Coord (X,Y,Z);
  const double XX = (theMat(lr,lc+3)   + X*theMat(lr,lc)   + Y*theMat(lr,lc+1)   + Z*theMat(lr,lc+2)) / theMat(lr+3,lc+3);
  const double YY = (theMat(lr+1,lc+3) + X*theMat(lr+1,lc) + Y*theMat(lr+1,lc+1) + Z*theMat(lr+1,lc+2))/theMat(lr+3,lc+3);
  const double ZZ = (theMat(lr+2,lc+3) + X*theMat(lr+2,lc) + Y*theMat(lr+2,lc+1) + Z*theMat(lr+2,lc+2))/theMat(lr+3,lc+3);
                     ...
                     const int theYp2)
{
  ...
  if (aDx != 0 || aDy != 0)
  {
    double aCoeff = Sqrt( (double)(aDx * aDx + aDy * aDy) ) / 100.0 + 1.0;
                            ...
                            const int theMouseEndY)
{
  ...
  double aDZoom = Abs (aDxy) / 100.0 + 1.0;
                           ...
                           const V3d_TypeOfAxe Axis)
{
  if( Dx != 0. || Dy != 0. ) {
    double Sx, Sy, Sz;
    AxialScale( Sx, Sy, Sz );
    double dscale = Sqrt(Dx*Dx + Dy*Dy) / 100. + 1;
                      ...
                      const double theYmax)
{
  ...
  double aFitSizeU  = Abs (theXmax - theXmin);
  double aFitSizeV  = Abs (theYmax - theYmin);
                             ...
                             const double zRotationThreshold)
{
  ...
  if( zRotationThreshold > 0. ) {
    double dx = Abs(sx - rx/2.);
    double dy = Abs(sy - ry/2.);
                                     ...
                                     const V3d_ImageDumpOptions& theParams)
{
   ...
   && aTargetSize.y() != 0)
  {
    ...
    if (theImage.IsEmpty()
     || theImage.SizeX() != size_t(aTargetSize.x())
...
//purpose  :
//=============================================================================
void V3d_View::Translate (const V3d_TypeOfAxe theAxe, const double theLength,const bool theStart)
{
  switch (theAxe)
  {
    case V3d_X:
...
//purpose  :
//=============================================================================
void toPolarCoords (const double theX, const double theY,
                          double& theR, double& thePhi)
{
...
//purpose  :
//=============================================================================
void toCartesianCoords (const double theR, const double thePhi,
...
Graphic3d_Vertex V3d_View::Compute (const Graphic3d_Vertex& theVertex) const
{
  ...
  const double aT    = aPointOrigin.Dot (MyPlane.Direction()) / aProjection.Dot (MyPlane.Direction());
  const gp_XYZ aPointOnPlane = gp_XYZ (theVertex.X(), theVertex.Y(), theVertex.Z()) + aProjection.XYZ() * aT;

  if (occ::handle<Aspect_RectangularGrid> aRectGrid = Handle(Aspect_RectangularGrid)::DownCast (MyGrid))
  {
    // project point on plane to grid local space
    const gp_Vec aToPoint (aPnt0, aPointOnPlane);
    const double anXSteps = Round (aGridX.Dot (aToPoint) / aRectGrid->XStep());
    const double anYSteps = Round (aGridY.Dot (aToPoint) / aRectGrid->YStep());
  ...
  else if (occ::handle<Aspect_CircularGrid> aCircleGrid = Handle(Aspect_CircularGrid)::DownCast (MyGrid))
  {
    ...
    // clamp point to grid
    const double anRSteps  = Round (anR / aCircleGrid->RadiusStep());
    const double aPhiSteps = Round (aPhi / anAlpha);
