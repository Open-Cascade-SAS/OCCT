...
void StdSelect_BRepSelectionTool::PreBuildBVH(const occ::handle<SelectMgr_Selection>& theSelection)
{
       ...
       aSelEntIter.Next())
  {
    ...
    occ::handle<Select3D_SensitiveGroup> aGroup = occ::down_cast<Select3D_SensitiveGroup>(aSensitive);
                                       ...
                                       const double    theMaxParam)
{
  ...
  for (NCollection_Vector<occ::handle<SelectMgr_SensitiveEntity>>::Iterator aSelEntIter(
       ...
       aSelEntIter.Next())
  {
    const occ::handle<SelectMgr_EntityOwner>& anOwner = aSelEntIter.Value()->BaseSensitive()->OwnerId();
  ...
  switch (theShape.ShapeType())
  {
    ...
    case TopAbs_FACE: {
      ...
      GetSensitiveForFace(aFace,
                          ...
                          theNbPOnEdge,
                          theMaxParam);
      for (NCollection_Sequence<occ::handle<Select3D_SensitiveEntity>>::Iterator aSensIter(aSensitiveList); aSensIter.More();
           aSensIter.Next())
      {
...
static occ::handle<TColgp_HArray1OfPnt> GetPointsFromPolygon(const TopoDS_Edge& theEdge)
{
  occ::handle<TColgp_HArray1OfPnt> aResultPoints;
  ...
  if (!aPolygon.IsNull())
  {
    ...
    {
      for (int aNodeId(aNodes.Lower()), aPntId(1); aNodeId <= aNodes.Upper();
           ++aNodeId, ++aPntId)
      {
                                                   ...
                                                   occ::handle<Select3D_SensitiveEntity>& theSensitive)
{
  ...
  occ::handle<TColgp_HArray1OfPnt> aPoints = GetPointsFromPolygon(anEdge);
  ...
  switch (cu3d.GetType())
  {
    ...
    default: {
      ...
      occ::handle<TColgp_HArray1OfPnt> aPointArray = new TColgp_HArray1OfPnt(1, aPntNb);
  ...
  const bool theInteriorFlag)
{
  ...
  if (BS.GetType() == GeomAbs_Plane)
  {
    ...
    occ::handle<TColgp_HArray1OfPnt> aPlanePnts = new TColgp_HArray1OfPnt(1, 5);
  ...
  occ::handle<TColgp_HArray1OfPnt> aFacePoints = new TColgp_HArray1OfPnt(1, aWirePoints.Length());
  {
    int aPntIndex = 1;
    for (NCollection_Sequence<gp_Pnt>::Iterator aPntIter(aWirePoints); aPntIter.More(); aPntIter.Next())
    {
