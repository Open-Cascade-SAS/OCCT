...
bool OpenGl_PrimitiveArray::initNormalVbo (const occ::handle<OpenGl_Context>& theCtx) const
{
  ...
  const int aNbVertexes = (isAttribMutable || !isAttribInterleaved) ? myAttribs->NbMaxElements() : myAttribs->NbElements;
  ...
  const int aNbIndexes = !myIndices->IsMutable() ? myIndices->NbElements : myIndices->NbMaxElements();
...
void OpenGl_PrimitiveArray::drawEdges (const occ::handle<OpenGl_Workspace>& theWorkspace) const
{
  ...
  {
    aGlContext->ShaderManager()->BindLineProgram (occ::handle<OpenGl_TextureSet>(), anAspect->Aspect()->EdgeLineType(),
...
void OpenGl_PrimitiveArray::drawMarkers (const occ::handle<OpenGl_Workspace>& theWorkspace) const
{
  ...
  else if (const occ::handle<OpenGl_PointSprite>& aSprite = anAspectMarker->SpriteRes (aCtx, theWorkspace->ToHighlight()))
  {
                                             ...
                                             bool theIsDetailed) const
{
  ...
  const int aNbIndices = !myVboIndices.IsNull() ? myVboIndices->GetElemsNb() : myVboAttribs->GetElemsNb();
...
void OpenGl_PrimitiveArray::Render (const occ::handle<OpenGl_Workspace>& theWorkspace) const
{
  ...
  if (!myIsVboInit)
  {
    ...
    bool toKeepData = myDrawMode == GL_POINTS
