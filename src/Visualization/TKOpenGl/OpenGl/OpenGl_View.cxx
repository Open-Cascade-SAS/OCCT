...
: Graphic3d_CView  (theMgr),
  ...
  myRaytraceBVHBuilder (new BVH_BinnedBuilder<float, 3, BVH_Constants_NbBinsBest> (BVH_Constants_LeafNodeSizeAverage,
...
void OpenGl_View::initTextureEnv (const occ::handle<OpenGl_Context>& theContext)
{
  ...
  occ::handle<OpenGl_Texture> aTextureEnv = new OpenGl_Texture (myTextureEnvData->GetId(), myTextureEnvData->GetParams());
...
static void SetMinMaxValuesCallback (Graphic3d_CView* theView)
{
  ...
  if (!aBox.IsVoid())
  {
    ...
    Graphic3d_Vec3 aMinVec ((float )aMin.X(), (float )aMin.Y(), (float )aMin.Z());
    Graphic3d_Vec3 aMaxVec ((float )aMax.X(), (float )aMax.Y(), (float )aMax.Z());
                                      ...
                                      bool theToUpdatePBREnv)
{
  ...
  occ::handle<Graphic3d_CubeMap> aCubeMap = Handle(Graphic3d_CubeMap)::DownCast (aNewMap);
  ...
  if (occ::handle<Graphic3d_Texture2D> aTextureMap = Handle(Graphic3d_Texture2D)::DownCast (aNewMap))
  {
...
int OpenGl_View::ZLayerMax() const
{
  ...
  for (NCollection_List<occ::handle<Graphic3d_Layer>>::Iterator aLayerIter (myZLayers.Layers()); aLayerIter.More(); aLayerIter.Next())
  {
...
{
  occ::handle<OpenGl_FrameBuffer> aFrameBuffer = Handle(OpenGl_FrameBuffer)::DownCast (theFbo);
...
void OpenGl_View::FBOGetDimensions (const occ::handle<Standard_Transient>& theFbo,
                                    ...
                                    int& theHeightMax)
{
  const occ::handle<OpenGl_FrameBuffer> aFrameBuffer = Handle(OpenGl_FrameBuffer)::DownCast (theFbo);
  if (aFrameBuffer.IsNull())
  {
    return;
  }
...
bool OpenGl_View::prepareFrameBuffers (Graphic3d_Camera::Projection& theProj)
{
  ...
  const Graphic3d_Vec2i aRendSize (int(myRenderParams.RenderResolutionScale * aSizeX + 0.5f),
     ...
     && aCtx->HasPBR())
    {
      ...
      if (myPBREnvState == OpenGl_PBREnvState_CREATED)
      {
        ...
        if (!aCtx->GetResource (THE_SHARED_ENV_LUT_KEY, anEnvLUT))
        {
          ...
          {
            aPixMap->InitWrapper (Image_Format_RGF, (uint8_t*)Textures_EnvLUT, Textures_EnvLUTSize, Textures_EnvLUTSize);
          }
          else
          {
            aPixMap->InitZero (anImgFormat, Textures_EnvLUTSize, Textures_EnvLUTSize);
            Image_PixMap aPixMapRG;
            aPixMapRG.InitWrapper (Image_Format_RGF, (uint8_t*)Textures_EnvLUT, Textures_EnvLUTSize, Textures_EnvLUTSize);
          ...
          if (!aTexFormat.IsValid()
           || !anEnvLUT->Init (aCtx, aTexFormat, Graphic3d_Vec2i((int)Textures_EnvLUTSize), Graphic3d_TypeOfTexture_2D, aPixMap.get()))
          {
  ...
  const occ::handle<Graphic3d_LightSet>& aLights = myRenderParams.ShadingModel == Graphic3d_TypeOfShadingModel_Unlit ? myNoShadingLight : myLights;
  if (!aLights.IsNull())
  {
...
void OpenGl_View::renderShadowMap (const occ::handle<OpenGl_ShadowMap>& theShadowMap)
{
  ...
  myWorkspace->SetRenderFilter (myWorkspace->RenderFilter() & ~(int)OpenGl_RenderFilter_SkipTrsfPersistence);
                          ...
                          const bool       theToDrawImmediate)
{
  ...
  if (aContext->core11ffp != NULL)
  {
    ...
    for (int aClipPlaneId = GL_CLIP_PLANE0; aClipPlaneId < GL_CLIP_PLANE0 + aMaxPlanes; ++aClipPlaneId)
    {
   ...
   || aLightsRevision != myLightsRevision)
  {
    ...
    aManager->UpdateLightSourceStateTo (aLights, SpecIBLMapLevels(), myShadowMaps->IsValid() ? myShadowMaps : occ::handle<OpenGl_ShadowMapArray>());
  ...
  if (!toRenderGL)
  {
    ...
    if (!toRenderGL)
    {
      ...
      if (aCtx->arbFBOBlit != NULL)
      {
        ...
        const int aPrevFilter = myWorkspace->RenderFilter() & ~(int )(OpenGl_RenderFilter_NonRaytraceableOnly);
...
void OpenGl_View::updateSkydomeBg (const occ::handle<OpenGl_Context>& theCtx)
{
  ...
  const int anOldViewport[4] = {theCtx->Viewport()[0], theCtx->Viewport()[1], theCtx->Viewport()[2], theCtx->Viewport()[3]};
