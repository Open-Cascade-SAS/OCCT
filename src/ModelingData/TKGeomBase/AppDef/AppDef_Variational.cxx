...
AppDef_Variational::AppDef_Variational(
  ...
  const int                                LastPoint,
  const occ::handle<AppParCurves_HArray1OfConstraintCouple>& TheConstraints,
  const int                                MaxDegree,
...

{
  ...
  gp_Pnt   P3d;
  {
    occ::handle<TColStd_HArray2OfReal> PolynomialIntervalsPtr =
      new TColStd_HArray2OfReal(1, NbElem, 1, 2);

    occ::handle<TColStd_HArray1OfInteger> NbCoeffPtr = new TColStd_HArray1OfInteger(1, myMaxSegment);

    int              size     = myMaxSegment * (myMaxDegree + 1) * myDimension;
    occ::handle<TColStd_HArray1OfReal> CoeffPtr = new TColStd_HArray1OfReal(1, size);

    CoeffPtr->Init(0.);

    occ::handle<TColStd_HArray1OfReal> IntervallesPtr = new TColStd_HArray1OfReal(1, NbElem + 1);
    ...
    */
    ...
    {
      occ::handle<TColStd_HArray2OfReal>    PolesPtr;
      occ::handle<TColStd_HArray1OfInteger> Mults;
...
//=======================================================================
//
const occ::handle<TColStd_HArray1OfReal>& AppDef_Variational::Parameters() const
{
  if (myIsDone == false)
    throw StdFail_NotDone();
  return myParameters;
...
//=======================================================================
//
const occ::handle<TColStd_HArray1OfReal>& AppDef_Variational::Knots() const
{
  if (myIsDone == false)
    throw StdFail_NotDone();
  return myKnots;
...
bool AppDef_Variational::SetConstraints(
  const occ::handle<AppParCurves_HArray1OfConstraintCouple>& aConstraint)
...
//=======================================================================
//
void AppDef_Variational::SetParameters(const occ::handle<TColStd_HArray1OfReal>& param)
{
...
//=======================================================================
//
bool AppDef_Variational::SetKnots(const occ::handle<TColStd_HArray1OfReal>& knots)
{
                                  ...
                                  NCollection_Array1<double>&  Ecarts)
{
  ...
  occ::handle<TColStd_HArray1OfReal>    CurrentTi, NewTi, OldTi;
  occ::handle<TColStd_HArray2OfInteger> Dependence;
  bool                 lestim, ToOptim, iscut;
  bool                 isnear = false, again = true;
  int                 NbEst, ICDANA, NumPnt, Iter;
  int                 MaxNbEst = 5;
  double VOCRI[3] = {BigValue, BigValue, BigValue}, EROLD = BigValue, VALCRI[3],
...
{
  int MxDeg = Curve->Base().WorkDegree(), NbElm = Curve->NbElements(),
                   NbDim = Curve->Dimension();

  occ::handle<FEmTool_HAssemblyTable> AssTable;
                             ...
                             const int Decima) const
{
  ...
  int      NbrPnt = Ti.Length(), TiFirst = Ti.Lower(), TiLast = Ti.Upper(),
...

{
  ...
  else
  {
    ...
    int adr1 = (ipnt - myFirstPoint - 1) * myDimension + 1,
                                               ...
                                               FEmTool_Assembly&            A) const
{
  ...
  int IndexOfConstraint, Ng3d, Ng2d, NBeg2d, NPass, NgPC1, NTang3d, NTang2d, Point,
