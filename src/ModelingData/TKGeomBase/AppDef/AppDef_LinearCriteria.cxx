...
void AppDef_LinearCriteria::SetParameters(const occ::handle<TColStd_HArray1OfReal>& Parameters)
{
...
void AppDef_LinearCriteria::SetCurve(const occ::handle<FEmTool_Curve>& C)
{
  ...
  if (myCurve.IsNull())
  {
    ...
    occ::handle<TColStd_HArray2OfReal> Coeff = new TColStd_HArray2OfReal(0, 0, 1, NbDim);
...
occ::handle<FEmTool_HAssemblyTable> AppDef_LinearCriteria::AssemblyTable() const
{
  if (myCurve.IsNull())
  ...
  occ::handle<FEmTool_HAssemblyTable> AssTable = new FEmTool_HAssemblyTable(1, NbDim, 1, NbElm);

  occ::handle<TColStd_HArray1OfInteger> GlobIndex, Aux;
...
occ::handle<TColStd_HArray2OfInteger> AppDef_LinearCriteria::DependenceTable() const
{
  if (myCurve.IsNull())
  ...
  occ::handle<TColStd_HArray2OfInteger> DepTab = new TColStd_HArray2OfInteger(1, Dim, 1, Dim, 0);
  int                 i;
  for (i = 1; i <= Dim; i++)
                                                      ...
                                                      double&      J3)
{
  ...
  occ::handle<TColStd_HArray2OfReal> Coeff;
...
void AppDef_LinearCriteria::InputVector(const math_Vector&                    X,
                                        const occ::handle<FEmTool_HAssemblyTable>& AssTable)
{
  int NbDim = myCurve->Dimension(), NbElm = myCurve->NbElements();
  int MxDeg = 0;
  MxDeg                  = myCurve->Base().WorkDegree();
  ...
  occ::handle<TColStd_HArray1OfInteger> GlobIndex;
