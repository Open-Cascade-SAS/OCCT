...
#ifndef _ExtremaCC_BSplineCurve_HeaderFile
#define _ExtremaCC_BSplineCurve_HeaderFile
...
//! @brief B-spline curve evaluator wrapper for curve-curve extrema.
//!
//! Provides uniform interface for evaluating B-spline curve geometry,
//! compatible with ExtremaCC_Numerical and ExtremaCC_GridEvaluator2D templates.
class ExtremaCC_BSplineCurve
{
public:
  DEFINE_STANDARD_ALLOC

  //! Constructor with B-spline curve (full domain).
  Standard_EXPORT explicit ExtremaCC_BSplineCurve(
    const occ::handle<Geom_BSplineCurve>& theCurve);

  //! Constructor with B-spline curve and parameter domain.
  Standard_EXPORT ExtremaCC_BSplineCurve(const occ::handle<Geom_BSplineCurve>& theCurve,
  ...
  bool IsBounded() const { return true; } // B-spline curves are always bounded

  //! Returns the domain.
  const ExtremaCC::Domain1D& Domain() const { return myDomain; }

  //! Returns the B-spline curve.
  const occ::handle<Geom_BSplineCurve>& Curve() const { return myCurve; }

private:
  occ::handle<Geom_BSplineCurve> myCurve;   //!< B-spline curve
  ExtremaCC::Domain1D            myDomain;  //!< Parameter domain
};

#endif // _ExtremaCC_BSplineCurve_HeaderFile
