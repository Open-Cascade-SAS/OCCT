...
#ifndef _ExtremaCS_SurfaceOfRevolution_HeaderFile
#define _ExtremaCS_SurfaceOfRevolution_HeaderFile
...
//! @brief Surface of revolution evaluator wrapper for curve-surface extrema.
//!
//! Provides uniform interface for evaluating surface of revolution geometry,
//! compatible with ExtremaCS_Numerical template.
class ExtremaCS_SurfaceOfRevolution
{
public:
  DEFINE_STANDARD_ALLOC

  //! Constructor with surface of revolution (uses natural domain).
  explicit ExtremaCS_SurfaceOfRevolution(const Handle(Geom_SurfaceOfRevolution)& theSurface)
      : mySurface(theSurface)
  {
    theSurface->Bounds(myDomain.UMin, myDomain.UMax, myDomain.VMin, myDomain.VMax);
  }

  //! Constructor with surface of revolution and parameter domain.
  ExtremaCS_SurfaceOfRevolution(const Handle(Geom_SurfaceOfRevolution)& theSurface,
  ...
  const Handle(Geom_SurfaceOfRevolution)& Surface() const { return mySurface; }

private:
  Handle(Geom_SurfaceOfRevolution) mySurface; //!< Surface of revolution
  ExtremaCS::Domain2D              myDomain;  //!< Parameter domain
};

#endif // _ExtremaCS_SurfaceOfRevolution_HeaderFile
