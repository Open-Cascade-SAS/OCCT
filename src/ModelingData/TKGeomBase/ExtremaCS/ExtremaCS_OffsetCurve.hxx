...
#ifndef _ExtremaCS_OffsetCurve_HeaderFile
#define _ExtremaCS_OffsetCurve_HeaderFile
...
//! @brief Offset curve evaluator wrapper for curve-surface extrema.
//!
//! Provides uniform interface for evaluating offset curve geometry,
//! compatible with ExtremaCS_Numerical template.
class ExtremaCS_OffsetCurve
{
public:
  DEFINE_STANDARD_ALLOC

  //! Constructor with offset curve (uses natural domain).
  explicit ExtremaCS_OffsetCurve(const Handle(Geom_OffsetCurve)& theCurve)
      : myCurve(theCurve)
  {
  ...
  //! Constructor with offset curve and parameter domain.
  ExtremaCS_OffsetCurve(const Handle(Geom_OffsetCurve)& theCurve,
  ...
  const Handle(Geom_OffsetCurve)& Curve() const { return myCurve; }

private:
  Handle(Geom_OffsetCurve) myCurve;  //!< Offset curve
  ExtremaCS::Domain1D      myDomain; //!< Parameter domain
};

#endif // _ExtremaCS_OffsetCurve_HeaderFile
