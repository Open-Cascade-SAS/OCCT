...
#ifndef _ExtremaCS_Cylinder_HeaderFile
#define _ExtremaCS_Cylinder_HeaderFile
...
//! @brief Cylinder surface evaluator wrapper for curve-surface extrema.
//!
//! Provides uniform interface for evaluating cylinder geometry,
//! compatible with ExtremaCS_Numerical template.
class ExtremaCS_Cylinder
{
public:
  DEFINE_STANDARD_ALLOC

  //! Constructor with cylinder geometry (default domain: U in [0, 2*PI], V unbounded).
  explicit ExtremaCS_Cylinder(const gp_Cylinder& theCylinder)
  ...
  ExtremaCS_Cylinder(const gp_Cylinder& theCylinder, const ExtremaCS::Domain2D& theDomain)
      : myCylinder(theCylinder),
        myDomain(theDomain)
  {
  }
  ...
  gp_Pnt Value(double theU, double theV) const { return ElSLib::Value(theU, theV, myCylinder); }
  ...
  {
    ElSLib::D1(theU, theV, myCylinder, thePt, theD1U, theD1V);
  }
  ...
  {
    ElSLib::D2(theU, theV, myCylinder, thePt, theD1U, theD1V, theD2UU, theD2VV, theD2UV);
  }
...
#endif // _ExtremaCS_Cylinder_HeaderFile
