...
#ifndef _ExtremaCS_BezierSurface_HeaderFile
#define _ExtremaCS_BezierSurface_HeaderFile
...
//! @brief Bezier surface evaluator wrapper for curve-surface extrema.
//!
//! Provides uniform interface for evaluating Bezier surface geometry,
//! compatible with ExtremaCS_Numerical template.
class ExtremaCS_BezierSurface
{
public:
  DEFINE_STANDARD_ALLOC

  //! Constructor with Bezier surface (uses natural domain [0,1] x [0,1]).
  explicit ExtremaCS_BezierSurface(const Handle(Geom_BezierSurface)& theSurface)
      : mySurface(theSurface)
  {
    // Bezier surfaces are defined on [0,1] x [0,1]
    myDomain.UMin = 0.0;
    myDomain.UMax = 1.0;
    myDomain.VMin = 0.0;
    myDomain.VMax = 1.0;
  ...
  ExtremaCS_BezierSurface(const Handle(Geom_BezierSurface)& theSurface,
...
private:
  Handle(Geom_BezierSurface) mySurface; //!< Bezier surface
  ExtremaCS::Domain2D        myDomain;  //!< Parameter domain
};

#endif // _ExtremaCS_BezierSurface_HeaderFile
