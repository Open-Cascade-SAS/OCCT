...
public:
  ...
  gp_Pnt Value1(double theU, double theV) const { return ElSLib::Value(theU, theV, myTorus1); }

  //! Evaluate point on second torus.
  //! @param theU U parameter
  //! @param theV V parameter
  //! @return point on torus
  gp_Pnt Value2(double theU, double theV) const { return ElSLib::Value(theU, theV, myTorus2); }
...
private:
  ...
  void computeConcentricCase(double theTol, ExtremaSS::SearchMode theMode) const
  {
    ...
    const bool aSearchMin =
      (theMode == ExtremaSS::SearchMode::MinMax || theMode == ExtremaSS::SearchMode::Min);
    const bool aSearchMax =
      (theMode == ExtremaSS::SearchMode::MinMax || theMode == ExtremaSS::SearchMode::Max);
    ...
    aMinDist =
      std::min(aMinDist,
               std::abs((myMajorRadius1 + myMinorRadius1) - (myMajorRadius2 + myMinorRadius2)));
    // Inner-inner
    aMinDist =
      std::min(aMinDist,
               std::abs((myMajorRadius1 - myMinorRadius1) - (myMajorRadius2 - myMinorRadius2)));
  ...
  void computeCoaxialCase(double theTol, ExtremaSS::SearchMode theMode) const
  {
    ...
    {
      const double aCenterDistSame =
        std::sqrt((myMajorRadius1 - myMajorRadius2) * (myMajorRadius1 - myMajorRadius2)
                  + aAxisDist * aAxisDist);
  ...
  void searchExtremaAtU(double                theU1,
                        double                theU2,
                        double                theTol,
                        ExtremaSS::SearchMode theMode) const
  {
    ...
    if (aCenterDist < ExtremaSS::THE_DEFAULT_TOLERANCE)
    {
      ...
      if (std::abs(aDotAxis) > 1.0 - ExtremaSS::THE_ANGULAR_TOLERANCE)
      {
        ...
        addExtremum(theU1,
                    ...
                    theU2,
                    0.0,
                    std::pow(myMinorRadius1 - myMinorRadius2, 2),
                    true,
                    theTol);
        addExtremum(theU1,
                    0.0,
    ...
    const double aSinV1 =
      myAxis1.IsParallel(aC1C2Dir, M_PI_2) ? 0.0 : myAxis1.Crossed(aTubeDir1).Dot(aC1C2Dir);
    const double aV1_near = std::atan2(aSinV1, aCosV1);

    const double aCosV2 = -aTubeDir2.Dot(aC1C2Dir);
    const double aSinV2 =
      myAxis2.IsParallel(aC1C2Dir, M_PI_2) ? 0.0 : myAxis2.Crossed(aTubeDir2).Dot(-aC1C2Dir);
    ...
    for (int i = 0; i <= aNbU; ++i)
    {
      ...
      for (int j = 0; j <= aNbU; ++j)
      {
        ...
        else
        {
          ...
          aV1 = std::atan2(myAxis1.Crossed(aTubeDir1).Dot(aC1C2Dir), aTubeDir1.Dot(aC1C2Dir));
          // V2 where tube 2 points toward C1
          aV2 = std::atan2(myAxis2.Crossed(aTubeDir2).Dot(-aC1C2Dir), -aTubeDir2.Dot(aC1C2Dir));
        }
  ...
  void search3DExtrema(double                theFixedU1,
                       double                theFixedV1,
                       double                theU2Min,
                       double                theU2Max,
                       double                theV2Min,
                   ...
                   double theTol) const
  {
    ...
    if (myDomain.has_value())
    {
      if (theU1 < myDomain->Domain1.UMin - theTol || theU1 > myDomain->Domain1.UMax + theTol
          || theV1 < myDomain->Domain1.VMin - theTol || theV1 > myDomain->Domain1.VMax + theTol
          || theU2 < myDomain->Domain2.UMin - theTol || theU2 > myDomain->Domain2.UMax + theTol
          || theV2 < myDomain->Domain2.VMin - theTol || theV2 > myDomain->Domain2.VMax + theTol)
      {
        return;
      }
    }
