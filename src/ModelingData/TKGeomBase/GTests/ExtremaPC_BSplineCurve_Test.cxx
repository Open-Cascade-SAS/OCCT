...
TEST_F(ExtremaPC_BSplineCurveTest, PointOnCurve_Start)
{
  ...
  ExtremaPC::Result      aResult = anEval.PerformWithEndpoints(aPoint, ExtremaPC::Domain1D{anAdaptor.FirstParameter(),
                                              anAdaptor.LastParameter()}, THE_TOL);
...
TEST_F(ExtremaPC_BSplineCurveTest, PartialRange_FirstHalf)
{
  ...
  ExtremaPC_BSplineCurve anEval(aBSpline);
  ExtremaPC::Result      aResult = anEval.PerformWithEndpoints(aPoint, ExtremaPC::Domain1D{0.0, 0.5}, THE_TOL);
...
TEST_F(ExtremaPC_BSplineCurveTest, PartialRange_SecondHalf)
{
  ...
  ExtremaPC_BSplineCurve anEval(aBSpline);
  ExtremaPC::Result      aResult = anEval.PerformWithEndpoints(aPoint, ExtremaPC::Domain1D{0.5, 1.0}, THE_TOL);
...
//==================================================================================================
// PerformWithHint tests (NextProject pattern)
//==================================================================================================

TEST_F(ExtremaPC_BSplineCurveTest, PerformWithHint_GoodHint)
{
  // Test that PerformWithHint works when given a good hint
  Handle(Geom_BSplineCurve) aBSpline = createCubicBSpline();
  GeomAdaptor_Curve         anAdaptor(aBSpline);

  // Point near middle of curve
  gp_Pnt aPoint(1.5, 2.0, 0.0);

  // First, find the actual closest point using full search
  ExtremaPC_BSplineCurve anEval(aBSpline);
  ExtremaPC::Result      aFullResult = anEval.PerformWithEndpoints(aPoint, ExtremaPC::Domain1D{anAdaptor.FirstParameter(),
                                                       anAdaptor.LastParameter()}, THE_TOL);
  ASSERT_TRUE(aFullResult.IsDone());
  ASSERT_GE(aFullResult.NbExt(), 1);

  double aRefParam = aFullResult[aFullResult.MinIndex()].Parameter;
  ...
  ASSERT_TRUE(aHintResult.IsDone());
  ASSERT_GE(aHintResult.NbExt(), 1);

  // Should find similar result
  double aHintParam = aHintResult[aHintResult.MinIndex()].Parameter;
  EXPECT_NEAR(aHintParam, aRefParam, 0.01);

  // Distance should be similar
  EXPECT_NEAR(aHintResult.MinSquareDistance(), aFullResult.MinSquareDistance(), THE_TOL * 10);
}

TEST_F(ExtremaPC_BSplineCurveTest, PerformWithHint_SequentialPoints)
{
  // Test sequential point projection (typical use case for NextProject pattern)
  Handle(Geom_BSplineCurve) aBSpline = createCubicBSpline();
  GeomAdaptor_Curve         anAdaptor(aBSpline);

  // Generate points along a line that "sweeps" near the curve
  double aHintU = anAdaptor.FirstParameter();
  int    aSuccessCount = 0;

  for (int i = 0; i < 10; ++i)
  {
    // Points along a line parallel to the curve's general direction
    double t      = i / 9.0;
    ...
    if (aResult.IsDone() && aResult.NbExt() >= 1)
    {
      // Use this result as hint for next point
      aHintU = aResult[aResult.MinIndex()].Parameter;
      aSuccessCount++;

      // Verify the point is on the curve
      gp_Pnt aPtOnCurve = aBSpline->Value(aHintU);
      EXPECT_NEAR(aResult[aResult.MinIndex()].Point.Distance(aPtOnCurve), 0.0, THE_TOL);
    }
  }

  // Should succeed for most/all points
  EXPECT_GE(aSuccessCount, 8);
}

TEST_F(ExtremaPC_BSplineCurveTest, PerformWithHint_BadHint)
{
  // Test that PerformWithHint still works with a bad hint (falls back to grid search)
  Handle(Geom_BSplineCurve) aBSpline = createCubicBSpline();
  GeomAdaptor_Curve         anAdaptor(aBSpline);

  gp_Pnt aPoint(2.5, 1.5, 0.0);  // Near end of curve
  ...
  gp_Pnt aPtOnCurve = aBSpline->Value(aResult[aResult.MinIndex()].Parameter);
  EXPECT_NEAR(aResult[aResult.MinIndex()].Point.Distance(aPtOnCurve), 0.0, THE_TOL);
}
...
TEST_F(ExtremaPC_BSplineCurveTest, Performance_PerformWithHint_vs_FullSearch)
{
  // Compare performance of PerformWithHint vs full search for sequential points
  Handle(Geom_BSplineCurve) aBSpline = createCubicBSpline();
  GeomAdaptor_Curve         anAdaptor(aBSpline);
  ExtremaPC_BSplineCurve    anEval(aBSpline);

  const int aNbPoints = 100;
  const int aNbIter   = 50;

  // Generate sequential points along a line
  std::vector<gp_Pnt> aPoints;
  for (int i = 0; i < aNbPoints; ++i)
  {
    double t = i / (aNbPoints - 1.0);
    aPoints.push_back(gp_Pnt(t * 3.0, 1.5 + t * 0.3, 0.2));
  }

  // Benchmark full search
  auto aStartFull = std::chrono::high_resolution_clock::now();
  for (int iter = 0; iter < aNbIter; ++iter)
  {
    for (const gp_Pnt& aP : aPoints)
    {
      ExtremaPC::Result aRes = anEval.PerformWithEndpoints(aP, ExtremaPC::Domain1D{anAdaptor.FirstParameter(),
                                               anAdaptor.LastParameter()}, THE_TOL);
      (void)aRes;  // Prevent optimization
    }
  }
  auto aEndFull = std::chrono::high_resolution_clock::now();
  auto aFullTime = std::chrono::duration_cast<std::chrono::microseconds>(aEndFull - aStartFull).count();

  // Benchmark PerformWithHint
  auto aStartHint = std::chrono::high_resolution_clock::now();
  for (int iter = 0; iter < aNbIter; ++iter)
  {
    double aHintU = anAdaptor.FirstParameter();
    for (const gp_Pnt& aP : aPoints)
    {
      ...
      {
        aHintU = aRes[aRes.MinIndex()].Parameter;
      }
    }
  }
  auto aEndHint = std::chrono::high_resolution_clock::now();
  auto aHintTime = std::chrono::duration_cast<std::chrono::microseconds>(aEndHint - aStartHint).count();

  double aSpeedup = static_cast<double>(aFullTime) / aHintTime;

  std::cout << "  [PERF] Sequential projection (" << aNbPoints << " points x " << aNbIter << " iterations):\n";
  std::cout << "    Full search:     " << aFullTime << " us\n";
  std::cout << "    PerformWithHint: " << aHintTime << " us\n";
  std::cout << "    Speedup:         " << std::fixed << std::setprecision(2) << aSpeedup << "x\n";

  // PerformWithHint should be faster for sequential points
  EXPECT_GT(aSpeedup, 1.0);
}
