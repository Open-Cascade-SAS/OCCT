                                                            ...
                                                            int& IndexStrip) const
{
  ...
  {
    const NCollection_Sequence<NCollection_Sequence<occ::handle<AdvApp2Var_Iso>>>& aSeq = anUVIter == 0 ? myUConstraints : myVConstraints;
    int                  i    = 1;
    for (NCollection_Sequence<NCollection_Sequence<occ::handle<AdvApp2Var_Iso>>>::Iterator aConstIter(aSeq); aConstIter.More();
         aConstIter.Next(), i++)
    {
      const NCollection_Sequence<occ::handle<AdvApp2Var_Iso>>& S = aConstIter.Value();
      int        j = 1;
      for (NCollection_Sequence<occ::handle<AdvApp2Var_Iso>>::Iterator anIsoIter(S); anIsoIter.More(); anIsoIter.Next(), j++)
      {
...
{
  NCollection_Sequence<occ::handle<AdvApp2Var_Iso>>& S0 = theIso->Type() == GeomAbs_IsoV ? myUConstraints.ChangeValue(IndexStrip)
...
{
  for (NCollection_Sequence<occ::handle<AdvApp2Var_Node>>::Iterator aNodeIter(myNodeConstraints); aNodeIter.More();
       aNodeIter.Next())
  {
...
{
  int i = 1;
  for (NCollection_Sequence<NCollection_Sequence<occ::handle<AdvApp2Var_Iso>>>::Iterator anUConstIter(myUConstraints); anUConstIter.More();
       anUConstIter.Next(), ++i)
  {
...

  {
    ...
    for (NCollection_Sequence<occ::handle<AdvApp2Var_Iso>>::Iterator aStripIter(S0); aStripIter.More(); aStripIter.Next())
    {
...
void AdvApp2Var_Framework::UpdateInV(const double CuttingValue)
{
...

  {
    ...
    for (NCollection_Sequence<occ::handle<AdvApp2Var_Iso>>::Iterator anIsoIter(S0); anIsoIter.More(); anIsoIter.Next())
    {
      const occ::handle<AdvApp2Var_Iso>& anIso = anIsoIter.Value();
...
const occ::handle<TColStd_HArray1OfReal>& AdvApp2Var_Framework::UEquation(
  const int IndexIso,
  const int IndexStrip) const
{
...
const occ::handle<TColStd_HArray1OfReal>& AdvApp2Var_Framework::VEquation(
  const int IndexIso,
  const int IndexStrip) const
{
