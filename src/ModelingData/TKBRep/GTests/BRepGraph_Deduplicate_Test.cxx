...
int nbPCurveEntries(const BRepGraph& theGraph)
{
  ...
  {
    const BRepGraph_EdgeId anEdgeId = anEdgeIt.CurrentId();
    const NCollection_Vector<BRepGraph_CoEdgeId>& aCoEdgeIdxs = theGraph.Topo().Edges().CoEdges(anEdgeId);
...
{
  BRepGraph aGraph;
  BRepGraph_Builder::Perform(aGraph, makeTwoCopiedIdenticalFaces());
...
TEST(BRepGraph_DeduplicateTest, SingleFace_NoSurfaceRewrite)
{
  ...
  BRepGraph aGraph;
  BRepGraph_Builder::Perform(aGraph, anExp.Current());
  ASSERT_TRUE(aGraph.IsDone());
...
{
  BRepGraph aGraph;
  // Do not call BRepGraph_Builder::Perform() - graph is not done.
...
TEST(BRepGraph_DeduplicateTest, MultipleCopies_NWayDedup)
{
  ...
  BRepGraph aGraph;
  BRepGraph_Builder::Perform(aGraph, makeNCopiedIdenticalFaces(aNbCopies));
  ASSERT_TRUE(aGraph.IsDone());
...
{
  BRepGraph aGraph;
  BRepGraph_Builder::Perform(aGraph, makeMixedCompound());
  ASSERT_TRUE(aGraph.IsDone());
...
{
  BRepGraph aGraph;
  BRepGraph_Builder::Perform(aGraph, makeNestedCompound());
  ASSERT_TRUE(aGraph.IsDone());
...
{
  BRepGraph aGraph;
  BRepGraph_Builder::Perform(aGraph, makeThreeDistinctPrimitives());
  ASSERT_TRUE(aGraph.IsDone());
...
{
  BRepGraph aGraph;
  BRepGraph_Builder::Perform(aGraph, makeTwoIdenticalBoxes());
  ASSERT_TRUE(aGraph.IsDone());
...
TEST(BRepGraph_DeduplicateTest, MultiplePCurveDups_AllEdgesDeduped)
{
  ...
  BRepGraph aGraph;
  BRepGraph_Builder::Perform(aGraph, aCopy.Shape());
  ASSERT_TRUE(aGraph.IsDone());
...
TEST(BRepGraph_DeduplicateTest, HistoryOff_NbRecordsUnchanged)
{
  ...
  BRepGraph aGraph2;
  BRepGraph_Builder::Perform(aGraph2, makeTwoCopiedIdenticalFaces());
...
TEST(BRepGraph_DeduplicateTest, AfterDedup_AllInlinePCurvesHaveCurve2d)
{
  ...
  {
    const BRepGraph_EdgeId anEdgeId = anEdgeIt.CurrentId();
    const NCollection_Vector<BRepGraph_CoEdgeId>& aCoEdgeIdxs = aGraph.Topo().Edges().CoEdges(anEdgeId);
    for (int aCEIter = 0; aCEIter < aCoEdgeIdxs.Length(); ++aCEIter)
    {
      ...
      EXPECT_TRUE(aCE.Curve2DRepId.IsValid())
        << "Edge " << anEdgeId.Index << " CoEdge " << aCEIter
        << " has null Curve2d after dedup";
    }
  }
}
...
{
  BRepGraph aGraph;
  BRepGraph_Builder::Perform(aGraph, makeNCopiedIdenticalFaces(4));
  ASSERT_TRUE(aGraph.IsDone());
...
TEST(BRepGraph_DeduplicateTest, ParallelBuild_SameResultAsSequential)
{
  ...
  BRepGraph aGraphSeq;
  BRepGraph_Builder::Perform(aGraphSeq, aCompound, false);
  ASSERT_TRUE(aGraphSeq.IsDone());

  BRepGraph aGraphPar;
  BRepGraph_Builder::Perform(aGraphPar, aCompound, true);
...
TEST(BRepGraph_DeduplicateTest, RoundTrip_TwoCopiedFaces_FewerSurfaces)
{
  ...
  BRepGraph aGraph1;
  BRepGraph_Builder::Perform(aGraph1, aCompound);
  ...
  BRepGraph aGraph2;
  BRepGraph_Builder::Perform(aGraph2, aReconstructed);
  ASSERT_TRUE(aGraph2.IsDone());
