...
TEST(BRepGraph_ChildExplorerTest, Box_EdgeOccurrences_Count24)
{
  ...
  int aCount = 0;
  for (BRepGraph_ChildExplorer anExp(aGraph, BRepGraph_SolidId::Start(), BRepGraph_NodeId::Kind::Edge);
       anExp.More();
       anExp.Next())
    ++aCount;
...
TEST(BRepGraph_ChildExplorerTest, Box_FaceOccurrences_Count6)
{
  ...
  int aFaceCount = 0;
  for (BRepGraph_ChildExplorer anExp(aGraph, BRepGraph_SolidId::Start(), BRepGraph_NodeId::Kind::Face);
       anExp.More();
       anExp.Next())
    ++aFaceCount;
...
TEST(BRepGraph_ChildExplorerTest, Box_VertexOccurrences)
{
  ...
  for (BRepGraph_ChildExplorer anExp(aGraph, BRepGraph_SolidId::Start(), BRepGraph_NodeId::Kind::Vertex);
       anExp.More();
       anExp.Next())
  {
...
TEST(BRepGraph_ChildExplorerTest, AllDescendants_Recursive_YieldsAllKinds)
{
  ...
  int anEdgeCount  = 0;
  int aVertexCount = 0;
  for (BRepGraph_ChildExplorer anExp(aGraph, BRepGraph_SolidId::Start()); anExp.More(); anExp.Next())
  {
...
TEST(BRepGraph_ChildExplorerTest, Compound_FaceCount)
{
  ...
  BRepGraph aGraph;
  BRepGraph_Builder::Perform(aGraph, aComp);
  ASSERT_TRUE(aGraph.IsDone());

  int aCount = 0;
  for (BRepGraph_ChildExplorer anExp(aGraph, BRepGraph_CompoundId::Start(), BRepGraph_NodeId::Kind::Face);
       anExp.More();
       anExp.Next())
    ++aCount;
...
TEST(BRepGraph_ChildExplorerTest, DeepCompound_NoStackOverflow)
{
  ...
  BRepGraph aGraph;
  BRepGraph_Builder::Perform(aGraph, aInner);
  ASSERT_TRUE(aGraph.IsDone());
...
TEST(BRepGraph_ChildExplorerTest, CoEdgeTarget_Reachable)
{
  ...
  int aCount = 0;
  for (BRepGraph_ChildExplorer anExp(aGraph, BRepGraph_SolidId::Start(), BRepGraph_NodeId::Kind::CoEdge);
       anExp.More();
       anExp.Next())
  {
...
TEST(BRepGraph_ChildExplorerTest, CoEdgeTarget_FromFace_Count4)
{
  ...
  int aCount = 0;
  for (BRepGraph_ChildExplorer anExp(aGraph, BRepGraph_FaceId::Start(), BRepGraph_NodeId::Kind::CoEdge);
       anExp.More();
       anExp.Next())
  {
    EXPECT_EQ(anExp.Current().DefId.NodeKind, BRepGraph_NodeId::Kind::CoEdge);
...
TEST(BRepGraph_ChildExplorerTest, DirectChildren_RemovedFaceRef_IsSkipped)
{
  ...
  {
    BRepGraph_MutGuard<BRepGraphInc::FaceRef> aFaceRef = aGraph.Editor().Faces().MutRef(aRemovedRef);
...
TEST(BRepGraph_ChildExplorerTest, DirectChildren_CompoundChildren_Basic)
{
  ...
  int aCount = 0;
  for (BRepGraph_ChildExplorer anIt =
         makeDirectChildExplorer(aGraph, BRepGraph_CompoundId::Start(), BRepGraph_NodeId::Kind::Solid);
       anIt.More();
       anIt.Next())
  {
...
TEST(BRepGraph_ChildExplorerTest, Recursive_SharedProduct_ChildrenHaveDistinctContexts)
{
  ...
  const BRepGraph_OccurrenceId anOcc1 =
    aGraph.Editor().Products().AddOccurrence(anAssembly, aPart, TopLoc_Location(aT1));
  const BRepGraph_OccurrenceId anOcc2 =
    aGraph.Editor().Products().AddOccurrence(anAssembly, aPart, TopLoc_Location(aT2));
  ASSERT_TRUE(anOcc1.IsValid());
...
TEST(BRepGraph_ChildExplorerTest, Recursive_ProductPartRootContext_ComposedWithOccurrence)
{
  ...
  const BRepGraph_OccurrenceId anOcc =
    aGraph.Editor().Products().AddOccurrence(anAssembly, aPart, TopLoc_Location(aOccTrsf));
    ...
    for (const BRepGraph_OccurrenceRefId& aRefId : aPartDef.OccurrenceRefIds)
    {
      ...
      if (BRepGraph_NodeId::IsTopologyKind(anOccDef.ChildDefId.NodeKind))
      {
        BRepGraph_MutGuard<BRepGraphInc::OccurrenceRef> aMutRef =
          aGraph.Editor().Products().MutOccurrenceRef(aRefId);
