...
TEST(BRepGraph_ChildExplorerTest, RefAtMatchesUsagePathWithoutBuildingPath)
{
  BRepGraph aGraph;
  aGraph.Clear();
  ...
  BRepGraph_ChildExplorer anExplorer(aGraph,
                                     BRepGraph_SolidId::Start(),
                                     BRepGraph_NodeId::Kind::Edge);
  ASSERT_TRUE(anExplorer.More());

  const BRepGraph_UsagePath aPath = anExplorer.CurrentUsagePath();
  ASSERT_EQ(aPath.Size(), anExplorer.Depth());
  for (size_t aStep = 0; aStep < aPath.Size(); ++aStep)
  {
    EXPECT_EQ(anExplorer.NodeAt(aStep), aPath.Value(aStep).Node);
    EXPECT_EQ(anExplorer.RefAt(aStep), aPath.Value(aStep).Ref);
  }
  EXPECT_EQ(anExplorer.CurrentStepIndex(), aPath.Last().StepIndex);
  EXPECT_FALSE(anExplorer.RefAt(anExplorer.Depth()).IsValid());
}

TEST(BRepGraph_ChildIteratorTest, ShellChildren_PositionResumesWithoutReplaying)
{
  BRepGraph aGraph;
  aGraph.Clear();
  [[maybe_unused]] const BRepGraph::ShapesView::Result aBuildResult =
  ...
  BRepGraph_ChildIterator anIterator(aGraph, aShell);
  ASSERT_TRUE(anIterator.More());
  const BRepGraph_NodeId aFirst = anIterator.Current().Child;
  EXPECT_EQ(aFirst.NodeKind, BRepGraph_NodeId::Kind::Face);
  EXPECT_EQ(anIterator.Current().Link, BRepGraph_ChildIterator::LinkKind::Reference);
  EXPECT_TRUE(anIterator.Current().Ref.IsValid());

  const uint32_t aPosition = anIterator.CurrentPosition();
  anIterator.Next();
  ASSERT_TRUE(anIterator.More());
  const BRepGraph_NodeId aSecond = anIterator.Current().Child;
  EXPECT_NE(aSecond, aFirst);

  BRepGraph_ChildIterator aResumed(aGraph, aShell, aPosition);
  ASSERT_TRUE(aResumed.More());
  EXPECT_EQ(aResumed.Current().Child, aSecond);
  EXPECT_EQ(aResumed.Current().Ref, anIterator.Current().Ref);
}

TEST(BRepGraph_ChildIteratorTest, WireAndCoEdge_ExposeStructuralSteps)
{
  BRepGraph aGraph;
  aGraph.Clear();
  [[maybe_unused]] const BRepGraph::ShapesView::Result aBuildResult =
  ...
  BRepGraph_ChildIterator aWireChildren(aGraph, BRepGraph_WireId::Start());
  ASSERT_TRUE(aWireChildren.More());
  EXPECT_EQ(aWireChildren.Current().Child.NodeKind, BRepGraph_NodeId::Kind::CoEdge);
  EXPECT_EQ(aWireChildren.Current().Link, BRepGraph_ChildIterator::LinkKind::Structural);
  EXPECT_FALSE(aWireChildren.Current().Ref.IsValid());

  BRepGraph_ChildIterator aCoEdgeChildren(aGraph, aWireChildren.Current().Child);
  ASSERT_TRUE(aCoEdgeChildren.More());
  EXPECT_EQ(aCoEdgeChildren.Current().Child.NodeKind, BRepGraph_NodeId::Kind::Edge);
  EXPECT_EQ(aCoEdgeChildren.Current().Link, BRepGraph_ChildIterator::LinkKind::Structural);
  EXPECT_FALSE(aCoEdgeChildren.Current().Ref.IsValid());
}
...
TEST(BRepGraph_ChildExplorerTest, RootEqualsTarget_ReturnsSelf)
{
  ...
  EXPECT_EQ(aCurrent.DefId, BRepGraph_FaceId::Start());
...
TEST(BRepGraph_ChildExplorerTest, Compound_FaceCount)
{
  ...
  BRepGraph aGraph;
  aGraph.Clear();
  [[maybe_unused]] const BRepGraph::ShapesView::Result aBuildRes16 = aGraph.Shapes().Add(aComp);
  ASSERT_FALSE(aGraph.IsEmpty());
...
TEST(BRepGraph_ChildExplorerTest, DeepCompound_NoStackOverflow)
{
  ...
  BRepGraph aGraph;
  aGraph.Clear();
  [[maybe_unused]] const BRepGraph::ShapesView::Result aBuildRes18 = aGraph.Shapes().Add(aInner);
  ASSERT_FALSE(aGraph.IsEmpty());
...
TEST(BRepGraph_ChildExplorerTest, DeepPlacedAssembly_GrowsSparseLocationStack)
{
  BRepGraph aGraph;
  aGraph.Clear();
  const BRepGraph_VertexId aVertex =
    aGraph.Editor().Vertices().Add(gp_Pnt(), Precision::Confusion());
  ASSERT_TRUE(aVertex.IsValid());

  constexpr size_t                              THE_ASSEMBLY_DEPTH = 32;
  NCollection_LinearVector<BRepGraph_ProductId> aProducts(THE_ASSEMBLY_DEPTH + 1);
  for (size_t aLevel = 0; aLevel < THE_ASSEMBLY_DEPTH; ++aLevel)
  {
    aProducts.Append(aGraph.Editor().Products().Add());
  }
  aProducts.Append(aGraph.Editor().Products().Add(aVertex));

  gp_Trsf aTranslation;
  aTranslation.SetTranslation(gp_Vec(1.0, 0.0, 0.0));
  BRepGraph_OccurrenceId aParentOccurrence;
  for (size_t aLevel = 0; aLevel < THE_ASSEMBLY_DEPTH; ++aLevel)
  {
    aParentOccurrence = aGraph.Editor().Products().Append(aProducts.Value(aLevel),
                                                          aProducts.Value(aLevel + 1),
                                                          TopLoc_Location(aTranslation),
                                                          aParentOccurrence);
    ASSERT_TRUE(aParentOccurrence.IsValid());
  }

  BRepGraph_ChildExplorer anExplorer(aGraph, aProducts.First(), BRepGraph_NodeId::Kind::Vertex);
  ASSERT_TRUE(anExplorer.More());
  EXPECT_EQ(anExplorer.CurrentNode(), BRepGraph_NodeId(aVertex));
  EXPECT_NEAR(anExplorer.CurrentLocation().Transformation().TranslationPart().X(),
              static_cast<double>(THE_ASSEMBLY_DEPTH),
              Precision::Confusion());
  anExplorer.Next();
  EXPECT_FALSE(anExplorer.More());
}
...
TEST(BRepGraph_ChildExplorerTest, DirectChildren_ShellFaces_CountAndOrder)
{
  ...
  for (const BRepGraph_FaceRefId& aFaceRefId : aFaceRefIds)
  {
    ...
    if (!aFaceRefId.IsRemoved(aGraph))
    {
      anExpectedFaceIds.Append(aRef.ChildFaceId.Index);
    }
  }
  ...
  ASSERT_EQ(anActualFaceIds.Size(), anExpectedFaceIds.Size());
  for (size_t i = 0; i < anExpectedFaceIds.Size(); ++i)
  {
...
TEST(BRepGraph_ChildExplorerTest, DirectChildren_ProductShapeRoot_ViaOccurrenceRef)
{
  ...
  const BRepGraph_ProductId aProductId = aGraph.Editor().Products().Add(BRepGraph_SolidId::Start());
  ...
  // The shape root is reached via an occurrence whose ChildNodeId is the topology node.
...
TEST(BRepGraph_ChildExplorerTest, DirectChildren_ProductOccurrences_ExposeOccurrenceRefs)
{
  ...
  ASSERT_EQ(anOccurrenceRefs.Size(), 2);
...
TEST(BRepGraph_ChildExplorerTest, DirectChildren_RemovedFaceRef_IsSkipped)
{
  ...
  const NCollection_LinearVector<BRepGraph_FaceRefId>& aFaceRefIds =
    aGraph.Refs().Faces().IdsOf(aShellId);
  ASSERT_GT(aFaceRefIds.Size(), 0);
  const size_t aNbFaceRefsBefore = aFaceRefIds.Size();

  const BRepGraph_FaceRefId aRemovedRef    = aFaceRefIds.Value(0);
  const BRepGraph_FaceId    aRemovedFaceId = aGraph.Refs().Faces().Entry(aRemovedRef).ChildFaceId;
  ...
  EXPECT_EQ(aCount, aNbFaceRefsBefore - 1);
}
...
TEST(BRepGraph_ChildExplorerTest, DirectChildren_WireChildren_AreCoEdges)
{
  ...
  ASSERT_GT(aWireRefs.Size(), 0);

  const BRepGraph_WireId aWireId = aGraph.Refs().Wires().Entry(aWireRefs.Value(0)).ChildWireId;
  ...
  EXPECT_EQ(aCount, aCoEdges.Size());
}
...
TEST(BRepGraph_ChildExplorerTest, DirectChildren_CompoundChildren_Basic)
{
  ...
  BRepGraph aGraph;
  aGraph.Clear();
  [[maybe_unused]] const BRepGraph::ShapesView::Result aBuildRes29 = aGraph.Shapes().Add(aComp);
  ASSERT_FALSE(aGraph.IsEmpty());
...
TEST(BRepGraph_ChildExplorerTest, DirectChildren_ChainedTraversal_ParityWithRecursiveTraversal)
{
  ...
  BRepGraph aGraph;
  aGraph.Clear();
  [[maybe_unused]] const BRepGraph::ShapesView::Result aBuildRes30 = aGraph.Shapes().Add(aComp);
  ASSERT_FALSE(aGraph.IsEmpty());
...
TEST(BRepGraph_ChildExplorerTest, Recursive_SharedProduct_ChildrenHaveDistinctContexts)
{
  ...
  const BRepGraph_OccurrenceId anOcc1 =
    aGraph.Editor().Products().Append(anAssembly, aPart, TopLoc_Location(aT1));
  const BRepGraph_OccurrenceId anOcc2 =
    aGraph.Editor().Products().Append(anAssembly, aPart, TopLoc_Location(aT2));
  ASSERT_TRUE(anOcc1.IsValid());
...
TEST(BRepGraph_ChildExplorerTest, Recursive_ProductOccurrenceChain_ReachesNestedOccurrences)
{
  BRepGraph aGraph;
  aGraph.Clear();
  ...
  int  aCount            = 0;
  for (BRepGraph_ChildExplorer anIt(aGraph, aTopAssembly, BRepGraph_NodeId::Kind::Occurrence);
       anIt.More();
       anIt.Next())
  {
...
TEST(BRepGraph_ChildExplorerTest, Recursive_ProductPartRootContext_ComposedWithOccurrence)
{
  ...
  const BRepGraph_OccurrenceId anOcc =
    aGraph.Editor().Products().Append(anAssembly, aPart, TopLoc_Location(aOccTrsf));
  ...
  {
    const BRepGraphInc::ProductRelations& aPartRelations =
      aGraph.Topo().Products().Relations(aPart);
    for (const BRepGraph_OccurrenceRefId& aRefId : aPartRelations.OccurrenceRefIds)
    {
      const BRepGraphInc::OccurrenceRef& aOccRef = aGraph.Refs().Occurrences().Entry(aRefId);
      const BRepGraphInc::OccurrenceDef& anOccDef =
        aGraph.Topo().Occurrences().Definition(aOccRef.ChildOccurrenceId);
...
TEST(BRepGraph_ChildExplorerTest, DirectChildren_HighFanout_DirectChildrenComplete)
{
  ...
  BRepGraph aGraph;
  aGraph.Clear();
  [[maybe_unused]] const BRepGraph::ShapesView::Result aBuildRes33 = aGraph.Shapes().Add(aComp);
  ASSERT_FALSE(aGraph.IsEmpty());
...
TEST(BRepGraph_ChildExplorerTest, HighFanout_CompletesAllChildren)
{
  ...
  BRepGraph aGraph;
  aGraph.Clear();
  [[maybe_unused]] const BRepGraph::ShapesView::Result aBuildRes34 = aGraph.Shapes().Add(aComp);
  ASSERT_FALSE(aGraph.IsEmpty());
...
TEST(BRepGraph_ChildExplorerTest, StructuredBindings_NodeInstance)
{
  ...
  BRepGraph aGraph;
  aGraph.Clear();
  [[maybe_unused]] const BRepGraph::ShapesView::Result aBuildRes35 = aGraph.Shapes().Add(aComp);
  ASSERT_FALSE(aGraph.IsEmpty());
