...
bool hasAnyActiveUsage(const BRepGraph& theGraph, const BRepGraph_NodeId theChild)
{
  ...
  const auto hasActiveRef = [&](const int theCount, const BRepGraph_RefId::Kind theKind) {
    for (int anIndex = 0; anIndex < theCount; ++anIndex)
    {
      const BRepGraph_RefId aRef(theKind, anIndex);
      if (refChildNode(theGraph, aRef) != theChild)
      {
        continue;
      }

      if (!aRefs.IsRemoved(aRef))
      {
        return true;
      }
    }
  ...
  if (hasActiveRef(aRefs.Shells().Nb(), BRepGraph_RefId::Kind::Shell)
      || hasActiveRef(aRefs.Faces().Nb(), BRepGraph_RefId::Kind::Face)
      || hasActiveRef(aRefs.Wires().Nb(), BRepGraph_RefId::Kind::Wire)
      || hasActiveRef(aRefs.CoEdges().Nb(), BRepGraph_RefId::Kind::CoEdge)
      || hasActiveRef(aRefs.Vertices().Nb(), BRepGraph_RefId::Kind::Vertex)
      || hasActiveRef(aRefs.Solids().Nb(), BRepGraph_RefId::Kind::Solid)
      || hasActiveRef(aRefs.Children().Nb(), BRepGraph_RefId::Kind::Child)
      || hasActiveRef(aRefs.Occurrences().Nb(), BRepGraph_RefId::Kind::Occurrence))
  {
    return true;
  }
  ...
  {
    const BRepGraph_ProductId aProduct = BRepGraph_ProductId::FromNodeId(theChild);
    for (int anOccIdx = 0; anOccIdx < aTopo.Occurrences().Nb(); ++anOccIdx)
    {
      const BRepGraphInc::OccurrenceDef& anOcc = aTopo.Occurrences().Definition(BRepGraph_OccurrenceId(anOccIdx));
      if (!anOcc.IsRemoved && anOcc.ProductDefId == aProduct)
      {
        return true;
      }
    }
  }
  else if (BRepGraph_NodeId::IsTopologyKind(theChild.NodeKind))
  {
    for (int aProdIdx = 0; aProdIdx < aTopo.Products().Nb(); ++aProdIdx)
    {
      const BRepGraphInc::ProductDef& aProduct = aTopo.Products().Definition(BRepGraph_ProductId(aProdIdx));
      if (!aProduct.IsRemoved && aProduct.ShapeRootId == theChild)
      {
        return true;
      }
    }
                                               ...
                                               const BRepGraph_NodeId      theNode)
{
  ...
  {
    case BRepGraph_NodeId::Kind::Vertex:
      return &theStorage.Vertex(BRepGraph_VertexId(theNode.Index));
    case BRepGraph_NodeId::Kind::Edge:
      return &theStorage.Edge(BRepGraph_EdgeId(theNode.Index));
    case BRepGraph_NodeId::Kind::CoEdge:
      return &theStorage.CoEdge(BRepGraph_CoEdgeId(theNode.Index));
    case BRepGraph_NodeId::Kind::Wire:
      return &theStorage.Wire(BRepGraph_WireId(theNode.Index));
    case BRepGraph_NodeId::Kind::Face:
      return &theStorage.Face(BRepGraph_FaceId(theNode.Index));
    case BRepGraph_NodeId::Kind::Shell:
      return &theStorage.Shell(BRepGraph_ShellId(theNode.Index));
    case BRepGraph_NodeId::Kind::Solid:
      return &theStorage.Solid(BRepGraph_SolidId(theNode.Index));
    case BRepGraph_NodeId::Kind::Compound:
      return &theStorage.Compound(BRepGraph_CompoundId(theNode.Index));
    case BRepGraph_NodeId::Kind::CompSolid:
      return &theStorage.CompSolid(BRepGraph_CompSolidId(theNode.Index));
    case BRepGraph_NodeId::Kind::Product:
      return &theStorage.Product(BRepGraph_ProductId(theNode.Index));
    case BRepGraph_NodeId::Kind::Occurrence:
      return &theStorage.Occurrence(BRepGraph_OccurrenceId(theNode.Index));
  }
...
BRepGraph_VertexId BRepGraph::BuilderView::AddVertex(const gp_Pnt& thePoint,
                                                     const double  theTolerance)
{
...
BRepGraph_EdgeId BRepGraph::BuilderView::AddEdge(const BRepGraph_VertexId       theStartVtx,
                                                 ...
                                                 const double                   theTolerance)
{
  ...
  if (theStartVtx.IsValid())
  {
    ...
    const int                aStartVtxRefIdx = aStorage.NbVertexRefs() - 1;
    aStartVtxRef.RefId                       = BRepGraph_VertexRefId(aStartVtxRefIdx);
  ...
  {
    BRepGraphInc::VertexRef& anEndVtxRef    = aStorage.AppendVertexRef();
    const int                anEndVtxRefIdx = aStorage.NbVertexRefs() - 1;
    anEndVtxRef.RefId                       = BRepGraph_VertexRefId(anEndVtxRefIdx);
  ...
  if (!theCurve.IsNull())
  {
    BRepGraphInc::Curve3DRep& aCurveRep    = aStorage.AppendCurve3DRep();
    const int                 aCurveRepIdx = aStorage.NbCurves3D() - 1;
...
BRepGraph_WireId BRepGraph::BuilderView::AddWire(
  const NCollection_Vector<std::pair<BRepGraph_EdgeId, TopAbs_Orientation>>& theEdges)
{
  BRepGraphInc_Storage& aStorage = myGraph->myData->myIncStorage;
  ...
  for (const std::pair<BRepGraph_EdgeId, TopAbs_Orientation>& anEdgeEntry : theEdges)
  {
    ...
    BRepGraphInc::CoEdgeRef& aCoEdgeRef    = aStorage.AppendCoEdgeRef();
    const int                aCoEdgeRefIdx = aStorage.NbCoEdgeRefs() - 1;
    aCoEdgeRef.RefId                       = BRepGraph_CoEdgeRefId(aCoEdgeRefIdx);
...
BRepGraph_FaceId BRepGraph::BuilderView::AddFace(
  ...
  const double                                theTolerance)
{
  ...
  {
    BRepGraphInc::SurfaceRep& aSurfRep    = aStorage.AppendSurfaceRep();
    const int                 aSurfRepIdx = aStorage.NbSurfaces() - 1;
  ...
  {
    BRepGraphInc::WireRef& anOuterWireRef    = aStorage.AppendWireRef();
    const int              anOuterWireRefIdx = aStorage.NbWireRefs() - 1;
    anOuterWireRef.RefId                     = BRepGraph_WireRefId(anOuterWireRefIdx);
    anOuterWireRef.ParentId                  = aFaceId;
    anOuterWireRef.WireDefId                 = theOuterWire;
    anOuterWireRef.IsOuter                   = true;
    myGraph->allocateRefUID(anOuterWireRef.RefId);
    aStorage.ChangeFace(aFaceId).WireRefIds.Append(BRepGraph_WireRefId(anOuterWireRefIdx));
  }
  ...
  for (const BRepGraph_WireId& aWireDefId : theInnerWires)
  {
    ...
    BRepGraphInc::WireRef& aWireRef    = aStorage.AppendWireRef();
    const int              aWireRefIdx = aStorage.NbWireRefs() - 1;
    aWireRef.RefId                     = BRepGraph_WireRefId(aWireRefIdx);
    aWireRef.ParentId                  = aFaceId;
    aWireRef.WireDefId                 = aWireDefId;
    aWireRef.IsOuter                   = false;
    myGraph->allocateRefUID(aWireRef.RefId);
    aStorage.ChangeFace(aFaceId).WireRefIds.Append(BRepGraph_WireRefId(aWireRefIdx));
  }
...
BRepGraph_ShellId BRepGraph::BuilderView::AddShell()
{
...
BRepGraph_SolidId BRepGraph::BuilderView::AddSolid()
{
...
BRepGraph_CompoundId BRepGraph::BuilderView::AddCompound(
  const NCollection_Vector<BRepGraph_NodeId>& theChildEntities)
{
  ...
  for (const BRepGraph_NodeId& aChild : theChildEntities)
  {
    BRepGraphInc::ChildRef& aCREntry = aStorage.AppendChildRef();
    const int               aCRIdx   = aStorage.NbChildRefs() - 1;
    aCREntry.RefId                   = BRepGraph_ChildRefId(aCRIdx);
...
BRepGraph_CompSolidId BRepGraph::BuilderView::AddCompSolid(
  const NCollection_Vector<BRepGraph_SolidId>& theSolidEntities)
{
  BRepGraphInc_Storage& aStorage = myGraph->myData->myIncStorage;
  ...
  for (const BRepGraph_SolidId& aSolidId : theSolidEntities)
  {
    BRepGraphInc::SolidRef& aSREntry = aStorage.AppendSolidRef();
...
BRepGraph_ProductId BRepGraph::BuilderView::AddProduct(const BRepGraph_NodeId theShapeRoot)
{
  BRepGraphInc_Storage& aStorage = myGraph->myData->myIncStorage;
  ...
  BRepGraphInc::ProductDef& aProductDef = aStorage.AppendProduct();
  const int                 aIdx        = aStorage.NbProducts() - 1;
  aProductDef.Id                        = BRepGraph_ProductId(aIdx);
  aProductDef.ShapeRootId               = theShapeRoot;
  myGraph->allocateUID(aProductDef.Id);

  return BRepGraph_ProductId(aIdx);
}

//=================================================================================================

BRepGraph_ProductId BRepGraph::BuilderView::AddAssemblyProduct()
{
  ...
  const BRepGraph_OccurrenceId theParentOccurrence)
{
  ...
  if (theParentOccurrence.IsValid()
      && aStorage.Occurrence(theParentOccurrence).ProductDefId != theParentProduct)
  {
    return BRepGraph_OccurrenceId();
  }
  ...
  anOccRef.RefId                           = BRepGraph_OccurrenceRefId(anOccRefIdx);
  ...
  return BRepGraph_OccurrenceId(anOccIdx);
}
...
{
  BRepGraph_Builder::AppendFlattened(*myGraph, theShape, theParallel, theOptions);
}

//=================================================================================================

void BRepGraph::BuilderView::AppendFullShape(const TopoDS_Shape&                   theShape,
                                             const bool                            theParallel,
                                             const BRepGraphInc_Populate::Options& theOptions)
{
  BRepGraph_Builder::AppendFull(*myGraph, theShape, theParallel, theOptions);
}
...
bool BRepGraph::BuilderView::RemoveRef(const BRepGraph_RefId theRef)
{
  if (!theRef.IsValid())
  {
    return false;
...
bool BRepGraph::BuilderView::RemoveRef(const BRepGraph_NodeId theParent,
                                       ...
                                       const bool             theToPruneOrphanedChild)
{
  ...
  Standard_ASSERT_RETURN(aRef.ParentId == theParent,
                         "RemoveRef: reference parent mismatch",
                         false);
...
void BRepGraph::BuilderView::SetCoEdgePCurve(const BRepGraph_CoEdgeId           theCoEdge,
                                             const occ::handle<Geom2d_Curve>& theCurve2d)
{
  BRepGraph_MutGuard<BRepGraphInc::CoEdgeDef> aCoEdge = MutCoEdge(theCoEdge);
  aCoEdge->Curve2DRepId = theCurve2d.IsNull() ? BRepGraph_Curve2DRepId()
                                               : CreateCurve2DRep(theCurve2d);
}

//=================================================================================================

BRepGraph_TriangulationRepId BRepGraph::BuilderView::CreateTriangulationRep(
  const occ::handle<Poly_Triangulation>& theTriangulation)
{
  if (theTriangulation.IsNull())
    return BRepGraph_TriangulationRepId();

  BRepGraphInc::TriangulationRep& aRep  = myGraph->myData->myIncStorage.AppendTriangulationRep();
  const int                       anIdx = myGraph->myData->myIncStorage.NbTriangulations() - 1;
  aRep.Id                               = BRepGraph_RepId::Triangulation(anIdx);
  aRep.Triangulation                    = theTriangulation;
  return BRepGraph_TriangulationRepId(anIdx);
}

//=================================================================================================

BRepGraph_Polygon3DRepId BRepGraph::BuilderView::CreatePolygon3DRep(
  const occ::handle<Poly_Polygon3D>& thePolygon)
{
  if (thePolygon.IsNull())
    return BRepGraph_Polygon3DRepId();

  BRepGraphInc::Polygon3DRep& aRep  = myGraph->myData->myIncStorage.AppendPolygon3DRep();
  const int                   anIdx = myGraph->myData->myIncStorage.NbPolygons3D() - 1;
  aRep.Id                           = BRepGraph_RepId::Polygon3D(anIdx);
  aRep.Polygon                      = thePolygon;
  return BRepGraph_Polygon3DRepId(anIdx);
}

//=================================================================================================

BRepGraph_PolygonOnTriRepId BRepGraph::BuilderView::CreatePolygonOnTriRep(
  ...
  const BRepGraph_TriangulationRepId              theTriRepId)
{
  ...
  BRepGraphInc::PolygonOnTriRep& aRep  = myGraph->myData->myIncStorage.AppendPolygonOnTriRep();
  const int                      anIdx = myGraph->myData->myIncStorage.NbPolygonsOnTri() - 1;
  aRep.Id                              = BRepGraph_RepId::PolygonOnTri(anIdx);
  aRep.Polygon                         = thePolygon;
  aRep.TriangulationRepId              = theTriRepId;
  return BRepGraph_PolygonOnTriRepId(anIdx);
}

//=================================================================================================

void BRepGraph::BuilderView::ClearFaceMesh(const BRepGraph_FaceId theFace)
{
  BRepGraphInc_Storage& aStorage = myGraph->myData->myIncStorage;
  if (!isActiveNode(aStorage, theFace))
    return;

  BRepGraphInc::FaceDef& aFace = aStorage.ChangeFace(theFace);

  // Clear coedge polygon-on-tri data for all wires of this face.
  for (const BRepGraph_WireRefId& aWireRefId : aFace.WireRefIds)
  {
    ...
    for (const BRepGraph_CoEdgeRefId& aCoEdgeRefId : aWire.CoEdgeRefIds)
    {
      if (!aCoEdgeRefId.IsValid())
        continue;
      const BRepGraphInc::CoEdgeRef& aCoEdgeRef = aStorage.CoEdgeRef(aCoEdgeRefId);
      if (aCoEdgeRef.IsRemoved || !aCoEdgeRef.CoEdgeDefId.IsValid())
        continue;
      BRepGraphInc::CoEdgeDef& aCoEdge = aStorage.ChangeCoEdge(aCoEdgeRef.CoEdgeDefId);
      if (aCoEdge.IsRemoved)
        continue;
      for (const BRepGraph_PolygonOnTriRepId& aPolyRepId : aCoEdge.PolygonOnTriRepIds)
      {
        if (aPolyRepId.IsValid())
          RemoveRep(aPolyRepId);
      }
      aCoEdge.PolygonOnTriRepIds.Clear();
      myGraph->markModified(aCoEdge.Id);
    }
  }

  // Clear face triangulation data.
...
void BRepGraph::BuilderView::ClearEdgePolygon3D(const BRepGraph_EdgeId theEdge)
{
  BRepGraphInc_Storage& aStorage = myGraph->myData->myIncStorage;
  ...
  {
    RemoveRep(anEdge.Polygon3DRepId);
...
void BRepGraph::BuilderView::EndDeferredInvalidation() noexcept
{
  ...
  NCollection_Vector<BRepGraph_RefId>& aDeferredRefList =
...
{
  BRepGraphInc_Storage& aStorage = myGraph->myData->myIncStorage;
  validateMutableNodeId(aStorage, BRepGraph_EdgeId(theEdge.Index));
...
{
  BRepGraphInc_Storage& aStorage = myGraph->myData->myIncStorage;
  validateMutableNodeId(aStorage, BRepGraph_CoEdgeId(theCoEdge.Index));
...
{
  BRepGraphInc_Storage& aStorage = myGraph->myData->myIncStorage;
  validateMutableNodeId(aStorage, BRepGraph_VertexId(theVertex.Index));
...
{
  BRepGraphInc_Storage& aStorage = myGraph->myData->myIncStorage;
  validateMutableNodeId(aStorage, BRepGraph_WireId(theWire.Index));
...
{
  BRepGraphInc_Storage& aStorage = myGraph->myData->myIncStorage;
  validateMutableNodeId(aStorage, BRepGraph_FaceId(theFace.Index));
...
{
  BRepGraphInc_Storage& aStorage = myGraph->myData->myIncStorage;
  validateMutableNodeId(aStorage, BRepGraph_ShellId(theShell.Index));
...
{
  BRepGraphInc_Storage& aStorage = myGraph->myData->myIncStorage;
  validateMutableNodeId(aStorage, BRepGraph_SolidId(theSolid.Index));
...
{
  BRepGraphInc_Storage& aStorage = myGraph->myData->myIncStorage;
  validateMutableNodeId(aStorage, BRepGraph_CompoundId(theCompound.Index));
...
{
  BRepGraphInc_Storage& aStorage = myGraph->myData->myIncStorage;
  validateMutableNodeId(aStorage, BRepGraph_CompSolidId(theCompSolid.Index));
...
{
  BRepGraphInc_Storage& aStorage = myGraph->myData->myIncStorage;
  validateMutableNodeId(aStorage, BRepGraph_ProductId(theProduct.Index));
...
{
  BRepGraphInc_Storage& aStorage = myGraph->myData->myIncStorage;
  validateMutableNodeId(aStorage, BRepGraph_OccurrenceId(theOccurrence.Index));
...
{
  BRepGraphInc_Storage& aStorage = myGraph->myData->myIncStorage;
  validateMutableRefId(aStorage, BRepGraph_OccurrenceRefId(theOccurrenceRef.Index));
  return BRepGraph_MutGuard<BRepGraphInc::OccurrenceRef>(
    myGraph,
    &aStorage.ChangeOccurrenceRef(theOccurrenceRef),
    BRepGraph_OccurrenceRefId(theOccurrenceRef.Index));
}
...
{
  BRepGraphInc_Storage& aStorage = myGraph->myData->myIncStorage;
  validateMutableRepId(aStorage, BRepGraph_RepId::Triangulation(theTriangulation.Index));
...
{
  BRepGraphInc_Storage& aStorage = myGraph->myData->myIncStorage;
  validateMutableRepId(aStorage, BRepGraph_RepId::Polygon3D(thePolygon.Index));
...
{
  BRepGraphInc_Storage& aStorage = myGraph->myData->myIncStorage;
  validateMutableRepId(aStorage, BRepGraph_RepId::Polygon2D(thePolygon.Index));
...
{
  BRepGraphInc_Storage& aStorage = myGraph->myData->myIncStorage;
  validateMutableRepId(aStorage, BRepGraph_RepId::PolygonOnTri(thePolygon.Index));
                                       ...
                                       BRepGraph_EdgeId&        theSubB)
{
  ...
  if (aOrigStartVertexRefId.IsValid())
  {
    ...
    aSubAStartRef.RefId                       = BRepGraph_VertexRefId(aSubAStartRefIdx);
  ...
  if (aSplitVertexDefId.IsValid())
  {
    ...
    aSubAEndRef.RefId                       = BRepGraph_VertexRefId(aSubAEndRefIdx);
  ...
  if (aSplitVertexDefId.IsValid())
  {
    ...
    aSubBStartRef.RefId                       = BRepGraph_VertexRefId(aSubBStartRefIdx);
  ...
  if (aOrigEndVertexRefId.IsValid())
  {
    ...
    aSubBEndRef.RefId                       = BRepGraph_VertexRefId(aSubBEndRefIdx);
  ...
  // Update incidence: wire CoEdgeRef rows and wire CoEdgeRefIds.
  {
    ...
    if (aWireIndices != nullptr)
    {
      ...
      {
        const int aWireIdx = aWireId.Index;
             ...
             aRefIt.Next())
        {
          ...
          {
            const TopAbs_Orientation aOrigOri = aOldCoEdge.Orientation;
            ...
            aSubBCoEdge.Id                          = BRepGraph_CoEdgeId(aSubBCoEdgeIdx);
            aSubBCoEdge.EdgeDefId                   = BRepGraph_EdgeId(aSubBIdx);
            aSubBCoEdge.Orientation                       = aOrigOri;
            ...
            aSubBRef.RefId                       = BRepGraph_CoEdgeRefId(aSubBRefIdx);
            aSubBRef.ParentId                    = BRepGraph_WireId(aWireIdx);
  ...
  NCollection_Vector<BRepGraphInc::CoEdgeDef> aOrigCoEdges;
  {
    ...
    if (aCoEdgeIdxs != nullptr)
    {
      for (const BRepGraph_CoEdgeId& aCoEdgeId : *aCoEdgeIdxs)
        aOrigCoEdges.Append(myGraph->myData->myIncStorage.CoEdge(aCoEdgeId));
    }
  }

  const double aParamRange = aOrigParamLast - aOrigParamFirst;
  for (const BRepGraphInc::CoEdgeDef& aCE : aOrigCoEdges)
  {
    ...
    aCoEdgeSubA.Id                          = BRepGraph_CoEdgeId(aCoEdgeSubAIdx);
    ...
    aCoEdgeSubB.Id                          = BRepGraph_CoEdgeId(aCoEdgeSubBIdx);
                                               ...
                                               const bool             theReversed)
{
  ...
  myGraph->markModified(BRepGraph_WireId(theWireDefId.Index));
