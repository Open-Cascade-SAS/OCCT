...
//! OwnGen (own-data version counter) and graph Generation (BRepGraph_Builder::Perform() cycle).
...
struct BRepGraph_VersionStamp
{
  ...
  uint32_t myGeneration; //!< Graph BRepGraph_Builder::Perform() generation at snapshot time.
  ...
  //! @param[in] theGeneration  graph BRepGraph_Builder::Perform() generation
