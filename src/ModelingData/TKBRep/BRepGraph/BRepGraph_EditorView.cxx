...
void removeFromRootProducts(NCollection_Vector<BRepGraph_ProductId>& theRoots,
                            const BRepGraph_ProductId               theProduct)
{
  NCollection_Vector<BRepGraph_ProductId> aFiltered;
...
int findOrderedRefIndex(const NCollection_Vector<RefIdT>& theRefIds, const RefIdT theRefId)
{
  for (int anIndex = 0; anIndex < theRefIds.Length(); ++anIndex)
  {
    if (theRefIds.Value(anIndex) == theRefId)
    {
      return anIndex;
    }
  }
...
template <typename RefIdT>
void eraseOrderedRef(const int theRefIndex, NCollection_Vector<RefIdT>& theRefIds)
{
  for (int anIndex = theRefIndex; anIndex < theRefIds.Length() - 1; ++anIndex)
  {
    theRefIds.ChangeValue(anIndex) = theRefIds.Value(anIndex + 1);
  }
  theRefIds.EraseLast();
}
...
bool detachOrderedParentRef(BRepGraph&                   theGraph,
                            ...
                            NCollection_Vector<RefIdT>& theParentRefIds,
                            const BRepGraph_NodeId      theChildNode,
                            const bool                  theToPruneOrphanedChild)
{
  const int aRefIndex = findOrderedRefIndex(theParentRefIds, theRefId);
  if (aRefIndex < 0)
  {
    return false;
  }
  ...
  eraseOrderedRef(aRefIndex, theParentRefIds);

  if (theToPruneOrphanedChild && theChildNode.IsValid() && !hasAnyActiveUsage(theGraph, theChildNode))
  {
...
BRepGraph_VertexRefId BRepGraph::EditorView::FaceOps::AddVertex(const BRepGraph_FaceId   theFaceEntity,
                                                              const BRepGraph_VertexId theVertexEntity,
                                                              const TopAbs_Orientation theOri)
{
  BRepGraphInc_Storage& aStorage = myGraph->myData->myIncStorage;
...
BRepGraph_ChildRefId BRepGraph::EditorView::ShellOps::AddChild(const BRepGraph_ShellId  theShellEntity,
...
BRepGraph_ShellRefId BRepGraph::EditorView::SolidOps::AddShell(const BRepGraph_SolidId theSolidEntity,
...
BRepGraph_ChildRefId BRepGraph::EditorView::SolidOps::AddChild(const BRepGraph_SolidId  theSolidEntity,
                                                             const BRepGraph_NodeId   theChildEntity,
                                                             const TopAbs_Orientation theOri)
{
  BRepGraphInc_Storage& aStorage = myGraph->myData->myIncStorage;
...
BRepGraph_ProductId BRepGraph::EditorView::ProductOps::AddAssembly()
{
  BRepGraphInc_Storage& aStorage = myGraph->myData->myIncStorage;
  const BRepGraph_ProductId aProductId(aStorage.NbProducts());
  aStorage.AppendProduct();
...
BRepGraph_OccurrenceId BRepGraph::EditorView::ProductOps::AddOccurrence(
  ...
  const BRepGraph_OccurrenceId theParentOccurrence)
{
  ...
  Standard_ASSERT_RETURN(isValidOccurrenceChildKind(anOccDef.ChildDefId.NodeKind),
                         "ProductOps::AddOccurrence: invalid occurrence child kind",
                         BRepGraph_OccurrenceId());
  myGraph->allocateUID(anOccId);
  ...
  aStorage.ChangeProduct(theParentProduct)
    .OccurrenceRefIds.Append(anOccRefId);
...
void BRepGraph::EditorView::AppendFlattenedShape(const TopoDS_Shape&                   theShape,
...
void BRepGraph::EditorView::AppendFullShape(const TopoDS_Shape&                   theShape,
                                             const bool                            theParallel,
                                             const BRepGraphInc_Populate::Options& theOptions)
{
  ...
  switch (theNode.NodeKind)
  {
    ...
    case BRepGraph_NodeId::Kind::Occurrence: {
      if (theNode.IsValid(aStorage.NbOccurrences()))
      {
        ...
        if (anOcc.ChildDefId.IsValid() && BRepGraph_NodeId::IsTopologyKind(anOcc.ChildDefId.NodeKind))
        {
        ...
        for (int aRefIdx = 0; aRefIdx < aStorage.NbOccurrenceRefs(); ++aRefIdx)
        {
          const BRepGraphInc::OccurrenceRef& aRef =
            aStorage.OccurrenceRef(BRepGraph_OccurrenceRefId(aRefIdx));
          ...
          {
            if (aRefIds.Value(i).Index == aRefIdx)
            {
              myGraph->myData->myIncStorage.MarkRemovedRef(BRepGraph_OccurrenceRefId(aRefIdx));
        ...
        {
          const BRepGraph_ProductId aChildProduct = BRepGraph_ProductId::FromNodeId(anOcc.ChildDefId);
  ...
  for (int i = 0; i < aAllModified.Length(); ++i)
  {
    ...
    switch (aNodeId.NodeKind)
    {
      ...
      case BRepGraph_NodeId::Kind::Occurrence: {
        ...
        for (int aRefIdx = 0; aRefIdx < myGraph->myData->myIncStorage.NbOccurrenceRefs(); ++aRefIdx)
        {
          const BRepGraphInc::OccurrenceRef& aRef =
            myGraph->myData->myIncStorage.OccurrenceRef(BRepGraph_OccurrenceRefId(aRefIdx));
                                       ...
                                       BRepGraph_EdgeId&        theSubB)
{
  ...
  const int aSubAIdx = aSubAId.Index;
  theSubA            = aSubAId;
  ...
  aMutStorage.AppendEdge();
  const int aSubBIdx = aSubBId.Index;
  theSubB            = aSubBId;
  ...
  NCollection_Vector<BRepGraph_FaceId> aOrigFaces;
  {
    ...
    for (int i = 0; i < aNbOrig; ++i)
    {
      ...
      aStorage.ChangeCoEdge(aNewACoEdgeIds.Value(i)).SeamPairId =
        aNewACoEdgeIds.Value(aJ);
      aStorage.ChangeCoEdge(aNewBCoEdgeIds.Value(i)).SeamPairId =
    ...
    {
      NCollection_Vector<BRepGraph_CoEdgeRefId> aSnapshot;
      {
        const BRepGraphInc::WireDef& aWireDef = aStorage.Wire(aWireId);
        for (NCollection_Vector<BRepGraph_CoEdgeRefId>::Iterator aRefIt(aWireDef.CoEdgeRefIds);
             aRefIt.More();
             aRefIt.Next())
        {
      ...
      for (int i = 0; i < aSnapshot.Length(); ++i)
      {
        ...
        const int              aPos     = i + anInsertOffset;
        BRepGraphInc::WireDef& aWireEnt = aStorage.ChangeWire(aWireId);
        if (aPos >= 0 && aPos < aWireEnt.CoEdgeRefIds.Length())
          aWireEnt.CoEdgeRefIds.InsertAfter(aPos, aNewRefId);
        else
          aWireEnt.CoEdgeRefIds.Append(aNewRefId);
        ++anInsertOffset;
    ...
    {
      const BRepGraph_CoEdgeId aOldId = anOrigCoEdgeIds.Value(i);
      const BRepGraph_CoEdgeId aNewA  = aNewACoEdgeIds.Value(i);
      const BRepGraph_CoEdgeId aNewB  = aNewBCoEdgeIds.Value(i);
    ...
    // moment a caller does populate them.
    {
      ...
      for (NCollection_Vector<BRepGraph_VertexRefId>::Iterator anIntRefIt(
  ...
  // Incremental vertex-to-edge updates: register sub-edge vertices.
  {
    ...
    for (const BRepGraph_FaceId& aFaceId : aOrigFaces)
    {
      ...
      aRevIdxMut.BindEdgeToFace(BRepGraph_EdgeId(aSubAIdx), aFaceId);
      aRevIdxMut.BindEdgeToFace(BRepGraph_EdgeId(aSubBIdx), aFaceId);
    }
  }
                                               ...
                                               const bool             theReversed)
{
  ...
  for (BRepGraph_RefsCoEdgeOfWire aRefIt(*myGraph, theWireDefId); aRefIt.More(); aRefIt.Next())
  {
    ...
    if (aCoEdge.EdgeDefId == theOldEdgeEntity)
    {
      ...
      // Wire-to-face mappings are built from FaceDef.WireRefs during BRepGraph_Builder::Perform() and are
                                                     ...
                                                     const BRepGraph_ChildRefId theChildRefId)
{
  ...
  NCollection_Vector<BRepGraph_ChildRefId>& aCompoundRefIds =
  ...
  const BRepGraph_OccurrenceRefId theOccurrenceRefId)
{
  Standard_ASSERT_RETURN(theProductDefId.IsValid(myGraph->myData->myIncStorage.NbProducts()),
                         ...
                         false);
  Standard_ASSERT_RETURN(theOccurrenceRefId.IsValid(
                           myGraph->myData->myIncStorage.NbOccurrenceRefs()),
  ...
  NCollection_Vector<BRepGraph_OccurrenceRefId>& aProductRefIds =
    aStorage.ChangeProduct(theProductDefId).OccurrenceRefIds;
  if (!detachOrderedParentRef(*myGraph,
                              theOccurrenceRefId,
                              aProductRefIds,
                              BRepGraph_NodeId(aRef.OccurrenceDefId),
                              true))
  {
    return false;
  }
  ...
  const BRepGraphInc::OccurrenceDef& anOccDef =
    aStorage.Occurrence(aRef.OccurrenceDefId);
...
bool BRepGraph::EditorView::ProductOps::RemoveShapeRoot(const BRepGraph_ProductId theProductDefId)
{
  ...
  if (!aShapeRootRefId.IsValid() || !aShapeRoot.IsValid()
      || !isActiveNode(aStorage, aShapeRoot))
  {
    return false;
  }
  ...
  detachOrderedParentRef(*myGraph,
                         ...
                         BRepGraph_NodeId(aStorage.OccurrenceRef(aShapeRootRefId).OccurrenceDefId),
                         true);
                                                  ...
                                                  const BRepGraph_ChildRefId theChildRefId)
{
  ...
  NCollection_Vector<BRepGraph_ChildRefId>& aShellRefIds =
                                                  ...
                                                  const BRepGraph_ChildRefId theChildRefId)
{
  ...
  NCollection_Vector<BRepGraph_ChildRefId>& aSolidRefIds =
...
bool BRepGraph::EditorView::CompSolidOps::RemoveSolid(
  const BRepGraph_CompSolidId theCompSolidDefId,
  const BRepGraph_SolidRefId  theSolidRefId)
{
  ...
  NCollection_Vector<BRepGraph_SolidRefId>& aCompSolidRefIds =
    aStorage.ChangeCompSolid(theCompSolidDefId).SolidRefIds;
  if (!detachOrderedParentRef(*myGraph,
                              theSolidRefId,
                              aCompSolidRefIds,
                              BRepGraph_NodeId(aRef.SolidDefId),
                              true))
  {
    return false;
  }
                                                  ...
                                                  const BRepGraph_ShellRefId theShellRefId)
{
  ...
  NCollection_Vector<BRepGraph_ShellRefId>& aSolidRefIds =
    aStorage.ChangeSolid(theSolidDefId).ShellRefIds;
  if (!detachOrderedParentRef(*myGraph,
                              theShellRefId,
                              aSolidRefIds,
                              BRepGraph_NodeId(aRef.ShellDefId),
                              true))
  {
    return false;
  }
                                                 ...
                                                 const BRepGraph_FaceRefId theFaceRefId)
{
  ...
  NCollection_Vector<BRepGraph_FaceRefId>& aShellRefIds =
    aStorage.ChangeShell(theShellDefId).FaceRefIds;
  if (!detachOrderedParentRef(*myGraph,
                              theFaceRefId,
                              aShellRefIds,
                              BRepGraph_NodeId(aRef.FaceDefId),
                              true))
  {
    return false;
  }
                                                  ...
                                                  const BRepGraph_VertexRefId theVertexRefId)
{
  ...
  NCollection_Vector<BRepGraph_VertexRefId>& aFaceRefIds =
                                                ...
                                                const BRepGraph_WireRefId theWireRefId)
{
  ...
  NCollection_Vector<BRepGraph_WireRefId>& aFaceRefIds = aStorage.ChangeFace(theFaceDefId).WireRefIds;
  if (!detachOrderedParentRef(*myGraph,
                              theWireRefId,
                              aFaceRefIds,
                              BRepGraph_NodeId(aRef.WireDefId),
                              true))
  {
    return false;
  }
                                                  ...
                                                  const BRepGraph_CoEdgeRefId theCoEdgeRefId)
{
  ...
  if (aRef.IsRemoved || aRef.ParentId != BRepGraph_NodeId(theWireDefId) || !aRef.CoEdgeDefId.IsValid()
      || !isActiveNode(aStorage, aRef.CoEdgeDefId))
  {
    return false;
  }
  ...
  NCollection_Vector<BRepGraph_CoEdgeRefId>& aWireRefIds =
                                                  ...
                                                  const BRepGraph_VertexRefId theVertexRefId)
{
  ...
  {
    NCollection_Vector<BRepGraph_VertexRefId>& anEdgeRefIds = aEdge.InternalVertexRefIds;
  ...
  if (isFixed && !hasAnyActiveUsage(*myGraph, BRepGraph_NodeId(aRef.VertexDefId)))
  {
    myGraph->Editor().Gen().RemoveSubgraph(BRepGraph_NodeId(aRef.VertexDefId));
  }
  ...
  const BRepGraph_VertexId    theNewVertexDefId)
{
  ...
  Standard_ASSERT_RETURN(theOldVertexRefId.IsValid(
                           myGraph->myData->myIncStorage.NbVertexRefs()),
  ...
  {
    bool isFound = false;
    for (int anIdx = 0; anIdx < anEdge.InternalVertexRefIds.Length(); ++anIdx)
    {
      if (anEdge.InternalVertexRefIds.Value(anIdx) == theOldVertexRefId)
      {
        anEdge.InternalVertexRefIds.ChangeValue(anIdx) = aNewRefId;
        isFound                                        = true;
        break;
      }
    }
...
{
  NCollection_Vector<BoundaryIssue> anIssues;
  ...
  NCollection_Vector<BoundaryIssue>* const theIssues) const
{
  ...
  for (BRepGraph_OccurrenceIterator anOccIt(*myGraph); anOccIt.More(); anOccIt.Next())
  {
    ...
    if (theIssues != nullptr)
    {
      ...
      aDesc += TCollection_AsciiString(anOccId.Index);
      aDesc += ": child kind=";
