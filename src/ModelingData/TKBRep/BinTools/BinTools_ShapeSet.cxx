...
void BinTools_ShapeSet::Write(Standard_OStream& OS, const Message_ProgressRange& theRange)
{
  ...
  for (i = 1; i <= nbShapes && aPSinner.More(); i++, aPSinner.Next())
  {
    ...
    // Type
    OS << static_cast<uint8_t>(S.ShapeType());
...
void BinTools_ShapeSet::Read(Standard_IStream& IS, const Message_ProgressRange& theRange)
{
  ...
  {
    TopoDS_Shape     S;
    TopAbs_ShapeEnum T = static_cast<TopAbs_ShapeEnum>(IS.get());
    ReadShape(T, IS, S);
...
void BinTools_ShapeSet::Write(const TopoDS_Shape& S, Standard_OStream& OS)
{
  ...
  else
  {
    ...
    OS << static_cast<uint8_t>(S.Orientation());
...
void BinTools_ShapeSet::ReadSubs(TopoDS_Shape& S, Standard_IStream& IS, const int nbshapes)
{
  ...
  else
  {
    ...
    anOrient = static_cast<TopAbs_Orientation>(aChar);
    int anIndx;
      ...
      while (itrc.More())
      {
        ...
        if (CR->IsCurve3D())
        {
          if (!CR->Curve3D().IsNull())
          {
            ...
            GC->Range(first, last);
            OS << static_cast<uint8_t>(1); // CURVE_3D;
        ...
        else if (IsWithTriangles())
        {
          ...
          else if (CR->IsPolygonOnTriangulation())
          {
            ...
            if (!CR->IsPolygonOnClosedTriangulation())
              ...
              OS << static_cast<uint8_t>(6);
            else
              // -7- Polygon on closed triangulation
              OS << static_cast<uint8_t>(7);
    ...
    else if (S.ShapeType() == TopAbs_FACE)
    {
      ...
      if (IsWithTriangles() || TF->Surface().IsNull())
      {
        if (!(TF->Triangulation()).IsNull())
        {
          ...
          OS << static_cast<uint8_t>(1);
      }
      else
        OS << static_cast<uint8_t>(0); // without triangulation
    }
  }
    ...
    switch (T)
    {
      ...
      case TopAbs_VERTEX: {
        ...
        do
        {
          ...
          {
            val = static_cast<int>(IS.get()); // case {0|1|2|3}
            if (val > 0 && val <= 3)
      ...
      case TopAbs_EDGE:
        ...
        // Create an edge
        {
          ...
          do
          {
            val = static_cast<int>(IS.get()); //{0|1|2|3|4|5|6|7}
            // -0- no representation
            // -1- Curve 3D
            // -2- Curve on surf
            // -3- Curve on closed surf
            ...
            switch (val)
            {
              ...
              case 3: // -3- Curve on closed surf
                ...
                if (closed)
                {
                  ...
                  reg = static_cast<GeomAbs_Shape>(IS.get());
                }
      ...
      case TopAbs_FACE: {
        ...
        uint8_t aByte = static_cast<uint8_t>(IS.get());
        // triangulation case
        if (aByte == 2)
        {
          BinTools::GetInteger(IS, s);
