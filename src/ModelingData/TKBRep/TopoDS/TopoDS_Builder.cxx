...
void TopoDS_Builder::Add(TopoDS_Shape& aShape, const TopoDS_Shape& aComponent) const
{
  ...
  if (aShape.Free())
  {
    static const unsigned int aTb[9] = {
      ...
      (1 << (static_cast<unsigned int>(TopAbs_COMPOUND))),
      // SOLID to:
      (1 << (static_cast<unsigned int>(TopAbs_COMPOUND))) | (1 << (static_cast<unsigned int>(TopAbs_COMPSOLID))),
      // SHELL to:
      (1 << (static_cast<unsigned int>(TopAbs_COMPOUND))) | (1 << (static_cast<unsigned int>(TopAbs_SOLID))),
      // FACE to:
      (1 << (static_cast<unsigned int>(TopAbs_COMPOUND))) | (1 << (static_cast<unsigned int>(TopAbs_SHELL))),
      // WIRE to:
      (1 << (static_cast<unsigned int>(TopAbs_COMPOUND))) | (1 << (static_cast<unsigned int>(TopAbs_FACE))),
      // EDGE to:
      (1 << (static_cast<unsigned int>(TopAbs_COMPOUND))) | (1 << (static_cast<unsigned int>(TopAbs_SOLID)))
        | (1 << (static_cast<unsigned int>(TopAbs_WIRE))),
      // VERTEX to:
      (1 << (static_cast<unsigned int>(TopAbs_COMPOUND))) | (1 << (static_cast<unsigned int>(TopAbs_SOLID)))
        | (1 << (static_cast<unsigned int>(TopAbs_FACE))) | (1 << (static_cast<unsigned int>(TopAbs_EDGE))),
      // SHAPE to:
      0};
