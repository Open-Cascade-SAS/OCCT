...
TEST(Geom_CurveEvalTest, Circle_EvalD0_ReturnsValidPoint)
{
  ...
  const gp_Pnt aResult = aCurve->EvalD0(aU);
...
TEST(Geom_CurveEvalTest, Circle_EvalD1D2D3_ReturnsValidDerivatives)
{
  ...
  // EvalD1
  const Geom_Curve::ResD1 aD1 = aCurve->EvalD1(aU);
  ...
  // EvalD2
  const Geom_Curve::ResD2 aD2 = aCurve->EvalD2(aU);
  ...
  // EvalD3
  const Geom_Curve::ResD3 aD3 = aCurve->EvalD3(aU);
...
TEST(Geom_CurveEvalTest, Line_EvalD2_ZeroSecondDerivative)
{
  ...
  const Geom_Curve::ResD2 aD2 = aCurve->EvalD2(5.0);
...
TEST(Geom_CurveEvalTest, BSplineCurve_EvalD1_ConsistentWithOldAPI)
{
  ...
  const Geom_Curve::ResD1 aEvalResult = aCurve->EvalD1(aMid);
...
TEST(Geom_CurveEvalTest, EvalDN_N1_ConsistentWithEvalD1)
{
  ...
  const Geom_Curve::ResD1 aD1  = aCurve->EvalD1(aU);
  const gp_Vec            aDN1 = aCurve->EvalDN(aU, 1);
...
TEST(Geom_CurveEvalTest, OffsetCurve_EvalD0_ReturnsNulloptAtSingular)
{
  ...
  const gp_Pnt aResult = anOffset->EvalD0(0.0);
  // For a circle offset by -radius, this should still return a valid point (the center)
  EXPECT_NEAR(aResult.Distance(gp_Pnt(0.0, 0.0, 0.0)), 0.0, Precision::Confusion());
}
...
TEST(Geom_CurveEvalTest, Parabola_EvalD3_ZeroThirdDerivative)
{
  ...
  const Geom_Curve::ResD3 aD3 = aCurve->EvalD3(1.0);
...
TEST(Geom_CurveEvalTest, BSplineCurve_EvalDN_InvalidN_Throws)
{
  ...
  EXPECT_THROW(aCurve->EvalDN(aMid, 0), Geom_UndefinedDerivative);
  EXPECT_THROW(aCurve->EvalDN(aMid, -1), Geom_UndefinedDerivative);
}
...
TEST(Geom_CurveEvalTest, TrimmedCurve_EvalD1_DelegatesToBasis)
{
  ...
  const Geom_Curve::ResD1 aBasisD1   = aBasis->EvalD1(aU);
  const Geom_Curve::ResD1 aTrimmedD1 = aTrimmed->EvalD1(aU);
...
TEST(Geom_CurveEvalTest, OffsetCurve_EvalD1_DegenerateAtCollapsed)
{
  ...
  const Geom_Curve::ResD1 aResult = anOffset->EvalD1(0.0);
