...
//=======================================================================
static void Rational(const NCollection_Array2<double>& Weights,
                     bool&           Urational,
                     bool&           Vrational)
{
  int I, J;
    ...
    : maxderivinvok(false)
{
  ...
  occ::handle<TColgp_HArray2OfPnt> npoles = new TColgp_HArray2OfPnt(1, NbUPoles, 1, NbVPoles);
  ...
  // Init non rational
  Init(npoles, occ::handle<TColStd_HArray2OfReal>());
}
    ...
    : maxderivinvok(false)
{
  ...
  occ::handle<TColStd_HArray2OfReal> nweights;
...
Geom_BezierSurface::Geom_BezierSurface(const occ::handle<TColgp_HArray2OfPnt>&   SurfacePoles,
                                       const occ::handle<TColStd_HArray2OfReal>& PoleWeights,
...
void Geom_BezierSurface::ExchangeUV()
{
  ...
  occ::handle<TColgp_HArray2OfPnt>   npoles = new TColgp_HArray2OfPnt(LC, UC, LR, UR);
  occ::handle<TColStd_HArray2OfReal> nweights;
  if (!weights.IsNull())
  {
...
void Geom_BezierSurface::Increase(const int UDeg, const int VDeg)
{
  ...
  occ::handle<TColgp_HArray2OfPnt>   npoles;
  occ::handle<TColStd_HArray2OfReal> nweights;
                                 ...
                                 const double V2)
{
  ...
  occ::handle<TColgp_HArray2OfPnt>   Coefs;
  occ::handle<TColStd_HArray2OfReal> WCoefs;
  ...
  NCollection_Array1<double> biduflatknots(BSplCLib::FlatBezierKnots(UDegree()), 1, 2 * (UDegree() + 1));
  NCollection_Array1<double> bidvflatknots(BSplCLib::FlatBezierKnots(VDegree()), 1, 2 * (VDegree() + 1));
  ...
  if (UDegree() <= VDegree())
  {
    occ::handle<TColgp_HArray2OfPnt>   coeffs  = Coefs;
    occ::handle<TColStd_HArray2OfReal> wcoeffs = WCoefs;
    int              ii, jj;
...
void Geom_BezierSurface::Bounds(double& U1,
                                double& U2,
                                double& V1,
                                double& V2) const
{
...
gp_Vec Geom_BezierSurface::DN(const double    U,
                              const double    V,
                              const int Nu,
                              const int Nv) const
{
...
void Geom_BezierSurface::Init(const occ::handle<TColgp_HArray2OfPnt>&   Poles,
                              const occ::handle<TColStd_HArray2OfReal>& Weights)
{
  // set fields
  poles = Poles;
  if (urational || vrational)
