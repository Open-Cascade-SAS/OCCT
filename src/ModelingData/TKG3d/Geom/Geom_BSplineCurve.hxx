...
public:
  ...
  Standard_EXPORT bool IsG1(const double theTf,
                                        const double theTl,
  ...
  const occ::handle<TColgp_HArray1OfPnt>& HArrayPoles() const { return poles; }

  //! Returns handle to the array of weights for efficient grid evaluation.
  //! May be null for non-rational curves.
  const occ::handle<TColStd_HArray1OfReal>& HArrayWeights() const { return weights; }

  //! Returns handle to the flat knots array for efficient grid evaluation.
  const occ::handle<TColStd_HArray1OfReal>& HArrayFlatKnots() const { return flatknots; }
