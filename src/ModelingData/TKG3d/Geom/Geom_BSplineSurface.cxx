...
void Geom_BSplineSurface::IncreaseDegree(const int UDegree, const int VDegree)
{
  if (UDegree != myUDeg)
  {
    ...
    int nbknots = BSplCLib::IncreaseDegreeCountKnots(myUDeg,
                                                     UDegree,
                                                     myUPeriodic,
                                                     myUMults);
  ...
  if (VDegree != myVDeg)
  {
    ...
    int nbknots = BSplCLib::IncreaseDegreeCountKnots(myVDeg,
                                                     VDegree,
                                                     myVPeriodic,
                                                     myVMults);
...
void Geom_BSplineSurface::SetUKnot(const int UIndex, const double K)
{
  ...
  {
    if (K <= myUKnots.Value(NewIndex - 1) + DU
        || K >= myUKnots.Value(NewIndex + 1) - DU)
    {
      throw Standard_ConstructionError("Geom_BSplineSurface::SetUKnot: K out of range");
    }
  }
...
void Geom_BSplineSurface::SetUKnots(const NCollection_Array1<double>& UK)
{
  ...
  int Lower = UK.Lower();
  int Upper = UK.Upper();
  if (Lower < 1 || Lower > myUKnots.Length() || Upper < 1
      || Upper > myUKnots.Length())
  {
...
void Geom_BSplineSurface::SetVKnot(const int VIndex, const double K)
{
  ...
  {
    if (K <= myVKnots.Value(NewIndex - 1) + DV
        || K >= myVKnots.Value(NewIndex + 1) - DV)
    {
      throw Standard_ConstructionError("Geom_BSplineSurface::SetVKnot: K out of range");
    }
  }
...
void Geom_BSplineSurface::SetVKnots(const NCollection_Array1<double>& VK)
{
  ...
  int Lower = VK.Lower();
  int Upper = VK.Upper();
  if (Lower < 1 || Lower > myVKnots.Length() || Upper < 1
      || Upper > myVKnots.Length())
  {
...
void Geom_BSplineSurface::UpdateUKnots()
{
  ...
  if (uknotSet == GeomAbs_Uniform && !myUPeriodic)
  {
  ...
  else
  {
    myUFlatKnots.Resize(
      1,
      BSplCLib::KnotSequenceLength(myUMults, myUDeg, myUPeriodic),
      false);

    BSplCLib::KnotSequence(myUKnots,
                           myUMults,
                           myUDeg,
                           myUPeriodic,
                           myUFlatKnots);
...
void Geom_BSplineSurface::UpdateVKnots()
{
  ...
  if (vknotSet == GeomAbs_Uniform && !myVPeriodic)
  {
  ...
  else
  {
    myVFlatKnots.Resize(
      1,
      BSplCLib::KnotSequenceLength(myVMults, myVDeg, myVPeriodic),
      false);

    BSplCLib::KnotSequence(myVKnots,
                           myVMults,
                           myVDeg,
                           myVPeriodic,
                           myVFlatKnots);
...
void Geom_BSplineSurface::SetWeight(const int UIndex, const int VIndex, const double Weight)
{
  ...
  if (myWeights.Size() == 0)
  {
    myWeights.Resize(myPoles.LowerRow(), myPoles.UpperRow(),
                          myPoles.LowerCol(), myPoles.UpperCol(), false);
    myWeights.Init(1.0);
  }
  myWeights(UIndex + myWeights.LowerRow() - 1,
                 VIndex + myWeights.LowerCol() - 1) = Weight;
                                       ...
                                       const NCollection_Array1<double>& CPoleWeights)
{
  ...
  while (I <= CPoleWeights.Upper())
  {
    ...
    myWeights(I + myWeights.LowerRow() - 1,
                   VIndex + myWeights.LowerCol() - 1) = CPoleWeights(I);
    I++;
  }
                                       ...
                                       const NCollection_Array1<double>& CPoleWeights)
{
  ...
  while (I <= CPoleWeights.Upper())
  {
    ...
    myWeights(UIndex + myWeights.LowerRow() - 1,
                   I + myWeights.LowerCol() - 1) = CPoleWeights(I);
    I++;
  }
  // Verifie si c'est rationnel
  Rational(myWeights, urational, vrational);
