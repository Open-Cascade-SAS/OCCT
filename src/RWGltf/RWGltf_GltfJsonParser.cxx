...
namespace
{
  ...
  {
    return parseObject(myExtrasValue) ? myResult : Handle(TDataStd_NamedData)();
  }
  ...
  bool RWGltf_ExtrasParser::parseNumber(const RWGltf_JsonValue& theValue, const std::string& theValueName)
  {
    ...
    if (theValue.IsInt())
    {
      myResult->SetInteger(theValueName.c_str(), theValue.GetInt());
      return true;
    }

    if (theValue.IsDouble())
    {
      myResult->SetReal(theValueName.c_str(), theValue.GetDouble());
      return true;
    }
  ...
  bool RWGltf_ExtrasParser::parseString(const RWGltf_JsonValue& theValue, const std::string& theValueName)
  {
    ...
    {
      myResult->SetString(theValueName.c_str(), theValue.GetString());
      return true;
  ...
  bool RWGltf_ExtrasParser::parseArray(const RWGltf_JsonValue& theValue, const std::string& theValueName)
  {
    ...
    if (theValue.Size() == 0)
    {
      ...
      myResult->SetString(theValueName.c_str(), "");
      return true;
    }
    ...
    if (theValue[0].IsInt())
    {
      ...
      myResult->SetArrayOfIntegers(theValueName.c_str(), anArray);
      return true;
    }
    else if (theValue[0].IsDouble())
    {
      ...
      myResult->SetArrayOfReals(theValueName.c_str(), anArray);
      return true;
    }
    else if (theValue[0].IsString())
    {
      ...
      myResult->SetString(theValueName.c_str(), anArrayString.c_str());
      return true;
    }
