...
GeomPlate_BuildAveragePlane::GeomPlate_BuildAveragePlane(const occ::handle<TColgp_HArray1OfPnt>& Pts,
...
GeomPlate_BuildAveragePlane::GeomPlate_BuildAveragePlane(const NCollection_Sequence<gp_Vec>&        Normals,
                                                         const occ::handle<TColgp_HArray1OfPnt>& Pts)
    : myPts(Pts)
{
  int i, j, k, n, m;
  ...
  else // the common case
  {
    ...
    for (i = 1; i <= NN - 1; i++)
      for (j = i + 1; j <= NN; j++)
      {
        double Angle = Normals(i).Angle(Normals(j));
        if (Angle > MaxAngle)
          MaxAngle = Angle;
      }
    MaxAngle *= 1.2;
    ...
    for (i = 1; i <= NN - 1; i++)
      for (j = i + 1; j <= NN; j++, k++)
      {
        ...
        for (n = 0; n <= 2 * Nint; n++)
        {
          ...
          double minScal = RealLast();
          for (m = 1; m <= NN; m++)
          {
            double Scal = Vec1 * Normals(m);
            if (Scal < minScal)
              minScal = Scal;
          }
