...
static double ComputeTorsion(const double            Param,
                                    const occ::handle<Adaptor3d_Curve>& aCurve)
{
  double Torsion;
...
static void smoothlaw(occ::handle<Law_BSpline>&                 Law,
                      const occ::handle<TColStd_HArray1OfReal>& Points,
                      const occ::handle<TColStd_HArray1OfReal>& Param,
                      const double                  Tol)
{
  double       tol, d;
  int    ii, Nbk;
...
{
  bool            found = true;
  occ::handle<TColgp_HArray1OfPnt> TabP;
                                                        ...
                                                        NCollection_Sequence<gp_Vec>&   SeqNormal) const
{
  ...
  else
  {
    ...
    occ::handle<TColStd_HArray1OfReal> pararr     = new TColStd_HArray1OfReal(1, Length);
    occ::handle<TColStd_HArray1OfReal> angleATarr = new TColStd_HArray1OfReal(1, Length);
...
GeomFill_Trihedron GeomFill_CorrectedFrenet::EvaluateBestMode()
{
  ...
  for (i = 1; i <= NbInt; i++)
  {
    ...
    occ::handle<Law_Function> trimmedlaw = EvolAroundT->Trim(tmin, tmax, Precision::PConfusion() / 2);
