    ...
    if (InitOffsetFace.HasImage(E1))
    {
      ...
      for (const auto & i : V)
      {
        if (!InitOffsetFace.HasImage(i))
        {
          ...
          //-----------------------------
          const NCollection_List<TopoDS_Shape>& Anc = Analyse.Ancestors(i);
          NCollection_List<TopoDS_Shape>        TangOnV;
          Analyse.TangentEdges(E1, i, TangOnV);
    ...
    for (exp.Init(CF.Oriented(TopAbs_FORWARD), TopAbs_EDGE); exp.More(); exp.Next())
    {
      ...
      for (const auto & i : V)
      {
        if (!MV.Add(i))
          continue;
        OF1.Nullify();
        const NCollection_List<TopoDS_Shape>&    LE = Analyse.Ancestors(i);
        ...
        for (; itLE.More(); itLE.Next())
        {
          ...
          if (InitOffsetFace.HasImage(EV))
          {
            ...
            OE  = TopoDS::Edge(InitOffsetEdge.Image(i).First());
