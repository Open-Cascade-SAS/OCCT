  ...
  for (; Exp.More(); Exp.Next(), aPSIntF.Next())
  {
    ...
    if (!L.IsEmpty() && L.First().Type() == OT)
    {
      ...
      if (Anc.Extent() == 2)
      {
        ...
        F1                        = TopoDS::Face(InitOffsetFace.FirstImage(InitF1));
        F2                        = TopoDS::Face(InitOffsetFace.FirstImage(InitF2));
        if (!IsDone(F1, F2))
        {
  ...
  const Message_ProgressRange&                                         theRange)
{
  ...
  for (j = 1; j <= ContextFaces.Extent(); j++, aPS.Next())
  {
    ...
    for (exp.Init(CF.Oriented(TopAbs_FORWARD), TopAbs_EDGE); exp.More(); exp.Next())
    {
      ...
      OF1                   = TopoDS::Face(InitOffsetFace.FirstImage(SI));
      OE                    = TopoDS::Edge(InitOffsetEdge.FirstImage(E));
      ...
      for (int i = 0; i < 2; i++)
      {
        ...
        for (; itLE.More(); itLE.Next())
        {
          ...
          if (InitOffsetFace.HasImage(EV))
          {
            ...
            //-------------------------------------------------
            OF1 = TopoDS::Face(InitOffsetFace.FirstImage(EV));
            OE  = TopoDS::Edge(InitOffsetEdge.FirstImage(V[i]));
    ...
    for (exp.Init(CF.Oriented(TopAbs_FORWARD), TopAbs_VERTEX); exp.More(); exp.Next())
    {
      ...
      for (; itLE.More(); itLE.Next())
      {
        ...
        for (; itLF.More(); itLF.Next())
        {
          ...
          //-------------------------------------------------
          OF1 = TopoDS::Face(InitOffsetFace.FirstImage(FEV));
          if (!IsDone(OF1, CF))
          {
            //-------------------------------------------------------
