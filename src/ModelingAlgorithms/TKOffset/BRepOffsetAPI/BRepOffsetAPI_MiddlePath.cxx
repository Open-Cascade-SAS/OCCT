  ...
  for (;;)
  {
    for (i = 1; i <= myPaths.Length(); i++)
    {
      ...
      if (!NextEdgeCandidates.IsEmpty())
      {
        ...
        {
          NCollection_Map<TopoDS_Shape, TopTools_ShapeMapHasher>::Iterator mapit(NextEdgeCandidates);
  ...
  */
  ...
  for (;;)
  {
    ...
    for (j = 2; j <= NbVer; j++)
    {
      ...
      TopoDS_Vertex PrevPrevVer =
        (E1.IsNull()) ? PrevVertex : TopExp::FirstVertex(E1, true);
  ...
  for (i = 1; i < NbSecFaces; i++)
  {
    if (MidEdges(i).IsNull())
    {
      ...
      occ::handle<TColgp_HArray1OfPnt>      thePoints = new TColgp_HArray1OfPnt(1, j - i + 1);
      NCollection_Array1<gp_Vec>               theTangents(1, j - i + 1);
      occ::handle<TColStd_HArray1OfBoolean> theFlags = new TColStd_HArray1OfBoolean(1, j - i + 1);
      for (k = i; k <= j; k++)
