...
//! Internal cache-value class storing cached UV bounds data.
class BRepGraphAlgo_UVBoundsCacheValue : public BRepGraph_CacheValue
{
public:
  DEFINE_STANDARD_RTTI_INLINE(BRepGraphAlgo_UVBoundsCacheValue, BRepGraph_CacheValue)
  BRepGraphAlgo_UVBoundsCacheValue() = default;
                                     ...
                                     CachedData&            theData)
{
  ...
  {
    const BRepGraphInc::FaceDef& aFaceEnt = theGraph.Topo().Faces().Definition(theFace);
  ...
  if (hasFaceWires)
  {
    ...
    auto checkWireEdges = [&](BRepGraph_WireId theWireId) {
      ...
      for (int aCRI = 0; aCRI < aWireEnt.CoEdgeRefIds.Length() && !isAborted; ++aCRI)
      {
        ...
        const BRepGraphInc::CoEdgeRef& aCR      = aRefs.CoEdges().Entry(aCERefId);
    ...
    const BRepGraphInc::FaceDef& aFaceEnt2 = theGraph.Topo().Faces().Definition(theFace);
  ...
  Bnd_Box2d                    aBox;
  const BRepGraphInc::FaceDef& aFaceEnt3 = theGraph.Topo().Faces().Definition(theFace);
...
const occ::handle<BRepGraph_CacheKind>& BRepGraphAlgo_UVBounds::CacheKind()
{
  static const occ::handle<BRepGraph_CacheKind> THE_KIND =
    new BRepGraph_CacheKind(THE_UVBOUNDS_GUID,
                            "UVBounds",
                            BRepGraph_CacheKind::KindBit(BRepGraph_NodeId::Kind::Face));
  return THE_KIND;
}
                                       ...
                                       CachedData&            theData)
{
  ...
  occ::handle<BRepGraph_CacheValue> aValue = theGraph.Cache().Get(theFace, cacheKindSlot());
  if (aValue.IsNull())
  {
    return false;
  }
  occ::handle<BRepGraphAlgo_UVBoundsCacheValue> aUVValue =
    occ::down_cast<BRepGraphAlgo_UVBoundsCacheValue>(aValue);
  return !aUVValue.IsNull() && aUVValue->Get(theData);
}
                                                                     ...
                                                                     const BRepGraph_FaceId theFace)
{
  ...
  {
    occ::handle<BRepGraphAlgo_UVBoundsCacheValue> aUVValue =
      occ::down_cast<BRepGraphAlgo_UVBoundsCacheValue>(anExistingValue);
    if (!aUVValue.IsNull())
    {
      CachedData aData;
      if (aUVValue->Get(aData))
      {
        return aData;
      }
    }
...
void BRepGraphAlgo_UVBounds::InvalidateCached(BRepGraph& theGraph, const BRepGraph_FaceId theFace)
{
  ...
  (void)theGraph.Cache().Remove(theFace, cacheKindSlot());
}
