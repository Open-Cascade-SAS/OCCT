                                                                       ...
                                                                       const Options& theOptions)
{
  ...
  const double aMinTol =
    theOptions.MinTolerance > 0.0
      ...
      : std::max(Precision::Confusion(), theGraph.Topo().Edges().Nb() > 0 ? 1.0e-10 : 0.0);
  ...
  for (int aFaceIdx = 0; aFaceIdx < aDefs.Faces().Nb(); ++aFaceIdx)
  {
    ...
    for (int aWireRefIter = 0; aWireRefIter < aFaceEnt.WireRefIds.Length(); ++aWireRefIter)
    {
      ...
      if (aWR.IsRemoved || !aWR.WireDefId.IsValid(aDefs.Wires().Nb()))
      {
        continue;
      }
           ...
           ++aCoEdgeRefIter)
      {
        ...
        if (aCR.IsRemoved || !aCR.CoEdgeDefId.IsValid(aDefs.CoEdges().Nb()))
        {
          continue;
        }
        ...
        if (!anEdgeId.IsValid(aDefs.Edges().Nb()))
        {
          continue;
        }
  ...
  if (!aFinalMerge.IsEmpty())
  {
    for (int anEdgeIdx = 0; anEdgeIdx < aDefs.Edges().Nb(); ++anEdgeIdx)
    {
      ...
      int                          aNewStart   = anEdge.StartVertexRefId.IsValid()
                                                   ? aRefs.Vertices().Entry(anEdge.StartVertexRefId).VertexDefId.Index
                                                   : -1;
      int                          aNewEnd     = anEdge.EndVertexRefId.IsValid()
                                                   ? aRefs.Vertices().Entry(anEdge.EndVertexRefId).VertexDefId.Index
                                                   : -1;
      ...
      if (aNeedUpdate)
      {
        const BRepGraphInc::EdgeDef& anEdgeRef =
          aDefs.Edges().Definition(BRepGraph_EdgeId(anEdgeIdx));
