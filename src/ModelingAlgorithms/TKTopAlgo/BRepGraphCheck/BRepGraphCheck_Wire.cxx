  ...
  const BRepGraph_NodeId theWireNodeId)
{
  ...
  const BRepGraphInc::WireDef& aWireDef = aDefs.Wires().Definition(BRepGraph_WireId(theWireNodeId.Index));
...
{
  const BRepGraph::TopoView&                          aDefs       = theGraph.Topo();
  const BRepGraphInc::WireDef&                        aWireDef    = aDefs.Wires().Definition(theWire);
                                     ...
                                     NCollection_Vector<BRepGraphCheck_Issue>& theIssues)
{
  ...
  const int aNbEdges = aCoEdgeRefs.Length();
  {
    ...
    while (aQueueHead < aQueueTail)
    {
      ...
      const BRepGraphInc::CoEdgeDef&   aCurrCoEdge = aDefs.CoEdges().Definition(aCurrCR.CoEdgeDefId);
      ...
      && !aHasSeamEdge)
  {
    ...
    const BRepGraphInc::CoEdgeDef& aFirstCoEdge = aDefs.CoEdges().Definition(aFirstCR.CoEdgeDefId);
    const BRepGraphInc::CoEdgeDef& aLastCoEdge  = aDefs.CoEdges().Definition(aLastCR.CoEdgeDefId);
