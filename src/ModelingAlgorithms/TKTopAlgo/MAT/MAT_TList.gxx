...
{
  bool previousisnull = thecurrentnode->Previous().IsNull();
  bool nextisnull     = thecurrentnode->Next().IsNull();
...
void MAT_TList::LinkBefore(const Item& anitem)
{
  ...
  occ::handle<MAT_TListNode> previous;

  occ::handle<MAT_TListNode> node = new MAT_TListNode(anitem);
...
{
  occ::handle<MAT_TListNode> previous = thecurrentnode->Previous();
  occ::handle<MAT_TListNode> current  = thecurrentnode;
  occ::handle<MAT_TListNode> next     = thecurrentnode->Next();
  occ::handle<MAT_TListNode> nextnext = next->Next();
  occ::handle<MAT_TListNode> null;
...
MAT_TList::~MAT_TList()
{
  occ::handle<MAT_TListNode> aNode = thefirstnode;
  while (!aNode.IsNull())
  {
    occ::handle<MAT_TListNode> aNext = aNode->Next();
    aNode->Next(NULL);
    aNode->Previous(NULL);
    aNode = aNext;
  }
