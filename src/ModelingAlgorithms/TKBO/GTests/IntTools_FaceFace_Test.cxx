...
TEST_F(IntTools_FaceFaceTest, BrepFilesBooleanCommon_Solids)
{
  ...
  const char* anArgPath = "/Users/dpasukhi/work/OCCT/vol1.brep";
  const char* aToolPath = "/Users/dpasukhi/work/OCCT/vol2.brep";
  std::swap(anArgPath, aToolPath); // Swap to match original test case
                                          ...
                                          double             theFuzzyValue = 1.0e-6)
{
  ...
  for (TopExp_Explorer anExp(anArgShape, TopAbs_EDGE); anExp.More(); anExp.Next())
    ++anArgEdgeCount;
  std::cout << "Argument: " << anArgFaceCount << " faces, " << anArgEdgeCount << " edges"
            << std::endl;
...
{
  runBooleanCommonInvestigation("/Users/dpasukhi/work/OCCT/NotDetected/",
                                "NotDetected_Fuzzy1e5",
                                1.0e-5);
}
