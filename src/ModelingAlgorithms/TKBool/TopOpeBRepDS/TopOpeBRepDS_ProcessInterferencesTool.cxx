...
Standard_EXPORT int FUN_selectTRAUNKinterference
  ...
  (NCollection_List<occ::handle<TopOpeBRepDS_Interference>>& L1, NCollection_List<occ::handle<TopOpeBRepDS_Interference>>& L2)
{
  NCollection_List<occ::handle<TopOpeBRepDS_Interference>>::Iterator it1(L1);
  while (it1.More())
  {
...
Standard_EXPORT void FUN_unkeepUNKNOWN
  //------------------------------------------------------
  (NCollection_List<occ::handle<TopOpeBRepDS_Interference>>& LI, TopOpeBRepDS_DataStructure& /*BDS*/, const int)
{
  //                 BDS.Shape(SIX);
  NCollection_List<occ::handle<TopOpeBRepDS_Interference>>::Iterator it1;
...
static bool FUN_find3dISEsameISF(const occ::handle<TopOpeBRepDS_Interference>&       I1,
...
// ------------------------------------------------------------
static int FUN_select3dISEsameISF(NCollection_List<occ::handle<TopOpeBRepDS_Interference>>& lFE,
                                               NCollection_List<occ::handle<TopOpeBRepDS_Interference>>& l3dFE,
...
// TRASHAk = TopAbs_EDGE : <l2dI> = {I2d = (T on same edge, G,S)}
{
  ...
  for (NCollection_List<occ::handle<TopOpeBRepDS_Interference>>::Iterator itt(l3dF); itt.More(); itt.Next())
...
// <l2dFE> = {I2dFE = (T(face),G,S=edge) / we cannot find (T(face),G,face)}
{
  ...
  while (itFE.More())
  {
    ...
    bool foundIF = false;
    for (NCollection_List<occ::handle<TopOpeBRepDS_Interference>>::Iterator itF(lF); itF.More(); itF.Next())
    {
