...
public:
  ...
  Standard_EXPORT ShapeExtend_CompositeSurface(
    const occ::handle<TColGeom_HArray2OfSurface>& GridSurf,
    const ShapeExtend_Parametrisation        param = ShapeExtend_Natural);

  //! Initializes by a grid of surfaces (calls Init()).
  Standard_EXPORT ShapeExtend_CompositeSurface(const occ::handle<TColGeom_HArray2OfSurface>& GridSurf,
  ...
  Standard_EXPORT bool
    Init(const occ::handle<TColGeom_HArray2OfSurface>& GridSurf,
         const ShapeExtend_Parametrisation        param = ShapeExtend_Natural);
  ...
  //! returns False.
  //! However, class is initialized even in that case.
  Standard_EXPORT bool Init(const occ::handle<TColGeom_HArray2OfSurface>& GridSurf,
                                        const NCollection_Array1<double>&              UJoints,
                                        const NCollection_Array1<double>&              VJoints);
  ...
  Standard_EXPORT const occ::handle<TColGeom_HArray2OfSurface>& Patches() const;
  ...
  Standard_EXPORT occ::handle<TColStd_HArray1OfReal> UJointValues() const;

  //! Returns the array of V values corresponding to joint
  //! points between patches as well as to start and end points,
  //! which define global parametrisation of the surface
  Standard_EXPORT occ::handle<TColStd_HArray1OfReal> VJointValues() const;
...
private:
  occ::handle<TColGeom_HArray2OfSurface> myPatches;
  occ::handle<TColStd_HArray1OfReal>     myUJointValues;
  occ::handle<TColStd_HArray1OfReal>     myVJointValues;
  bool                  myUClosed;
  bool                  myVClosed;
};
