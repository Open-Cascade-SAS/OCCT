...
void ShapeFix_Face::Init (const occ::handle<Geom_Surface>& surf,
			  const double preci, const bool fwd) 
{
  myStatus = 0;
...
//  for (TopoDS_Iterator ws (face,false); ws.More(); ws.Next())
...
//=======================================================================
static bool SplitWire(const TopoDS_Face &face, const TopoDS_Wire& wire,
  ...
  for(i=1; i<=sewd->NbEdges(); i++) {
    ...
    for(j=2; j<=sewd->NbEdges() && IsConnectedEdge; j++) {
      ...
      if(V1.IsSame(V0)) {
        ...
        double tol = Max(BRep_Tool::Tolerance(V0),BRep_Tool::Tolerance(V1));
        double maxResolution = 2 * Max ( anAdaptor.UResolution(tol), anAdaptor.VResolution(tol) );
...
bool ShapeFix_Face::Perform() 
{
  ...
  if (theAdvFixWire.IsNull()) return false;
  ...
  if ( NeedFix ( myFixWireMode ) ) {
    ...
    double dPreci = aSavPreci*aSavPreci;
    dPreci*=4;
    double newpreci=dPreci;
    ...
    */
    ...
    if( myAutoCorrectPrecisionMode ) {
      ...
      double newpreci = Min(aSavPreci,size/2.);
...
static void Shift2dWire(const TopoDS_Wire& w, const TopoDS_Face& f,
			const gp_Vec2d vec, 
                        const occ::handle<ShapeAnalysis_Surface>& mySurf,
                        bool recompute3d = false)
{
  ...
  for (TopoDS_Iterator ei (w,false); ei.More(); ei.Next()){
    ...
    double cf, cl;
    if ( ! sae.PCurve(edge, f, C2d, cf, cl, true) ) continue; 
    C2d->Transform(tr2d);
    if ( recompute3d ) {
...
static bool CutInterval (TColgp_SequenceOfPnt2d &intervals, 
                                     const gp_Pnt2d &toAddI,
                                     const double period)
{
  if ( intervals.Length() <=0 ) return false;
  for ( int j=0; j <2; j++ ) { // try twice, align to bottom and to top
...
static double FindBestInterval (TColgp_SequenceOfPnt2d &intervals)
{
...
/* variant 1
  ...
  occ::handle<TColGeom_HArray2OfSurface> grid = new TColGeom_HArray2OfSurface ( 1, 1, 1, 1 );
  ...
  CompShell.NaturalBoundMode() = true;
...
/* variant 2 */
  ...
  if ( mySurf->Adaptor3d()->GetType() == GeomAbs_Sphere &&
       ws.Length() == nb+1 ) {
    occ::handle<ShapeExtend_WireData> bnd = 
...
// purpose  :
//=======================================================================
bool ShapeFix_Face::isNeedAddNaturalBound(const TopTools_SequenceOfShape& theOrientedWires)
{
...
bool ShapeFix_Face::FixOrientation(TopTools_DataMapOfShapeListOfShape &MapWires) 
{
  bool done = false;
  ...
  if ( VerySmallWires.Length() >0 ) done = true;
  ...
  if ( nb == 1 ) {
    ...
    {
      occ::handle<ShapeExtend_WireData> sbdw =
...
//    double toluv = Min ( Ads.UResolution(Precision()), Ads.VResolution(Precision()) );
                                   ...
                                   bool &isDeg)
{
  ...
  for ( TopoDS_Iterator ed(wire); ed.More(); ed.Next() ) {
    ...
    if ( ! BRep_Tool::Degenerated ( edge ) ) isDeg = false;
...
bool ShapeFix_Face::FixMissingSeam() 
{
  ...
  if ( ! uclosed && ! vclosed ) return false; 
...
//    else return false; //  abort
  ...
  for ( int i1 = 1; i1 <= nb1 + nb2; i1++ ) {
    ...
    occ::handle<Geom2d_Curve> c2d;
    double f, l;
    if ( ! sae.PCurve ( edge1, tmpF, c2d, f, l, true ) ) return false;
    ...
    if ( uclosed && ismodeu ) {
      ...
      if ( foundU ==2 && Abs ( pos1.X() ) > Abs(uf) ) skipU = true;
    ...
    if ( vclosed && ! ismodeu ) {
      ...
      if ( foundV ==2 && Abs ( pos1.Y() ) > Abs(vf) ) skipV = true;
    ...
    for ( int i2 = 1; i1 <= nb1 && i2 <= nb2; i2++ ) {
      ...
      if ( ! sae.PCurve ( edge2, tmpF, c2d, f, l, true ) ) return false;
...
bool ShapeFix_Face::FixLoopWire(TopTools_SequenceOfShape& aResWires)
{
  TopTools_IndexedMapOfShape aMapVertices;
  TopTools_DataMapOfShapeListOfShape aMapVertexEdges;
  ...
  bool isDone =(aResWires.Length() && isClosed);
...
bool ShapeFix_Face::FixSplitFace(const TopTools_DataMapOfShapeListOfShape &MapWires) 
{
  BRep_Builder B;
  ...
  if(NbWires!=NbWiresNew) return false;
                                              ...
                                              bool& isUDecrease)
{
  ...
  for ( ; aWireIter.More(); aWireIter.Next() )
  {
    ...
    double aCurMaxU = Max(aUFirst, aULast),
                  aCurMinU = Min(aUFirst, aULast);
    double aCurMaxV = Max(aVFirst, aVLast),
  ...
  bool is2PIDelta = Abs(aCumulDeltaUAbs - 2*M_PI) <= theTolerance;
  bool isAroundApex = Abs(theMaxU - theMinU) > 2*M_PI - theTolerance;
...
bool ShapeFix_Face::FixPeriodicDegenerated() 
{
  ...
  bool isConicLoop = IsPeriodicConicalLoop(aConeSurf, aSoleWire, Precision(),
                                                       aMinLoopU, aMaxLoopU,
                                                       aMinLoopV, aMaxLoopV,
                                                       isUDecrease);
  ...
  double aConeBaseH = aConeBaseR / Sin(aSemiAngle);
