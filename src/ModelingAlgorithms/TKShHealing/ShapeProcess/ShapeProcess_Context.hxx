...
public:
  ...
  //! Calls method Init()
  Standard_EXPORT ShapeProcess_Context(const char* const file,
  ...
  Standard_EXPORT bool Init(const char* const file,
                                        const char* const scope = "");
  ...
  Standard_EXPORT bool GetInteger(const char* const param,
                                              int&      val) const;

  Standard_EXPORT bool GetBoolean(const char* const param,
                                              bool&      val) const;

  //! Get value of parameter as being of specific type
  //! Returns False if parameter is not defined or has a wrong type
  Standard_EXPORT bool GetString(const char* const   param,
                                             TCollection_AsciiString& val) const;

  Standard_EXPORT double RealVal(const char* const param,
                                        const double    def) const;

  Standard_EXPORT int IntegerVal(const char* const param,
                                              const int def) const;

  Standard_EXPORT bool BooleanVal(const char* const param,
                                              const bool def) const;

  //! Get value of parameter as being of specific type
  //! If parameter is not defined or does not have expected
  //! type, returns default value as specified
  Standard_EXPORT const char* StringVal(const char* const param,
