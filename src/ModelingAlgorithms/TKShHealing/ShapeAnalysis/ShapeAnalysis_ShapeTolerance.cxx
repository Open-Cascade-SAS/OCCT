...
occ::handle<TopTools_HSequenceOfShape> ShapeAnalysis_ShapeTolerance::OverTolerance(
...
occ::handle<TopTools_HSequenceOfShape> ShapeAnalysis_ShapeTolerance::InTolerance(
  const TopoDS_Shape&    shape,
  const double    valmin,
  const double    valmax,
  const TopAbs_ShapeEnum type) const
{
  ...
  occ::handle<TopTools_HSequenceOfShape> sl   = new TopTools_HSequenceOfShape();
  ...
  if (type == TopAbs_SHELL)
  {
    ...
    while (myExp.More())
    {
      ...
      for (TopExp_Explorer face(ash, TopAbs_FACE); face.More(); face.Next())
      {
        ...
        occ::handle<TopTools_HSequenceOfShape> fc = InTolerance(face.Current(), valmin, valmax, type);
        if (fc->Length() > 0)
        {
          sl->Append(fc);
          iashell = true;
    ...
    for (; myExp.More(); myExp.Next())
    {
      ...
      {
        // les edges contenues ?
        occ::handle<TopTools_HSequenceOfShape> fl =
