...
static void CorrectSplitValues(const occ::handle<TColStd_HSequenceOfReal>& orig3d,
                               const occ::handle<TColStd_HSequenceOfReal>& orig2d,
                               occ::handle<TColStd_HSequenceOfReal>        new2d,
                               occ::handle<TColStd_HSequenceOfReal>        new3d)
{
...
void ShapeUpgrade_WireDivide::Perform()
{
  ...
  for (TopoDS_Iterator ItW(myWire, false); ItW.More(); ItW.Next())
  {
    ...
    for (TopExp_Explorer exp(sh, TopAbs_EDGE); exp.More(); exp.Next())
    {
      ...
      occ::handle<TColStd_HSequenceOfReal> theKnots3d = myEdgeDivide->Knots3d();
      occ::handle<TColStd_HSequenceOfReal> theKnots2d = myEdgeDivide->Knots2d();

      // second iteration: transfer parameters and build segments
      occ::handle<TColStd_HSequenceOfReal> SplitValues2d;
      occ::handle<TColStd_HSequenceOfReal> SplitValues3d;
      ...
      occ::handle<TColGeom2d_HArray1OfCurve> theSegments2dR;
      if (isSeam)
      {
        ...
        if (sae.PCurve(erev, myFace, c2, f2, l2, false))
        {
          ...
          occ::handle<TColStd_HSequenceOfReal> revKnots2d = theSplit2dTool->SplitValues();
