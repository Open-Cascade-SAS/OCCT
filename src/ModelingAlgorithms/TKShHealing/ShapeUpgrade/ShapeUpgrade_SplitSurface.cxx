...
void ShapeUpgrade_SplitSurface::SetUSplitValues(const occ::handle<TColStd_HSequenceOfReal>& UValues)
{
  if (UValues.IsNull())
    return;
  constexpr double precision = Precision::PConfusion();
...
void ShapeUpgrade_SplitSurface::SetVSplitValues(const occ::handle<TColStd_HSequenceOfReal>& VValues)
{
  if (VValues.IsNull())
    return;
  constexpr double precision = Precision::PConfusion();
...
void ShapeUpgrade_SplitSurface::Build(const bool Segment)
{
  ...
  if (mySurface->IsKind(STANDARD_TYPE(Geom_SurfaceOfRevolution)))
  {
    ...
    spc.Build(Segment);
    occ::handle<TColGeom_HArray2OfSurface> Surfaces;
  ...
  else if (mySurface->IsKind(STANDARD_TYPE(Geom_OffsetSurface)))
  {
    ...
    sps.myStatus = myStatus;
    sps.Build(Segment);
    occ::handle<TColGeom_HArray2OfSurface> Patches =
...
const occ::handle<TColStd_HSequenceOfReal>& ShapeUpgrade_SplitSurface::USplitValues() const
{
...
const occ::handle<TColStd_HSequenceOfReal>& ShapeUpgrade_SplitSurface::VSplitValues() const
{
