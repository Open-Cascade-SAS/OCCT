...
void ShapeUpgrade_ConvertSurfaceToBezierBasis::Compute(const bool Segment)
{
  ...
  else if (mySurface->IsKind(STANDARD_TYPE(Geom_Plane)) && myPlaneMode)
  {
    ...
    VJoints(1)                             = VFirst;
    VJoints(2)                             = VLast;
    occ::handle<TColGeom_HArray2OfSurface> surf = new TColGeom_HArray2OfSurface(1, 1, 1, 1);
  ...
  else if (mySurface->IsKind(STANDARD_TYPE(Geom_BSplineSurface)) && myBSplineMode)
  {
    ...
    occ::handle<TColGeom_HArray2OfSurface> srf =
  ...
  else if (mySurface->IsKind(STANDARD_TYPE(Geom_SurfaceOfRevolution)) && myRevolutionMode)
  {
    ...
    occ::handle<TColGeom_HArray1OfCurve> curves;
    int                nbCurves;
    occ::handle<TColStd_HSequenceOfReal> vPar  = new TColStd_HSequenceOfReal;
    occ::handle<TColStd_HSequenceOfReal> vSVal = new TColStd_HSequenceOfReal;
    ...
    occ::handle<TColGeom_HArray2OfSurface> surf = new TColGeom_HArray2OfSurface(1, 1, 1, nbCurves);
  ...
  else if (mySurface->IsKind(STANDARD_TYPE(Geom_SurfaceOfLinearExtrusion)) && myExtrusionMode)
  {
    ...
    occ::handle<TColGeom_HArray1OfCurve>     curves;
    int                    nbCurves;
    occ::handle<TColStd_HSequenceOfReal>     uPar  = new TColStd_HSequenceOfReal;
    occ::handle<TColStd_HSequenceOfReal>     uSVal = new TColStd_HSequenceOfReal;
    ShapeUpgrade_ConvertCurve3dToBezier converter;
    ...
    occ::handle<TColGeom_HArray2OfSurface> surf = new TColGeom_HArray2OfSurface(1, nbCurves, 1, 1);
...
void ShapeUpgrade_ConvertSurfaceToBezierBasis::Build(const bool /*Segment*/)
{
  ...
  constexpr double       prec           = Precision::PConfusion();
  occ::handle<TColStd_HArray1OfReal> myUSplitParams = mySegments->UJointValues();
  occ::handle<TColStd_HArray1OfReal> myVSplitParams = mySegments->VJointValues();
  int              nbU            = myUSplitValues->Length();
  int              nbV            = myVSplitValues->Length();

  occ::handle<TColGeom_HArray2OfSurface> resSurfaces =
