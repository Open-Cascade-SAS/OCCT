...
public:
  ...
  //! @param theWeights    optional weights array (for rational surfaces)
...
private:
  ...
  double periodicNormalization(double theParam, double theFirst, double theLast, bool thePeriodic) const;

  //! Creates a cache template for cloning.
  void createCacheTemplate();
...
private:
  ...
  bool myIsRational;   //!< true if surface is rational

  // Cache grid storage - lazily populated
  mutable NCollection_Array2<occ::handle<BSplSLib_Cache>> myCacheGrid;

  // Shared allocator for cache building
  mutable occ::handle<NCollection_IncAllocator> myAllocator;
