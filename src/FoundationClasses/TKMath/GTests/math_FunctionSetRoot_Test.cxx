...
namespace
{
  ...
  public:
    ...
    virtual Standard_Boolean Derivatives(const math_Vector&, math_Matrix& D) override
    {
      ...
      D(2, 1) = 2.0; D(2, 2) = -1.0; D(2, 3) = 0.0;
      D(3, 1) = 0.0; D(3, 2) = 0.0; D(3, 3) = 1.0;
      return Standard_True;
    }
...
TEST(math_FunctionSetRoot, NonlinearSystem)
{
  ...
  const math_Vector& root = solver.Root();
  // Just check that we found a valid solution (may not be the exact analytical one due to algorithm limitations)
