...
public:
  ...
  Standard_Boolean Values(const math_Vector& theX, Standard_Real& theF, 
                         math_Vector& theG, math_Matrix& theH) override
  {
    Value(theX, theF);
    Gradient(theX, theG);
...
TEST(MathNewtonMinimumTest, NonConvexFunction)
{
  ...
  EXPECT_NO_THROW(aSolver.Perform(aFunc, aStartPoint)) << "Should handle non-convex function gracefully";
}

TEST(MathNewtonMinimumTest, NotDoneExceptions)
{
...
TEST(MathNewtonMinimumTest, DimensionErrors)
{
