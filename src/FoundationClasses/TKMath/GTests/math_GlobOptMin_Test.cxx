...
TEST(MathGlobOptMinTest, FunctionalMinimalValue)
{
  ...
  EXPECT_NEAR(aSolver.GetFunctionalMinimalValue(), -1.0, 1.0e-12) << "Functional minimal value should be set";
...
TEST(MathGlobOptMinTest, MultipleExtremaAccess)
{
  ...
  for (Standard_Integer i = 1; i <= aNbSol; ++i)
  {
    ...
    EXPECT_TRUE(aSol(1) >= -2.0 && aSol(1) <= 2.0) << "Solution " << i << " should be within bounds";
    EXPECT_TRUE(aSol(2) >= -2.0 && aSol(2) <= 2.0) << "Solution " << i << " should be within bounds";
  }
}
