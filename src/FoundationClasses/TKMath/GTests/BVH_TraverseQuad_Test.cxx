...
opencascade::handle<BVH_Tree<float, 3, BVH_QuadTree>> MakeFlatQBVH(
  const BVH_Vec3f& theMin0, const BVH_Vec3f& theMax0,
  const BVH_Vec3f& theMin1, const BVH_Vec3f& theMax1,
  const BVH_Vec3f& theMin2, const BVH_Vec3f& theMax2,
  const BVH_Vec3f& theMin3, const BVH_Vec3f& theMax3)
{
  auto aTree = new BVH_Tree<float, 3, BVH_QuadTree>;

  // Root bbox = union of children.
  BVH_Vec3f aRootMin(
    std::min({theMin0.x(), theMin1.x(), theMin2.x(), theMin3.x()}),
    std::min({theMin0.y(), theMin1.y(), theMin2.y(), theMin3.y()}),
    std::min({theMin0.z(), theMin1.z(), theMin2.z(), theMin3.z()}));
  BVH_Vec3f aRootMax(
    std::max({theMax0.x(), theMax1.x(), theMax2.x(), theMax3.x()}),
...
TEST(BVH_TraverseQuadTest, FlatRoot_RayHitsAllFour)
{
  ...
  auto aTree = MakeFlatQBVH(BVH_Vec3f(0, 0, 0), BVH_Vec3f(1, 1, 1),
                            BVH_Vec3f(2, 0, 0), BVH_Vec3f(3, 1, 1),
                            BVH_Vec3f(4, 0, 0), BVH_Vec3f(5, 1, 1),
                            BVH_Vec3f(6, 0, 0), BVH_Vec3f(7, 1, 1));
...
TEST(BVH_TraverseQuadTest, FlatRoot_RayHitsMosaic)
{
  ...
  auto aTree = MakeFlatQBVH(BVH_Vec3f(0, 0, 0), BVH_Vec3f(1, 1, 1),       // hit
                            BVH_Vec3f(2, 5, 0), BVH_Vec3f(3, 6, 1),       // miss
                            BVH_Vec3f(4, 0, 0), BVH_Vec3f(5, 1, 1),       // hit
                            BVH_Vec3f(6, 5, 0), BVH_Vec3f(7, 6, 1));      // miss
  BVH_Ray<float, 3>  aRay(BVH_Vec3f(-1, 0.5f, 0.5f), BVH_Vec3f(1, 0, 0));
  std::set<int>      aHits;
  CollectingAcceptor aAcc{&aHits};
  BVH::SIMD::TraverseQuad(aTree, aRay, aAcc);
