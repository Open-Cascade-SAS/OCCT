                                          ...
                                          const Standard_Real thePeriod) const
{
  ...
  const Standard_Real aPeriod = std::abs(thePeriod);
  ...
  const Standard_Integer aPar1 = static_cast<Standard_Integer>(std::floor(aVal1));
  const Standard_Integer aPar2 = static_cast<Standard_Integer>(std::floor(aVal2));
  if (aPar1 != aPar2)
                      ...
                      const Standard_Real          thePeriod) const
{
  ...
  Standard_Real aValPrev = theVal + aPeriod * std::ceil((myFirst - theVal) / aPeriod);
