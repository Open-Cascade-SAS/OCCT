...
This package exposes two interface styles:

- Generic vector interface (`MathSys_Newton.hxx`, `MathSys_LevenbergMarquardt.hxx`):
  function sets provide `NbVariables()`, `NbEquations()`, and `Values(X, F, J)`.
- Specialized small-dimension interface (`MathSys_Newton2D.hxx`, `MathSys_Newton3D.hxx`, `MathSys_Newton4D.hxx`):
  callables provide direct function/Jacobian evaluation with scalar arguments.

Return types are solver-specific (`VectorResult`, `Newton2DResult`, `Newton3DResult`, `Newton4DResult`)
and all include convergence `Status`, iteration count, and final residual norm.
