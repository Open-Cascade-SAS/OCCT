...
public:
  ...
  public:
    ...
    explicit Iterator(NCollection_OrderedDataMap& theMap) noexcept
        : iterator(static_cast<OrderedDataMapNode*>(theMap.myFirst)),
          myMap(&theMap)
    {
    }

    //! Legacy cursor construction from a const map.
    explicit Iterator(const NCollection_OrderedDataMap& theMap) noexcept
        : iterator(const_cast<OrderedDataMapNode*>(theMap.myFirst)),
          myMap(const_cast<NCollection_OrderedDataMap*>(&theMap))
    {
    }

    bool More() const noexcept { return !iterator::operator==(iterator()); }
  ...
  OrderedDataMapNode* FirstNode() noexcept { return myFirst; }

  //! Returns the first node in insertion order for the shared iterator state.
  const OrderedDataMapNode* FirstNode() const noexcept { return myFirst; }
...
public:
        ...
        myLast(nullptr)
  {
    ...
    if (anExt <= 0)
      return;
    ReSize(anExt - 1);
    for (const_iterator anIter(theOther); anIter != theOther.cend(); ++anIter)
  ...
  NCollection_OrderedDataMap& Assign(const NCollection_OrderedDataMap& theOther)
  {
    ...
    {
      ReSize(anExt - 1);
      const_iterator anIter(theOther);
      for (; anIter != theOther.cend(); ++anIter)
        Bind(anIter.Key(), *anIter);
    }
    return *this;
  }
  ...
  //! Destructor
  ~NCollection_OrderedDataMap() { Clear(true); }
