...
Standard_Real Quantity_Color::DeltaE2000(const Quantity_Color& theOther) const
{
  ...
  static const double a25_pow7     = std::pow(25., 7);
  Standard_Real       aG       = 0.5 * (1. - std::sqrt(aC_mean_pow7 / (aC_mean_pow7 + a25_pow7)));
  ...
  // mean H
  Standard_Real ah1x = (aC1x > TheEpsilon ? std::atan2(ab1, aa1x) * 180. / M_PI : 270.);
  Standard_Real ah2x = (aC2x > TheEpsilon ? std::atan2(ab2, aa2x) * 180. / M_PI : 270.);
  if (ah1x < 0.)
    ah1x += 360.;
  if (ah2x < 0.)
    ah2x += 360.;
  ...
  Standard_Real aDeltaHx = 2. * std::sqrt(aC1x * aC2x) * std::sin(0.5 * aDeltahx * M_PI / 180.);

  // factors
  Standard_Real aT = 1. - 0.17 * std::cos((aHx_mean - 30.) * M_PI / 180.)
                     + 0.24 * std::cos((2. * aHx_mean) * M_PI / 180.)
                     + 0.32 * std::cos((3. * aHx_mean + 6.) * M_PI / 180.)
                     - 0.20 * std::cos((4. * aHx_mean - 63.) * M_PI / 180.);
  ...
  Standard_Real aR_C          = 2. * std::sqrt(aCx_mean_pow7 / (aCx_mean_pow7 + a25_pow7));
  Standard_Real aR_T          = -aR_C * std::sin(2. * aDelta_theta * M_PI / 180.);
...
{
  return theValue > 0.008856451679035631 ? std::pow(theValue, 1. / 3.)
...
NCollection_Vec3<float> Quantity_Color::Convert_Lab_To_Lch(const NCollection_Vec3<float>& theLab)
{
  ...
  double aH = (aC > TheEpsilon ? std::atan2(ab, aa) * 180. / M_PI : 0.);
