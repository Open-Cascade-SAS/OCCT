                           ...
                           const occ::handle<Storage_Data>&       aData) const
{
  ...
  occ::handle<Storage_HSeqOfRoot>  plist;
  ...
  if ((theDriver->OpenMode() == Storage_VSWrite) || (theDriver->OpenMode() == Storage_VSReadWrite))
  {
    try
    {
      ...
      occ::handle<TColStd_HSequenceOfAsciiString> tlist;
      ...
      occ::handle<Storage_HArrayOfCallBack> WFunc = new Storage_HArrayOfCallBack(1, len);

      theDriver->SetTypeSectionSize(len);

      NCollection_DataMap<TCollection_AsciiString, occ::handle<Storage_TypedCallBack>>::Iterator cbit(iData->myTypeBinding);
...
occ::handle<TColStd_HSequenceOfAsciiString> Storage_Schema::InstalledCallBackList() const
{
  NCollection_DataMap<TCollection_AsciiString, occ::handle<Storage_TypedCallBack>>::Iterator it(myCallBack);
  occ::handle<TColStd_HSequenceOfAsciiString> result = new TColStd_HSequenceOfAsciiString;
