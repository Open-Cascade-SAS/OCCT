                               ...
                               const char*       astring)
{
  ...
  occ::handle<Units_TokensSequence> lstlexicon = alexicon->Sequence();
...
static occ::handle<Units_Token> CalculateLocal(const occ::handle<Units_TokensSequence>& aSeq)
{
  // std::cout<<std::endl;
  ...
  while (IsBracket)
  {
    for (i = 1; i <= aSeq->Length(); i++)
    {
      ...
      {
        occ::handle<Units_TokensSequence> TmpSeq     = new Units_TokensSequence;
        int             NbBrackets = 1;
