...
namespace
{
  static bool convertToColor (const occ::handle<TColStd_HSequenceOfAsciiString>& theColorValues,
  ...
  {
    NCollection_DataMap<TCollection_AsciiString, occ::handle<TColStd_HSequenceOfAsciiString>> aMapOfArgs;
    TCollection_AsciiString aParseKey;
    for (int anArgIt = 1; anArgIt < theArgsNb; ++anArgIt)
    {
      TCollection_AsciiString anArg (theArgVec [anArgIt]);
      if (anArg.Value (1) == '-'
      && !anArg.IsRealValue (true))
      {
        aParseKey = anArg;
        aParseKey.Remove (1);
        aParseKey.LowerCase();
    ...
    occ::handle<TColStd_HSequenceOfAsciiString> aValues;
    ...
    occ::handle<TColStd_HSequenceOfAsciiString> aXValues, aZValues;
  ...
  static bool parseFontStyle (const TCollection_AsciiString& theArg,
                         ...
                         const char**      theArgVec)
{
  ...
  occ::handle<AIS_PlaneTrihedron> anAISPlaneTri = new AIS_PlaneTrihedron (aMkPlane.Value());
                       ...
                       const char** theArgVec)
{
  ...
  occ::handle<AIS_Trihedron> aTrihedron;
  ...
  {
    occ::handle<Geom_Axis2Placement> aPlacement = new Geom_Axis2Placement (gp_Pnt (0.0, 0.0, 0.0),
                                                                      gp::DZ(), gp::DX());
...
static int VSize (Draw_Interpretor& di, int argc, const char** argv)
{
  ...
  NCollection_Sequence<occ::handle<AIS_Trihedron>> aTrihedrons;
  if (!aName.IsEmpty())
  {
    occ::handle<AIS_InteractiveObject> aShape;
    if (GetMapOfAIS().Find2 (aName, aShape))
    {
      if (occ::handle<AIS_Trihedron> aTrihedron = Handle(AIS_Trihedron)::DownCast (aShape))
      {
  ...
  else if (TheAISContext()->NbSelected() > 0)
  {
    ...
    {
      const occ::handle<AIS_InteractiveObject>& aShape = it.Key1();
      if (!aShape.IsNull()
  ...
  for (NCollection_Sequence<occ::handle<AIS_Trihedron>>::Iterator anObjIter (aTrihedrons); anObjIter.More(); anObjIter.Next())
  {
    const occ::handle<AIS_Trihedron>& aTrihedron = anObjIter.Value();
    ...
    TheAISContext()->Redisplay (aTrihedron, false);
  }
...

{
  ...
  occ::handle<AIS_InteractiveObject> aTest = TheAISContext()->SelectedInteractive();
  occ::handle<AIS_Plane> aPlane = Handle(AIS_Plane)::DownCast (aTest);
  if (aPlane.IsNull())
  {
...
static int VAxisBuilder(Draw_Interpretor& di, int argc, const char** argv)
{
  ...
  if (HasArg) {
    ...
    gp_Vec myVect (p1,p2);
    occ::handle<Geom_Line> myLine=new Geom_Line (p1 ,myVect );
    occ::handle<AIS_Axis> TheAxis=new AIS_Axis (myLine );
  ...
  // Pas d'arguments
    ...
    if ( !strcasecmp(argv[0], "vaxis")) {
      ...
      if (aShapeA.ShapeType() == TopAbs_VERTEX)
      {
        ...
        gp_Vec V (A,B);
        gp_Dir D (V);
        occ::handle<Geom_Axis1Placement> OrigineAndVect=new Geom_Axis1Placement (A,D);
        occ::handle<AIS_Axis> TheAxis=new AIS_Axis (OrigineAndVect);
        GetMapOfAIS().Bind (TheAxis,name);
        TheAISContext()->Display (TheAxis, true);
      }
      else
    ...
    else if ( !strcasecmp(argv[0], "vaxispara"))
    {
      ...
      gp_Vec  V (A,C);
      gp_Dir   D (V);
      occ::handle<Geom_Axis1Placement> OrigineAndVect=new Geom_Axis1Placement (B,D);
      occ::handle<AIS_Axis> TheAxis=new AIS_Axis (OrigineAndVect);
      GetMapOfAIS().Bind (TheAxis,name);
      TheAISContext()->Display (TheAxis, true);
                         ...
                         const char** theArgVec)
{
  ...
  occ::handle<AIS_Point> aPointPrs = new AIS_Point (aGeomPoint);
  if (is2d)
  {
...
static int VPlaneBuilder (Draw_Interpretor& /*di*/,
                                       int argc,
                                       const char** argv)
{
  // Declarations
  ...
  if (hasArg)
  {
      ...
      && aShapeA->Signature() == 1)
    {
      ...
      occ::handle<AIS_Point> anAISPointA = Handle(AIS_Point)::DownCast (aShapeA);
      occ::handle<AIS_Point> anAISPointB = Handle(AIS_Point)::DownCast (aShapeB);
      occ::handle<AIS_Point> anAISPointC = Handle(AIS_Point)::DownCast (aShapeC);
          ...
          && aShapeA->Signature() == 2)
    {
      ...
      occ::handle<Geom_CartesianPoint> aCartPointB = occ::down_cast<Geom_CartesianPoint>(aGeomPointB);
          ...
          && aShapeA->Signature() == 7)
    {
      ...
      occ::handle<Geom_Plane> aNewGeomPlane= anAISPlaneA->Component();
      occ::handle<Geom_Point> aGeomPointB = anAISPointB->Component();
  ...
  else 
  {
    ...
    else if (!strcasecmp(argv[0], "vplanepara"))
    {
      ...
      if (aSurface.GetType() == GeomAbs_Plane)
      {
        ...
        TheAISContext()->Display (aAISPlane, true);
      }
      else
      {
...
static int VChangePlane (Draw_Interpretor& /*theDi*/, int theArgsNb, const char** theArgVec)
{
  ...
  occ::handle<AIS_Plane> aPlane = GetMapOfAIS().IsBound2(aName)
  ...
  double aCenterX = aPlane->Center().X();
  double aCenterY = aPlane->Center().Y();
  double aCenterZ = aPlane->Center().Z();

  double aDirX = aPlane->Component()->Axis().Direction().X();
  double aDirY = aPlane->Component()->Axis().Direction().Y();
  double aDirZ = aPlane->Component()->Axis().Direction().Z();

  double aSizeX = 0.0;
  double aSizeY = 0.0;
  bool aHasMinSize = aPlane->HasMinimumSize();
  double aMinSizeY = 0.0;
...
static int VLineBuilder(Draw_Interpretor& di, int argc, const char** argv)
{
  if (argc == 4)   // parameters: AIS_Point AIS_Point
  {
    ...
    occ::handle<Geom_Point> aGeomPointBA = anAISPointA->Component();
    occ::handle<Geom_CartesianPoint> aCartPointA = Handle(Geom_CartesianPoint)::DownCast (aGeomPointBA);

    occ::handle<Geom_Point> aGeomPointB = anAISPointB->Component();
    occ::handle<Geom_CartesianPoint> aCartPointB = Handle(Geom_CartesianPoint)::DownCast (aGeomPointB);
    ...
    occ::handle<AIS_Line> anAISLine = new AIS_Line (aCartPointA, aCartPointB);
  ...
  else if (argc == 8) // parametres 6 reals
  {
    ...
    occ::handle<Geom_CartesianPoint> aCartPointA = new Geom_CartesianPoint (aCoord[0], aCoord[1], aCoord[2]);
    occ::handle<Geom_CartesianPoint> aCartPointB = new Geom_CartesianPoint (aCoord[3], aCoord[4], aCoord[5]);
  ...
  else if (argc == 2) // selection in 3D viewer
  {
    ...
    occ::handle<Geom_CartesianPoint> aCartPointA = new Geom_CartesianPoint (A);
    occ::handle<Geom_CartesianPoint> aCartPointB = new Geom_CartesianPoint (B);
...
FilledCircle::FilledCircle (const occ::handle<Geom_Circle>& theCircle,
                            const double theUStart,
                            const double theUEnd)
: myCircle (theCircle),
  myUStart (theUStart),
...
void FilledCircle::Compute (const occ::handle<PrsMgr_PresentationManager>& ,
                            const occ::handle<Prs3d_Presentation>& thePrs,
                            const int theMode)
{
  thePrs->Clear();
...
void DisplayCircle (const occ::handle<Geom_Circle>& theGeomCircle,
                    ...
                    const double theUEnd)
{
  occ::handle<AIS_InteractiveObject> aCircle;
  if (isFilled)
  {
  ...
  if (GetMapOfAIS().IsBound2(theName))
  {
    ...
    TheAISContext()->Remove (anInterObj, false);
...
static int VCircleBuilder (Draw_Interpretor& /*di*/, int argc, const char** argv)
{
  ...
  double anUStart = 0, anUEnd = M_PI * 2.0;
  if (argc == 8)
  {
  ...
  if (argc == 6 || argc == 8)
  {
    ...
    bool isFilled = Draw::Atoi(argv[5]) != 0;
     ...
     &&  aShapeA->Signature() == 1)
    {
      ...
      occ::handle<Geom_CartesianPoint> aCartPointA = Handle(Geom_CartesianPoint)::DownCast (anAISPointA->Component());
      occ::handle<Geom_CartesianPoint> aCartPointB = Handle(Geom_CartesianPoint)::DownCast (anAISPointB->Component());
      occ::handle<Geom_CartesianPoint> aCartPointC = Handle(Geom_CartesianPoint)::DownCast (anAISPointC->Component());
      // Test A=B
    ...
    // Arguments: AIS_Plane AIS_Point Real
          ...
          && aShapeA->Signature() == 7)
    {
      occ::handle<AIS_Plane> anAISPlane  = Handle(AIS_Plane)::DownCast (aShapeA);
      occ::handle<AIS_Point> anAISPointB = Handle(AIS_Point)::DownCast (aShapeB);
                      ...
                      const char**      theArgVec)
{
  ...
  int           anArgIt = 1;
  TCollection_ExtendedString aName (theArgVec[anArgIt++], true);
  TCollection_ExtendedString aText (theArgVec[anArgIt++], true);
  occ::handle<AIS_TextLabel>      aTextPrs;
  ...
  occ::handle<Font_TextFormatter> aTextFormatter;
  for (; anArgIt < theArgsNb; ++anArgIt)
  {
    ...
    {
      Quantity_Color aColor;
      int aNbParsed = Draw::ParseColor (theArgsNb - anArgIt - 1,
                                                     theArgVec + anArgIt + 1,
                                                     aColor);
      if (aNbParsed == 0)
      {
          ...
          && anArgIt + 1 < theArgsNb)
    {
      ...
      aTextFormatter->SetWrapping ((float)Draw::Atof(theArgVec[++anArgIt]));
    }
  ...
  {
    aContext->SetTransformPersistence (aTextPrs, occ::handle<Graphic3d_TransformPers>());
  }

  if (isNewPrs)
  {
    ViewerTest::Display (aName, aTextPrs, false);
  }
  else
  {
    aContext->Redisplay (aTextPrs, false, true);
  }
  return 0;
}
...
Handle( Poly_Triangulation ) CalculationOfSphere( double X , double Y , double Z ,
  ...
  occ::handle<Poly_Triangulation> polyTriangulation = new Poly_Triangulation (number_pointArray, number_triangle, false, true);
...
static int VDrawSphere (Draw_Interpretor& /*di*/, int argc, const char** argv)
{
  ...
  double aCenterY = (argc > 5) ? Draw::Atof (argv[4]) : 0.0;
  double aCenterZ = (argc > 5) ? Draw::Atof (argv[5]) : 0.0;
  double aRadius =  (argc > 6) ? Draw::Atof (argv[6]) : 100.0;
  ...
  occ::handle<TColStd_HArray1OfInteger> aColorArray = new TColStd_HArray1OfInteger (1, aNumberPoints);
  for (int aNodeId = 1; aNodeId <= aNumberPoints; ++aNodeId)
  {
    aColorArray->SetValue (aNodeId, *reinterpret_cast<const int*> (aColor.GetData()));
  }
  ...
  occ::handle<Graphic3d_AspectFillArea3d> anAspect
    = new Graphic3d_AspectFillArea3d (Aspect_IS_SOLID,
                                      ...
                                      aMat,
                                      aMat);
  occ::handle<Prs3d_ShadingAspect> aShAsp = new Prs3d_ShadingAspect();
                        ...
                        const char** theArgVec)
{
  ...
  if (occ::handle<V3d_Viewer> aViewer = ViewerTest::GetViewerFromContext())
  {
    ...
    int aWidth, aHeight;
    double aCentX, aCentY, aCentZ, aDirX, aDirY, aDirZ;
    double aRightX, aRightY, aRightZ;
  ...
  {
    occ::handle<HLRBRep_Algo> aHlrAlgo = new HLRBRep_Algo();
    ...
    // extract 3d
    //aVisible[HLRBRep_Sharp]   = aHLRToShape.CompoundOfEdges (HLRBRep_Sharp, true, true);
  ...
  if (!ViewerTest::GetAISContext().IsNull())
  {
    ...
    if (toShowHiddenEdges)
    {
      occ::handle<Prs3d_LineAspect> aLineAspect = new Prs3d_LineAspect (Quantity_Color (Quantity_NOC_RED), Aspect_TOL_DASH, 1.0f);
      for (int aTypeIter = 0; aTypeIter < 6; ++aTypeIter)
      {
        if (!aHidden[aTypeIter].IsNull())
        {
          occ::handle<AIS_ColoredDrawer> aDrawer = anObject->CustomAspects (aHidden[aTypeIter]);
...
public:
  ...
  MyPArrayObject (const occ::handle<Graphic3d_ArrayOfPrimitives>& thePArray) : myPArray (thePArray) {}

  MyPArrayObject (Graphic3d_TypeOfPrimitiveArray thePrimType,
                  const occ::handle<TColStd_HArray1OfAsciiString>& theDesc,
...
private:
  ...
  bool CheckInputCommand (const TCollection_AsciiString& theCommand,
                          const occ::handle<TColStd_HArray1OfAsciiString>& theArgsArray,
                          int &theArgIndex,
                          int theArgCount,
                          int theMaxArgs);
...
protected:
  ...
  occ::handle<Graphic3d_AspectMarker3d> myMarkerAspect;
  occ::handle<Graphic3d_ArrayOfPrimitives> myPArray;

};

void MyPArrayObject::Compute (const occ::handle<PrsMgr_PresentationManager>& ,
                              const occ::handle<Prs3d_Presentation>& thePrs,
                              const int theMode)
{
...
bool MyPArrayObject::Init (Graphic3d_TypeOfPrimitiveArray thePrimType,
                                       const occ::handle<TColStd_HArray1OfAsciiString>& theDesc,
                                       const occ::handle<Graphic3d_AspectMarker3d>& theMarkerAspect,
                                       bool theToPatch)
{
  ...
  const int anArgsCount = theDesc->Length();
  TCollection_AsciiString aCommand;
  for (int anArgIndex = theDesc->Lower(); anArgIndex <= theDesc->Upper(); )
  {
  ...
  else
  {
    ...
    if (occ::handle<Graphic3d_AttribBuffer> anAttribs = Handle(Graphic3d_AttribBuffer)::DownCast (myPArray->Attributes()))
    {
...
void MyPArrayObject::ComputeSelection (const occ::handle<SelectMgr_Selection>& theSelection,
                                       const int theMode)
{
  if (theMode != 0
   || myPArray.IsNull())
  ...
  if (occ::handle<Graphic3d_ArrayOfTriangles> aTris = Handle(Graphic3d_ArrayOfTriangles)::DownCast (myPArray))
  {
    occ::handle<Select3D_SensitivePrimitiveArray> aSensitive = new Select3D_SensitivePrimitiveArray (anOwner);
    aSensitive->InitTriangulation (myPArray->Attributes(), myPArray->Indices(), TopLoc_Location(), true);
    theSelection->Add (aSensitive);
  }
  else if (occ::handle<Graphic3d_ArrayOfSegments> aSegs = Handle(Graphic3d_ArrayOfSegments)::DownCast (myPArray))
  {
    if (aSegs->EdgeNumber() > 0)
    {
      for (int aPntIter = 1; aPntIter <= aSegs->EdgeNumber(); aPntIter += 2)
      {
        occ::handle<Select3D_SensitiveSegment> aSeg = new Select3D_SensitiveSegment (anOwner, aSegs->Vertice (aSegs->Edge (aPntIter)), aSegs->Vertice (aSegs->Edge (aPntIter + 1)));
    ...
    {
      for (int aPntIter = 1; aPntIter <= aSegs->VertexNumber(); aPntIter += 2)
      {
        occ::handle<Select3D_SensitiveSegment> aSeg = new Select3D_SensitiveSegment (anOwner, aSegs->Vertice (aPntIter), aSegs->Vertice (aPntIter + 1));
        aSeg->SetSensitivityFactor (4);
        theSelection->Add (aSeg);
      }
    }
...
static int VDrawPArray (Draw_Interpretor& di, int argc, const char** argv)
{
  ...
  occ::handle<MyPArrayObject> aPObject;
  if (anArrayType == "-shape")
  {
    const char* aShapeName = argv[anArgIndex++];
    TopoDS_Shape aShape = DBRep::Get (aShapeName);
    occ::handle<Graphic3d_ArrayOfPrimitives> aTris = StdPrs_ShadedShape::FillTriangles (aShape);
    if (aShape.IsNull())
    {
  ...
  int aLowerArg = anArgIndex;
  occ::handle<TColStd_HArray1OfAsciiString> anArgsArray = new TColStd_HArray1OfAsciiString (0, argc - 3);
  ...
  {
    aPObject->Init (aPrimType, anArgsArray, anAspPoints, true);
...
namespace
{
  ...
  static int parseTranslationVec (int theArgNb,
                                               const char**     theArgVec,
                                               gp_Vec&          theVec)
  {
    if (theArgNb < 2)
    ...
    if (!anX.IsRealValue (true)
     || !anY.IsRealValue (true))
    {
      return 0;
    }
...
static int VSetLocation (Draw_Interpretor& theDI,
                                      int  theArgNb,
                                      const char**      theArgVec)
{
  occ::handle<AIS_InteractiveContext> aContext = ViewerTest::GetAISContext();
  ...
  for (int anArgIter = 1; anArgIter < theArgNb; ++anArgIter)
  {
          ...
          || anArg == "-copy")
    {
      ...
      occ::handle<AIS_InteractiveObject> anObj2;
          ...
          || anArg == "-pretranslate")
    {
      toPrintInfo = false;
      gp_Vec aLocVec;
      int aNbParsed = parseTranslationVec (theArgNb - anArgIter - 1, theArgVec + anArgIter + 1, aLocVec);
      anArgIter += aNbParsed;
      if (aNbParsed == 0)
      {
        Message::SendFail() << "Syntax error at '" << anArg << "'";
          ...
          || anArg == "-setscale")
    {
      ...
      gp_XYZ aScaleLoc;
      double aScale = 1.0;
      bool toPrintScale = true;
      bool hasScaleLoc  = false;
      if (anArgIter + 4 < theArgNb)
      {
        ...
        {
          if (!aScaleArgs[aScaleArgIter].IsRealValue (true))
          {
            break;
          }
        }
          ...
          || anArg == "-rotation")
    {
      ...
      if (anArgIter + 4 < theArgNb)
      {
        ...
        {
          if (!aQuatArgs[aQuatArgIter].IsRealValue (true))
          {
            break;
          }
        }
...
static occ::handle<AIS_InteractiveObject> findConnectedObject (const TCollection_AsciiString& theName)
{
  occ::handle<AIS_InteractiveObject> aPrs;
  if (!GetMapOfAIS().Find2 (theName, aPrs))
  {
    return occ::handle<AIS_InteractiveObject>();
  }
  if (occ::handle<AIS_ConnectedInteractive> aConnected = Handle(AIS_ConnectedInteractive)::DownCast (aPrs))
  {
    return aConnected;
  }
  else if (occ::handle<AIS_MultipleConnectedInteractive> aMultiCon = Handle(AIS_MultipleConnectedInteractive)::DownCast (aPrs))
  {
    return aMultiCon;
  }
...
//===============================================================================================
static int VConnect (Draw_Interpretor& /*di*/,
                                  int argc,
                                  const char ** argv)
{
  occ::handle<AIS_InteractiveContext> aContext = ViewerTest::GetAISContext();
  ...
  const int aNbShapes = hasColor ? (argc - 1) : argc;
  for (int i = 5; i < aNbShapes; ++i)
  {
    ...
    if (anObject.IsNull())
    {
      ...
      occ::handle<AIS_Shape> aShapePrs = new AIS_Shape (aTDShape);
      occ::handle<AIS_ConnectedInteractive> aConnectedOrig = new AIS_ConnectedInteractive();
...
//purpose  : Creates and displays AIS_ConnectedInteractive object from input object and location 
//===============================================================================================
static int VConnectTo (Draw_Interpretor& /*di*/,
                                    int argc,
                                    const char ** argv)
{
  occ::handle<AIS_InteractiveContext> aContext = ViewerTest::GetAISContext();
  ...
  occ::handle<AIS_InteractiveObject> anOriginObject;
...
//purpose  :
//=======================================================================
static int VDisconnect (Draw_Interpretor& di,
                                     int argc,
                                     const char ** argv)
{
  occ::handle<AIS_InteractiveContext> aContext = ViewerTest::GetAISContext();
...
//purpose  :
//=======================================================================
static int VAddConnected (Draw_Interpretor& ,
                                       int argc,
                                       const char ** argv)
{
  occ::handle<AIS_InteractiveContext> aContext = ViewerTest::GetAISContext();
  ...
  const double aX = Draw::Atof (argv[2]);
  const double aY = Draw::Atof (argv[3]);
  ...
  occ::handle<AIS_InteractiveObject> anIObj = findConnectedObject (anObjectName);
  if (anIObj.IsNull())
  {
  ...
  TheAISContext()->Display (anAssembly, false);
...
//purpose  :
//=======================================================================
static int VListConnected (Draw_Interpretor& /*di*/,
                                        int argc,
                                        const char ** argv)
{
  occ::handle<AIS_InteractiveContext> aContext = ViewerTest::GetAISContext();
  ...
  for (PrsMgr_ListOfPresentableObjectsIter anIter (anAssembly->Children()); anIter.More(); anIter.Next())
  {
    occ::handle<AIS_InteractiveObject> anObj = Handle(AIS_InteractiveObject)::DownCast (anIter.Value());
    ...
    occ::handle<AIS_ConnectedInteractive> aConnected = Handle(AIS_ConnectedInteractive)::DownCast (anIter.Value());
...
//purpose  :
//=======================================================================
static int VChild (Draw_Interpretor& ,
                                int theNbArgs,
                                const char** theArgVec)
{
  occ::handle<AIS_InteractiveContext> aContext = ViewerTest::GetAISContext();
  ...
  int toAdd = -1;
  occ::handle<AIS_InteractiveObject> aParent;
...
//purpose  :
//=======================================================================
static int VParent(Draw_Interpretor&,
  int theNbArgs,
  const char** theArgVec)
{
  occ::handle<AIS_InteractiveContext> aContext = ViewerTest::GetAISContext();
...
//purpose  : vselmode
//===============================================================================================
static int VSetSelectionMode (Draw_Interpretor& /*di*/,
                                           int  theNbArgs,
                                           const char**      theArgv)
{
  // Check errors
  ...
  {
    const occ::handle<AIS_InteractiveObject>& anIO = aTargetIt.Value();
...
//===========================================================================
static int VTriangle (Draw_Interpretor& /*di*/,
                                   int argc,
                                   const char ** argv)
{
  ...
  gp_Pnt aPnts[3];
  for (int aPntIter = 0; aPntIter < aPrims->VertexNumberAllocated(); ++aPntIter)
  {
    const TCollection_AsciiString aName (argv[2 + aPntIter]);
    if (occ::handle<AIS_Point> aPntPrs = Handle(AIS_Point)::DownCast (GetMapOfAIS().IsBound2 (aName) ? GetMapOfAIS().Find2 (aName) : NULL))
    {
    ...
    for (int aPnt2Iter = 0; aPnt2Iter < aPntIter; ++aPnt2Iter)
    {
  ...
  occ::handle<AIS_InteractiveObject> aPrs = new MyPArrayObject (aPrims);
  if (!isTri)
  {
...
//===========================================================================
static int VTorus (Draw_Interpretor& /*di*/,
                                int theNbArgs,
                                const char** theArgVec)
{
  if (ViewerTest::GetAISContext().IsNull())
  ...
  TCollection_AsciiString aName;
  int aNbSlices = 100, aNbStacks = 100;
  double aMajorRad = -1.0, aMinorRad = -1.0;
  double aPipeAngle = 360.0, aSegAngle1 = 0.0, aSegAngle2 = 360.0;
  double anAngles[3] = { -1.0, -1.0, -1.0 };
  ...
  occ::handle<Graphic3d_ArrayOfTriangles> aTriangles = Prs3d_ToolTorus::Create (aMajorRad, aMinorRad, aSegAngle1, aSegAngle2, aPipeAngle, aNbSlices, aNbStacks, gp_Trsf());
  occ::handle<AIS_InteractiveObject> anIO = new MyPArrayObject (aTriangles);
...
//===========================================================================
static int VCylinder (Draw_Interpretor& /*di*/,
                                   int theNbArgs,
                                   const char** theArgVec)
{
  if (ViewerTest::GetAISContext().IsNull())
  ...
  TCollection_AsciiString aName;
  int aNbSlices = 100, aNbStacks = 1;
  double aBotRad = -1.0, aTopRad = -1.0, aHeight = -1.0;
  ViewerTest_AutoUpdater anUpdateTool (ViewerTest::GetAISContext(), ViewerTest::CurrentView());
  for (int anArgIter = 1; anArgIter < theNbArgs; ++anArgIter)
  {
    TCollection_AsciiString anArg (theArgVec[anArgIter]);
  ...
  occ::handle<Graphic3d_ArrayOfTriangles> aTriangles = Prs3d_ToolCylinder::Create (aBotRad, aTopRad, aHeight, aNbSlices, aNbStacks, gp_Trsf());
  occ::handle<AIS_InteractiveObject> anIO = new MyPArrayObject (aTriangles);
  ViewerTest::Display (aName, anIO, false);
  return 0;
}
...
//===========================================================================
static int VSphere (Draw_Interpretor& /*di*/,
                                 int theNbArgs,
                                 const char** theArgVec)
{
  if (ViewerTest::GetAISContext().IsNull())
  ...
  occ::handle<Graphic3d_ArrayOfTriangles> aTriangles = Prs3d_ToolSphere::Create (aRad, aNbSlices, aNbStacks, gp_Trsf());
  occ::handle<AIS_InteractiveObject> anIO = new MyPArrayObject (aTriangles);
  ViewerTest::Display (aName, anIO, false);
  return 0;
}
...
static int VObjZLayer (Draw_Interpretor& di,
                                    int argc,
                                    const char ** argv)
{
  occ::handle<AIS_InteractiveContext> aContext = ViewerTest::GetAISContext();
...
//=======================================================================
static int VPolygonOffset(Draw_Interpretor& /*di*/,
                                       int argc,
                                       const char ** argv)
{
  occ::handle<AIS_InteractiveContext> aContext = ViewerTest::GetAISContext();
  ...
  if (argc == 5)
  {
    ...
    aFactor = (float) Draw::Atof(argv[3]);
    aUnits  = (float) Draw::Atof(argv[4]);
...
void ViewerTest_MarkersArrayObject::Compute (const occ::handle<PrsMgr_PresentationManager>& ,
                                             const occ::handle<Prs3d_Presentation>& aPresentation,
                                             const int )
{
  occ::handle<Graphic3d_ArrayOfPrimitives> anArray = new Graphic3d_ArrayOfPoints ((int )Pow (myPointsOnSide, 3), myPointsOnSide != 1);
  if (myPointsOnSide == 1)
  {
  ...
  {
    for (double i = 1; i <= myPointsOnSide; i++)
    {
      for (double j = 1; j <= myPointsOnSide; j++)
      {
        for (double k = 1; k <= myPointsOnSide; k++)
        {
...
void ViewerTest_MarkersArrayObject::ComputeSelection (const occ::handle<SelectMgr_Selection>& theSelection,
                                       const int /*theMode*/)
{
  occ::handle<SelectMgr_EntityOwner> anEntityOwner = new SelectMgr_EntityOwner (this);
  ...
  if (myPointsOnSide == 1)
  {
    ...
    occ::handle<Select3D_SensitivePoint> aSensetivePoint = new Select3D_SensitivePoint (anEntityOwner, aPoint);
...
//=======================================================================
static int VMarkersTest (Draw_Interpretor&,
                                      int  theArgNb,
                                      const char**      theArgVec)
{
  occ::handle<AIS_InteractiveContext> aContext = ViewerTest::GetAISContext();
  ...
  occ::handle<ViewerTest_MarkersArrayObject> aMarkersArray = new ViewerTest_MarkersArrayObject (aPnt, aPointsOnSide, anAspect);
                       ...
                       const char**      theArgVec)
{
  ...
  for (; anArgIt < theArgNb; ++anArgIt)
  {
    ...
    else if (aParam == "-plane")
    {
      ...
      double aX = Draw::Atof (theArgVec[++anArgIt]);
      double aY = Draw::Atof (theArgVec[++anArgIt]);
      double aZ = Draw::Atof (theArgVec[++anArgIt]);
...
{
  bool operator() (const occ::handle<Font_SystemFont>& theFontA,
                   const occ::handle<Font_SystemFont>& theFontB)
  {
  ...
  for (int anArgIter = 1; anArgIter < theArgNb; ++anArgIter)
  {
           ...
           || anArgCase == "find"))
    {
       ...
       || aFontName.Search ("*") != -1)
      {
        ...
        for (std::vector<occ::handle<Font_SystemFont>>::iterator aFontIter = aFontsSorted.begin(); aFontIter != aFontsSorted.end(); ++aFontIter)
        {
      ...
      else if (occ::handle<Font_SystemFont> aFont = aMgr->FindFont (aFontName, aStrictLevel, aFontAspect))
      {
        aResult = toPrintInfo ? aFont->ToString() : aFont->FontName();
      }
                         ...
                         const char**      theArgs)
{
  ...
  if (aParam == "-set")
  {
    ...
    occ::handle<AIS_InteractiveObject> anObject;
    for (int aCount = 3; aCount < theArgNum; aCount++)
    {
    ...
    for (AIS_ListIteratorOfListOfInteractive anIt (anObjs); anIt.More(); anIt.Next())
    {
      ...
      aContext->Redisplay (anObject, false);
      aRedrawNeeded = true;
    }
...
//=======================================================================
static int VPointCloud (Draw_Interpretor& theDI,
                                     int  theArgNum,
                                     const char**      theArgs)
{
  if (theArgNum < 2)
  ...
  if (!aShape.IsNull())
  {
    ...
    {
    public:
      PointCloudPntFiller (double theTol) : BRepLib_PointCloudShape (TopoDS_Shape(), theTol) {}
      void SetPointArray (const occ::handle<Graphic3d_ArrayOfPoints>& thePoints) { myPoints = thePoints; }
    ...
    protected:
                             ...
                             const TopoDS_Shape& ) override
      {
        ...
        {
          Quantity_Color aColor (360.0 * double(aPntIndex) / double(myPoints->VertexNumberAllocated()),
  ...
  else if (!aDistribution.IsEmpty())
  {
    ...
    for (int aPntIt = 0; aPntIt < aDistNbPoints; ++aPntIt)
    {
      double anAlpha   = anAlphaDistrib(aRandomGenerator);
      double aBeta     = aBetaDistrib  (aRandomGenerator);
      double aDistance = isSurface ? aDistRadius : aRadiusDistrib (aRandomGenerator);
      ...
      const int anIndexOfPoint = anArrayPoints->AddVertex (aPoint);
      if (toRandColors)
      {
        Quantity_Color aColor (360.0 * double (anIndexOfPoint) / double (aDistNbPoints),
                               1.0, 0.5, Quantity_TOC_HLS);
                      ...
                      const char**      theArgs)
{
  ...
  int aPriority = Graphic3d_DisplayPriority_INVALID;
  int aNbArgs   = theArgNum;
...
protected:
                        ...
                        const int theMode) override
  {
    ...
    occ::handle<Graphic3d_Group> aPrsGroup = thePrs->NewGroup();
         ...
         aFaceIt.More(); aFaceIt.Next())
    {
      ...
      occ::handle<Graphic3d_ArrayOfSegments> aSegments = new Graphic3d_ArrayOfSegments (2 * aFaceIt.Value().Size());
                     ...
                     const char**      theArgs)
{
  ...
  int anArgIter = 1;
  const char* aShapeName = theArgs[anArgIter++];
  TopoDS_Shape     aShape     = DBRep::Get (aShapeName);
  bool isOn = true;
  if (aShape.IsNull())
  {
  ...
  ViewerTest_DoubleMapOfInteractiveAndName& aMap = GetMapOfAIS();
  occ::handle<MyShapeWithNormals> aShapePrs;
