...
static int VDumpJson(Draw_Interpretor& theDI, Standard_Integer theNbArgs, const char** theArgVec)
{
  const Handle(AIS_InteractiveContext)& aCtx = ViewerTest::GetAISContext();
  if (aCtx.IsNull())
  {
  ...
  for (Standard_Integer anArgIter = 1; anArgIter < theNbArgs; ++anArgIter)
  {
    ...
    if (anArg == "-depth")
    {
      if (anArgIter + 1 >= theNbArgs || !Draw::ParseInteger(theArgVec[anArgIter + 1], aDumpDepth)
          || aDumpDepth < -1)
      {
    ...
    else
    {
      ...
      {
        Message::SendFail() << "Syntax error: '" << theArgVec[anArgIter]
                            << "' is not bound to some object.";
        return 1;
      }
