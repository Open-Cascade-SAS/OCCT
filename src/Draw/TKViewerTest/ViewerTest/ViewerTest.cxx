...
//purpose  :
//=======================================================================
bool ViewerTest::ParseOnOff (const char*  theArg,
                                         bool& theIsOn)
{
...
//purpose  :
//=======================================================================
bool ViewerTest::ParseLineType (const char* theArg,
...
//purpose  :
//=======================================================================
bool ViewerTest::ParseShadingModel (const char*              theArg,
...
occ::handle<ViewerTest_EventManager> ViewerTest::CurrentEventManager()
{
  return !theEventMgrs.IsEmpty()
        ? theEventMgrs.First()
        : occ::handle<ViewerTest_EventManager>();
}
...
void ViewerTest::Clear()
{
  ...
  {
    for (NCollection_Sequence<occ::handle<AIS_InteractiveObject>>::Iterator anObjIter (aListRemoved); anObjIter.More(); anObjIter.Next())
    {
...
//==============================================================================
static occ::handle<Prs3d_IsoAspect> CopyIsoAspect
      (const occ::handle<Prs3d_IsoAspect> &theIsoAspect,
       const int theNbIsos)
{
  ...
  occ::handle<Prs3d_IsoAspect> aResult =
...
static int VDispSensi (Draw_Interpretor& ,
                                    int  theArgNb,
                                    const char** )
{
  if (theArgNb > 1)
...
static int VClearSensi (Draw_Interpretor& ,
                                     int  theArgNb,
                                     const char** )
{
  if (theArgNb > 1)
...
//==============================================================================
static int VDump (Draw_Interpretor& theDI,
                               int  theArgNb,
                               const char** theArgVec)
{
...
static int VDispMode (Draw_Interpretor& , int argc, const char** argv)
{
  ...
  if (aType == TypeOfDispOperation_UnsetDispMode)
  {
    if (argc == 1)
    {
      ...
      {
        VwrTst_DispErase (occ::handle<AIS_InteractiveObject>(), -1, TypeOfDispOperation_UnsetDispMode, false);
      }
      else
      {
        for (aCtx->InitSelected(); aCtx->MoreSelected(); aCtx->NextSelected())
        {
          VwrTst_DispErase (aCtx->SelectedInteractive(), -1, TypeOfDispOperation_UnsetDispMode, false);
        }
      }
  ...
  else if (argc == 2)
  {
    ...
    {
      VwrTst_DispErase (occ::handle<AIS_InteractiveObject>(), aDispMode, TypeOfDispOperation_SetDispMode, true);
    }
    for (aCtx->InitSelected(); aCtx->MoreSelected(); aCtx->NextSelected())
    {
...
public:
  ...
  const occ::handle<AIS_InteractiveObject>& Current() const
  {
    return myCurrent;
  }

  const occ::handle<Standard_Transient>& CurrentTrs() const
  {
    return myCurrentTrs;
  }
  ...
  bool Apply (const occ::handle<Prs3d_Drawer>& theDrawer)
  {
    ...
    if (ToSetFreeBoundaryWidth != 0)
    {
       ...
       || theDrawer->HasOwnFreeBoundaryAspect())
      {
        ...
        {
          occ::handle<Prs3d_LineAspect> aBoundaryAspect = new Prs3d_LineAspect (Quantity_NOC_RED, Aspect_TOL_SOLID, 1.0);
    ...
    if (ToSetTypeOfMarker != 0)
    {
       ...
       || theDrawer->HasOwnPointAspect())
      {
        ...
        theDrawer->PointAspect()->Aspect()->SetMarkerImage (MarkerImage.IsNull() ? occ::handle<Graphic3d_MarkerImage>() : new Graphic3d_MarkerImage (MarkerImage));
      }
    }
...
//purpose  :
//==============================================================================
static int VAspects (Draw_Interpretor& theDI,
                                  int  theArgNb,
                                  const char**      theArgVec)
{
  ...
  else if (aCmdName == "vshowfaceboundary")
  {
     ...
     && aNames.Value (2).IsIntegerValue())
    {
      ...
      if (aNames.Size() == 6
       && aNames.Value (6).IsRealValue (true))
      {
  ...
  for (ViewTest_PrsIter aPrsIter (aNames); aPrsIter.More(); aPrsIter.Next())
  {
    ...
    {
      occ::handle<AIS_Shape> aShapePrs = Handle(AIS_Shape)::DownCast (aPrs);
      if (aShapePrs.IsNull())
      {
    ...
    if (!aPrs.IsNull())
    {
      ...
      {
        occ::handle<AIS_ColoredDrawer> aColDrawer = aColoredPrs->CustomAspects (aColoredPrs->Shape());
            ...
            const char**      theArgVec)
{
  ...
  {
    // Erase named objects
    NCollection_IndexedDataMap<occ::handle<AIS_InteractiveObject>, TCollection_AsciiString> aPrsList;
    ...
    for (NCollection_IndexedDataMap<occ::handle<AIS_InteractiveObject>, TCollection_AsciiString>::Iterator anIter (aPrsList); anIter.More(); anIter.Next())
    {
      theDI << anIter.Value() << " ";
      if (toEraseInView)
      {
...
int VTexture (Draw_Interpretor& theDi, int theArgsNb, const char** theArgVec)
{
  ...
  for (int anArgIter = 1; anArgIter < theArgsNb; ++anArgIter)
  {
           ...
           || aCommandName == "vtexscale"))
    {
      ...
      if (anArgIter < theArgsNb)
      {
        ...
        else if (anArgIter + 1 < theArgsNb)
        {
          ...
          if (aValU.IsRealValue (true)
           && aValV.IsRealValue (true))
          {
           ...
           && anArgIter + 1 < theArgsNb)
    {
      ...
      occ::handle<Graphic3d_MediaTextureSet> aMedia = Handle(Graphic3d_MediaTextureSet)::DownCast (aTextureSetOld);
      if (aMedia.IsNull())
      {
      ...
      for (int aTexIter = 0; aTexIter < aTextureSetNew->Size(); ++aTexIter)
      {
         ...
         && !aTextureNew.IsNull())
        {
          *aTextureNew->GetParams() = *aTextureOld->GetParams();
          ...
          occ::handle<Graphic3d_Texture2D> aTex2dNew = Handle(Graphic3d_Texture2D)::DownCast (aTextureNew);
          occ::handle<Graphic3d_Texture2D> aTex2dOld = Handle(Graphic3d_Texture2D)::DownCast (aTextureOld);
          if (!aTex2dOld.IsNull()
           && !aTex2dNew.IsNull())
          {
   ...
   && theArgsNb == 2)
  {
    if (!aTextureSetOld.IsNull())
    {
      ...
      aTexturedIO->Attributes()->ShadingAspect()->Aspect()->SetTextureSet (occ::handle<Graphic3d_TextureSet>());
      aTextureSetOld.Nullify();
    }
  }
...
// purpose  :
// =============================================================================
bool ViewerTest::ParseCorner (const char* theArg,
...
//=======================================================================
static int VNbDisplayed (Draw_Interpretor& theDi,
                                      int theArgsNb,
                                      const char** theArgVec)
{
  if(theArgsNb != 1)
                     ...
                     Draw_Interpretor&                                     theDI)
{
  ...
  else if (theObj->Type() == AIS_KindOfInteractive_Relation)
  {
    ...
    occ::handle<PrsDim_Relation> aRelation = Handle(PrsDim_Relation)::DownCast (theObj);
...
static void printLocalSelectionInfo (const T& theContext, Draw_Interpretor& theDI)
{
  ...
  {
    const occ::handle<AIS_Shape> aShapeIO = Handle(AIS_Shape)::DownCast (theContext->SelectedInteractive());
...
//purpose  :
//==============================================================================
static int VState (Draw_Interpretor& theDI,
                                int  theArgNb,
                                const char** theArgVec)
{
  ...
  if (toPrintEntities)
  {
    ...
    for (int aPickIter = 1; aPickIter <= aSelector->NbPicked(); ++aPickIter)
    {
      ...
      if (occ::handle<Select3D_SensitiveWire> aWire = Handle(Select3D_SensitiveWire)::DownCast (anEntity))
      {
      ...
      else if (occ::handle<Select3D_SensitivePrimitiveArray> aPrimArr = Handle(Select3D_SensitivePrimitiveArray)::DownCast (anEntity))
      {
        theDI << "                       Detected Element: "
              << aPrimArr->LastDetectedElement()
              << "\n";
      }
      else if (occ::handle<Select3D_SensitiveTriangulation> aTriSens = Handle(Select3D_SensitiveTriangulation)::DownCast (anEntity))
      {
  ...
  if (aCtx->NbSelected() > 0 && !toShowAll)
  {
    ...
    for (aCtx->InitSelected(); aCtx->MoreSelected(); aCtx->NextSelected())
    {
      ...
      const occ::handle<AIS_InteractiveObject> anObj = Handle(AIS_InteractiveObject)::DownCast (anOwner->Selectable());
...
{
  occ::handle<TopTools_HArray1OfShape> aResArray = new TopTools_HArray1OfShape (1, 1);
...
bool ViewerTest::PickShapes (const TopAbs_ShapeEnum theShapeType,
                                         occ::handle<TopTools_HArray1OfShape>& theResArray,
                                         const int theMaxPick)
{
  ...
  {
    if (occ::handle<AIS_Shape> aShapePrs = Handle(AIS_Shape)::DownCast (anObjIter.Value()))
    {
...
static int VPickShape( Draw_Interpretor& di, int argc, const char** argv)
{
  ...
  if (aNbToPick == 1)
  {
    ...
    else
    {
      ...
      aName = TCollection_AsciiString ("Picked_") + THE_NAME_TYPE[int(aShapeType)] + "_" + aShapeIndex;
    }
  ...
  else
  {
    ...
    occ::handle<TopTools_HArray1OfShape> aPickedArray = new TopTools_HArray1OfShape (1, aNbToPick);
    if (ViewerTest::PickShapes (aShapeType, aPickedArray))
    {
      for (int aPickedIter = aPickedArray->Lower(); aPickedIter <= aPickedArray->Upper(); ++aPickedIter)
      {
...
//purpose  :
//=======================================================================
static int VSelFilter(Draw_Interpretor& , int theArgc,
                      const char** theArgv)
{
  occ::handle<AIS_InteractiveContext> aContext = ViewerTest::GetAISContext();
  if (aContext.IsNull())
...
//==============================================================================
static int VLoadSelection (Draw_Interpretor& /*theDi*/,
                                        int theArgNb,
                                        const char** theArgVec)
{
  if (theArgNb < 2)
