...
namespace
{
  ...
  using ViewerTest_ViewerCommandsViewMap = NCollection_DoubleMap <TCollection_AsciiString, occ::handle<V3d_View>>;
  using ViewerTest_ViewerCommandsInteractiveContextMap = NCollection_DoubleMap <TCollection_AsciiString, occ::handle<AIS_InteractiveContext>>;
  using ViewerTest_ViewerCommandsGraphicDriverMap = NCollection_DoubleMap <TCollection_AsciiString, occ::handle<Graphic3d_GraphicDriver>>;
...
const occ::handle<WNT_WClass>& ViewerTest::WClass()
{
  ...
  if (theWClass.IsNull())
  {
    theWClass = new WNT_WClass ("GW3D_Class", (void* )AdvViewerWindowProc,
  ...
  ViewerTest_Names (const TCollection_AsciiString& theInputString)
  {
    ...
    if (theInputString.IsEmpty())
    {
      ...
      if (ViewerTest_myDrivers.IsEmpty())
        myDriverName = CreateName<occ::handle<Graphic3d_GraphicDriver>>
      ...
      myViewName = CreateName <occ::handle<V3d_View>> (ViewerTest_myViews, TCollection_AsciiString(myViewerName + "/View"));
    }
    else
    {
...
TCollection_AsciiString ViewerTest::ViewerInit (const ViewerTest_VinitParams& theParams)
{
  ...
  occ::handle<Graphic3d_GraphicDriverFactory> aFactory = Graphic3d_GraphicDriverFactory::DefaultDriverFactory();
  if (aFactory.IsNull())
  {
  ...
  {
    aViewNames.SetViewName (aViewNames.GetViewerName() + "/" + CreateName<occ::handle<V3d_View>>(ViewerTest_myViews, "View"));
  }
...
static int VDriver (Draw_Interpretor& theDi, int theArgsNb, const char** theArgVec)
{
  if (theArgsNb == 1)
  {
    ...
    theDi << "\n";
    theDi << "Default: ";
    if (occ::handle<Graphic3d_GraphicDriverFactory> aFactory =  Graphic3d_GraphicDriverFactory::DefaultDriverFactory())
    {
      theDi << aFactory->Name();
    }
    else
...
//! Parse HLR algo type.
static bool parseHlrAlgoType (const char* theName,
...
static int VHLRType (Draw_Interpretor& , int argc, const char** argv)
{
  ...
  for (int anArgIter = 1; anArgIter < argc; ++anArgIter)
  {
    ...
    else
    {
      ...
      occ::handle<AIS_Shape> aShape = Handle(AIS_Shape)::DownCast (aMap.Find2 (aName));
      if (aShape.IsNull())
      {
...
//purpose  :
//==============================================================================
void ViewerTest::ActivateView (const occ::handle<V3d_View>& theView,
                               bool theToUpdate)
{
...
//purpose  :
//==============================================================================
void ViewerTest::GetMousePosition (int& theX,
                                   int& theY)
{
...
static void VProcessEvents (ClientData theDispX, int)
{
  ...
  {
    const occ::handle<Aspect_DisplayConnection>& aDispConnTmp = aDriverIter.Key2()->GetDisplayConnection();
                        ...
                        const char**      theArgVec)
{
  ...
  for (int anArgIter = 2; anArgIter < theArgNb; ++anArgIter)
  {
    ...
    else if (aFlag == "-range")
    {
      ...
      if (!aRangeMin.IsRealValue (true)
       || !aRangeMax.IsRealValue (true))
      {
    ...
    else if (aFlag == "-colorrange")
    {
      ...
      int aNbParsed1 = Draw::ParseColor (theArgNb  - (anArgIter + 1),
                                                      theArgVec + (anArgIter + 1),
                                                      aColorMin);
      anArgIter += aNbParsed1;
      int aNbParsed2 = Draw::ParseColor (theArgNb  - (anArgIter + 1),
                                                      theArgVec + (anArgIter + 1),
                                                      aColorMax);
      anArgIter += aNbParsed2;
      if (aNbParsed1 == 0
...
//==============================================================================
static bool GetColor (const TCollection_AsciiString& theValue,
...
static int VGraduatedTrihedron (Draw_Interpretor& /*theDi*/, int theArgNum, const char** theArgs)
{
  ...
  // Check parameters
  for (NCollection_DataMap<TCollection_AsciiString, occ::handle<TColStd_HSequenceOfAsciiString>>::Iterator aMapIt (aMapOfArgs);
       aMapIt.More(); aMapIt.Next())
  {
    const TCollection_AsciiString& aKey = aMapIt.Key();
    const occ::handle<TColStd_HSequenceOfAsciiString>& anArgs = aMapIt.Value();
    ...
    if ( (aKey.IsEqual ("xnamecolor") || aKey.IsEqual ("ynamecolor") || aKey.IsEqual ("znamecolor")
          ...
          && anArgs->Length() == 1 && !anArgs->Value(1).IsIntegerValue() && !anArgs->Value(1).IsRealValue (true))
    {
      continue;
    }
  ...
  for (; anArgIter < theArgNb; ++anArgIter)
  {
          ...
          || anArg == "settings")
    {
      ...
      printZLayerInfo (theDI, aSettings);
          ...
          || anArg == "disable")
    {
      const bool toEnable = anArg == "-enable"
                                     || anArg == "enable";
      if (++anArgIter >= theArgNb)
      {
...
public:
  ...
  // constructor
  Standard_EXPORT V3d_LineItem(double X1, double Y1,
                               double X2, double Y2,
...
V3d_LineItem::V3d_LineItem(double X1, double Y1,
                           double X2, double Y2,
                            ...
                            const int )
{
  ...
  occ::handle<Prs3d_LineAspect> anAspect = new Prs3d_LineAspect (aColor, (Aspect_TypeOfLine)myType, myWidth);
                     ...
                     const char**      theArgVec)
{
  ...
  if (aCoord.Length() == 1)
  {
    switch (aMode)
    {
      case View   : theDI << "View Vv: "   << aView->Convert ((int)aCoord (1)); return 0;
                 ...
                 const char**      theArgVec)
{
  ...
  if (aParams.Method == Graphic3d_RM_RAYTRACING)
  {
    ...
    const double aMRays = aWinSize.x() * aWinSize.y() * aFpsAver * aParams.RaytracingDepth * 2 / 1.0e6f;
...
public:
  ...
  virtual bool AcceptDisplayMode (const int theMode) const override { return theMode == 0 || theMode == 1; }
  ...
  //! Compute selection.
  virtual void ComputeSelection (const occ::handle<SelectMgr_Selection>& theSel, const int theMode) override
  {
    if (theMode == 0)
    {
      occ::handle<SelectMgr_EntityOwner> anEntityOwner = new SelectMgr_EntityOwner (this, 5);
      occ::handle<Select3D_SensitivePrimitiveArray> aSensitive = new Select3D_SensitivePrimitiveArray (anEntityOwner);
...
static int VDiffImage (Draw_Interpretor& theDI, int theArgNb, const char** theArgVec)
{
  ...
  double aSizeY = aSizeX / aRatio;
  {
    ...
    occ::handle<ViewerTest_ImagePrs> anImgRefPrs = new ViewerTest_ImagePrs (anImgRef, aSizeX, aSizeY, aLabelRef);
    gp_Trsf aTrsfRef;
  ...
  }
  {
    ...
    occ::handle<ViewerTest_ImagePrs> anImgNewPrs = new ViewerTest_ImagePrs (anImgNew, aSizeX, aSizeY, aLabelNew);
    gp_Trsf aTrsfRef;
    aTrsfRef.SetTranslationPart (gp_Vec (aSizeX * 0.5, 0.0, 0.0));
                                ...
                                const char**     theArgVec)
{
  ...
  for (int anArgIter = 1; anArgIter < theNbArgs; ++anArgIter)
  {
     ...
     || anArgStr == "-clear")
    {
      ...
      const bool toEchoGrid = aContext->CurrentViewer()->IsGridActive()
...
namespace
{
  ...
  static NCollection_DataMap<TCollection_AsciiString, occ::handle<AIS_Animation>> ViewerTest_AnimationTimelineMap;
  ...
  static bool parseXYZ (const char** theArgVec, gp_XYZ& thePnt)
  {
    ...
    if (!anXYZ[0].IsRealValue (true)
     || !anXYZ[1].IsRealValue (true)
  ...
  static bool parseQuaternion (const char** theArgVec, gp_Quaternion& theQRot)
  {
    ...
    if (!anXYZW[0].IsRealValue (true)
     || !anXYZW[1].IsRealValue (true)
     || !anXYZW[2].IsRealValue (true)
...
{
  bool is2dMode = true;
  occ::handle<ViewerTest_V3dView> aV3dView = Handle(ViewerTest_V3dView)::DownCast (ViewerTest::CurrentView());
  if (aV3dView.IsNull())
  {
    Message::SendFail ("Error: no active viewer");
    return 1;
                                    ...
                                    const char**      theArgVec)
{
  ...
  if (anArgIter >= theArgNb)
  {
    ...
    for (NCollection_Sequence<occ::handle<AIS_Animation>>::Iterator anAnimIter (anAnimation->Children()); anAnimIter.More(); anAnimIter.Next())
    {
  ...
  for (; anArgIter < theArgNb; ++anArgIter)
  {
          ...
          || anArg == "-proc")
    {
      ...
      occ::handle<ViewerTest_AnimationProc> aCmdAnimation = new ViewerTest_AnimationProc (anAnimation->Name(), &theDI, theArgVec[anArgIter]);
          ...
          || anArg == "-obj")
    {
      ...
      replaceAnimation (aParentAnimation, anAnimation, anObjAnimation);
          ...
          || anArg == "-view")
    {
      occ::handle<AIS_AnimationCamera> aCamAnimation = Handle(AIS_AnimationCamera)::DownCast (anAnimation);
  ...
  && !isLockLoop)
  {
    occ::handle<ViewerTest_AnimationHolder> aHolder = new ViewerTest_AnimationHolder (anAnimation, aView, isFreeCamera);
  ...
  Message_ProgressScope aPS(Message_ProgressIndicator::Start(aProgress),
                            "Video recording, sec", Max(1, int(aPlayDuration / aPlaySpeed)));
  ...
  for (; aPts <= anUpperPts && aPS.More();)
  {
    ...
    if (aRecParams.FpsNum > 0)
    {
      aRecPts = aPlaySpeed * ((double(aRecParams.FpsDen) / double(aRecParams.FpsNum)) * double(aNbFrames));
    }
    else
    {
...
//=======================================================================
static int VNbSelected (Draw_Interpretor& di,
                                int argc,
                                const char ** argv)
{
  if(argc != 1)
...
//purpose  :
//=======================================================================
static int VSetViewSize (Draw_Interpretor& di,
                                int argc,
                                const char ** argv)
{
  occ::handle<AIS_InteractiveContext> aContext = ViewerTest::GetAISContext();
...
//purpose  :
//=======================================================================
static int VMoveView (Draw_Interpretor& di,
                                int argc,
                                const char ** argv)
{
  occ::handle<AIS_InteractiveContext> aContext = ViewerTest::GetAISContext();
...
//purpose  :
//=======================================================================
static int VTurnView (Draw_Interpretor& di,
                                int argc,
                                const char ** argv)
{
  occ::handle<AIS_InteractiveContext> aContext = ViewerTest::GetAISContext();
...
static int VTextureEnv (Draw_Interpretor& /*theDI*/, int theArgNb, const char** theArgVec)
{
  ...
  {
    TCollection_AsciiString aTextureOpt(theArgVec[2]);
    occ::handle<OCC_TextureEnv> aTexEnv = aTextureOpt.IsIntegerValue() ?
...
static int VClipPlane (Draw_Interpretor& theDi, int theArgsNb, const char** theArgVec)
{
   ...
   || aCommand == "clone")
  {
    ...
    bool toCreate = aCommand == "-create"
                             || aCommand == "create";
    bool toClone  = aCommand == "-clone"
                             || aCommand == "clone";
    bool toDelete = aCommand == "-delete"
                             || aCommand == "delete";
  ...
  for (; anArgIter < theArgsNb; ++anArgIter)
  {
            ...
            && aNbChangeArgs >= 7)
    {
      ...
      for (int aSubPlaneIter = 0; aSubPlaneIter < aNbSubPlanes; ++aSubPlaneIter)
      {
        ...
        const double aW = -aDir.Dot ((aSubPlaneIter % 2 == 1) ? aBndBox.CornerMax() : aBndBox.CornerMin());
    ...
    {
      Quantity_Color aColor;
      int aNbParsed = Draw::ParseColor (aNbChangeArgs - 1,
                                                     aChangeArgs + 1,
                                                     aColor);
      if (aNbParsed == 0)
      {
...
inline bool parseStereoMode (const char*      theArg,
                   ...
                   const char**      theArgVec)
{
          ...
          || anArgCase == "clear")
    {
      ...
      if (aLayer == Graphic3d_ZLayerId_UNKNOWN)
      {
        ...
        for (V3d_ListOfLightIterator aLightIter (aView->ActiveLightIterator()); aLightIter.More();)
        {
          ...
          if (occ::handle<AIS_LightSource> aLightSourceDel = Handle(AIS_LightSource)::DownCast (aPrsObject))
          {
  ...
  if (!aPrsList.IsEmpty())
  {
    ...
    {
      if (occ::handle<AIS_LightSource> aLightPrs2 = Handle(AIS_LightSource)::DownCast (aPrsIter.Value()))
      {
    ...
    int aTopStack = 0;
    for (std::vector<occ::handle<AIS_LightSource>>::iterator aPrsIter = aLightPrsVec.begin(); aPrsIter != aLightPrsVec.end(); ++aPrsIter)
    {
...
//purpose  :
//=======================================================================
static int VXRotate (Draw_Interpretor& di,
                                   int argc,
                                   const char ** argv)
{
  occ::handle<AIS_InteractiveContext> aContext = ViewerTest::GetAISContext();
                         ...
                         const char**      theArgVec)
{
  ...
  TCollection_AsciiString aName;
  // parameters
  int toAutoActivate = -1, toFollowTranslation = -1, toFollowRotation = -1, toFollowDragging = -1, isZoomable = -1;
  ...
  for (; anArgIter < theArgsNb; ++anArgIter)
  {
          ...
          && anArgIter + 1 < theArgsNb)
    {
      ...
      {
        occ::handle<AIS_Manipulator> aManip = Handle(AIS_Manipulator)::DownCast (anIter.Key1());
        if (!aManip.IsNull()
          && aManip->IsAttached()
   ...
   && (anAttachObject.IsNull() || anAttachPos != ManipAjustPosition_Center))
  {
    ...
    switch (anAttachPos)
    {
      ...
      {
        if (occ::handle<AIS_Shape> aShapePrs = Handle(AIS_Shape)::DownCast (aManipulator->Object()))
        {
                                 ...
                                 const char**      theArgVec)
{
  ...
  for (; anArgIter < theArgsNb; ++anArgIter)
  {
          ...
          || anArg == "-color")
    {
      ...
      Quantity_Color aColor;
      int aNbParsed = Draw::ParseColor (theArgsNb - anArgIter - 1,
                                                     theArgVec + anArgIter + 1,
                                                     aColor);
      if (aNbParsed == 0)
      {
                      ...
                      const char**      theArgVec)
{
  ...
  for (int anArgIter = 1; anArgIter < theNbArgs; ++anArgIter)
  {
    ...
    else if (anArg == "-orthopers")
    {
      ...
      occ::handle<Graphic3d_TransformPers> anOrthoPers = new Graphic3d_TransformPers (Graphic3d_TMF_TriedronPers | Graphic3d_TMF_OrthoPers, aTrsfPers->Corner2d(), aTrsfPers->Offset2d());
...
{
  NCollection_List<TCollection_AsciiString> aViewList;
  for (NCollection_DoubleMap<TCollection_AsciiString, occ::handle<V3d_View>>::Iterator anIter (ViewerTest_myViews);
       anIter.More(); anIter.Next())
  {
    aViewList.Append (anIter.Key1());
  }
