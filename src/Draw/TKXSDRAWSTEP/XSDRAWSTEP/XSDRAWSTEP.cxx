...
namespace
{
  using ExternalFileMap = NCollection_DataMap<TCollection_AsciiString, occ::handle<STEPCAFControl_ExternFile>>;
}
                                  ...
                                  const char** theArgVec)
{
  ...
  occ::handle<STEPControl_Controller> aCtl =
    occ::down_cast<STEPControl_Controller>(aWS->NormAdaptor());
  if (aCtl.IsNull())
  {
    aWS->SelectNorm("STEP");
  }
  ...
  int stat = sw.Transfer(shape, mode, true, aPSRoot.Next(90));
                                      ...
                                      const char** /*theArgVec*/)
{
  ...
  occ::handle<TColStd_HSequenceOfTransient> roots = WS->GiveList("xst-transferrable-roots", "");
                                 ...
                                 const char** theArgVec)
{
  ...
  bool isFileMode = XSDRAW::FileAndVar(aFilePath.ToCString(), aDocumentName, "STEP", aFileName, anOldVarName);
  ...
  if (!aModeStr.IsEmpty())
  {
    ...
    for (int i = 1; i <= aModeStr.Length(); ++i)
    {
  ...
  if (!DDocStd::GetDocument(aDocumentName, aDocument, false))
  {
    ...
    occ::handle<DDocStd_DrawDocument> aDrawDoc = new DDocStd_DrawDocument(aDocument);
                                  ...
                                  const char** theArgVec)
{
  ...
  for (int anArgIter = 1; anArgIter < theNbArgs; ++anArgIter)
  {
    ...
    {
      const char* aDocNameStr = theArgVec[anArgIter];
    ...
    else if (!hasModeArg)
    {
      ...
      for (int i = 1; i <= anArgCase.Length(); ++i)
      {
  ...
  TCollection_AsciiString aFileName, anOldVarName;
  const bool isFileMode = XSDRAW::FileAndVar(aFilePath.ToCString(), aDocumentName.ToCString(), "STEP", aFileName, anOldVarName);
