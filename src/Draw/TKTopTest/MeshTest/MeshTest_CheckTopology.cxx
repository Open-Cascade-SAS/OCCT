    ...
    for (i = 1; i <= nbTri; i++)
    {
      ...
      for (j = 0; j < 3; j++)
      {
        if (t[j] == 0)
        {
          ...
          {
            occ::handle<TColStd_HSequenceOfInteger> tmpSeq = new TColStd_HSequenceOfInteger;
            myMapFaceLinks.Add(iF, tmpSeq);
          }
          occ::handle<TColStd_HSequenceOfInteger>& aSeq = myMapFaceLinks.ChangeFromKey(iF);
          aSeq->Append(n1);
          aSeq->Append(n2);
        }
      }
...
void MeshTest_CheckTopology::GetFreeLink(const int theFaceIndex,
                                         ...
                                         int&      theNode2) const
{
  const occ::handle<TColStd_HSequenceOfInteger>& aSeq = myMapFaceLinks(theFaceIndex);
