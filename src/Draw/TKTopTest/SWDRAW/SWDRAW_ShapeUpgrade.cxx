...
/*
  if (!inputShape.IsNull()) {
    ...
    return 1;
      ...
      occ::handle<TColGeom_HArray2OfSurface>
...

{
  ...
  if (Split == 1)
  {
    occ::handle<TColStd_HSequenceOfReal> spval = new TColStd_HSequenceOfReal;
    for (int i = 1; i <= 5; i++)
      spval->Append(i);
    theTool->SetSplitValues(spval);
  }
  theTool->Perform(true);
  occ::handle<TColGeom_HArray1OfCurve> theCurves = theTool->GetCurves();
...

{
  ...
  theTool->Perform(true);
  occ::handle<TColGeom2d_HArray1OfCurve> theCurves = theTool->GetCurves();
  int                  NbC       = theCurves->Length();
  for (int icurv = 1; icurv <= NbC; icurv++)
  {
    char name[100];
...

{
  ...
  occ::handle<TColStd_HArray1OfReal>        GlobalU = Grid->UJointValues();
  occ::handle<TColStd_HArray1OfReal>        GlobalV = Grid->VJointValues();
  ...
  di << "appel a Surfaces\n";
  occ::handle<TColGeom_HArray2OfSurface> theSurfaces = Grid->Patches();
...
static int splitface(Draw_Interpretor& di, int argc, const char** argv)
{
  ...
  occ::handle<TColGeom_HArray2OfSurface> AS =
