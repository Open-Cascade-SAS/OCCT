...
static occ::handle<TColStd_HSequenceOfAsciiString>                         thenews;
...
void XSDRAW::LoadDraw(Draw_Interpretor& theCommands)
{
  ...
  occ::handle<TColStd_HSequenceOfAsciiString> list = IFSelect_Activator::Commands(0);
...
occ::handle<TColStd_HSequenceOfTransient> XSDRAW::GetList(const char* first,
                                                     const char* second)
{
  if (!first || first[0] == '\0')
  {
...
int XSDRAW::MoreShapes(occ::handle<TopTools_HSequenceOfShape>& list,
                                    const char*             name)
{
