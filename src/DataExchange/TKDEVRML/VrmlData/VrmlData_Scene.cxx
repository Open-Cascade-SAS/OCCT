                                               ...
                                               const occ::handle<Standard_Type>& /*theType*/) const
{
  ...
  if (myNamedNodes.Contains(aDummyNode))
    aResult = const_cast<NCollection_Map<occ::handle<VrmlData_Node>>&>(myNamedNodes).Added(aDummyNode);
#endif
  return aResult;
}
...
TopoDS_Shape VrmlData_Scene::GetShape(NCollection_DataMap<occ::handle<TopoDS_TShape>, occ::handle<VrmlData_Appearance>>& aMap)
{
  TopoDS_Shape aShape;
    ...
    else
    {
      ...
      while (isMore && VrmlData_Node::OK(aStatus, ReadLine(theBuffer)))
      {
        ...
        if ((anIntValue < 0 || isMore == false) && vecInt.Length() > 0)
        {
          ...
          int* bufFace = static_cast<int*>(
            myAllocator->Allocate((aLen + 1) * sizeof(int)));
          if (bufFace == 0L)
          {
            aStatus = VrmlData_UnrecoverableError;
            break;
      ...
      if (aStatus == VrmlData_StatusOK)
      {
        ...
        if (aNbBlocks)
        {
          const int** anArray = static_cast<const int**>(
            myAllocator->Allocate(aNbBlocks * sizeof(int*)));
          if (anArray == 0L)
            aStatus = VrmlData_UnrecoverableError;
          else
          {
              ...
              const TCollection_AsciiString& theIndent)
{
  ...
  {
    const occ::handle<VrmlData_Appearance> anAppearance = occ::down_cast<VrmlData_Appearance>(theNode);
  ...
  {
    const occ::handle<VrmlData_UnknownNode> anUnknown = occ::down_cast<VrmlData_UnknownNode>(theNode);
    char                               buf[64];
