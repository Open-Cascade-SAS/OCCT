...
bool XCAFDoc_NotesTool::RemoveAllNotes(const XCAFDoc_AssemblyItemId& theItemId,
                                                   bool              theDelIfOrphan)
{
  TDF_Label anAnnotatedItem = FindAnnotatedItem(theItemId);
  if (anAnnotatedItem.IsNull())
...
bool XCAFDoc_NotesTool::RemoveAllNotes(const TDF_Label& theItemLabel,
                                                   bool theDelIfOrphan)
{
...
{
  int nbNotes = 0;
  for (NCollection_Sequence<TDF_Label>::Iterator anIter(theNoteLabels); anIter.More(); anIter.Next())
  {
