...
void IGESBasic_HArray1OfHArray1OfReal::SetValue(const int               num,
                                                const occ::handle<TColStd_HArray1OfReal>& val)
{
  thelist.SetValue(num, val);
}

occ::handle<TColStd_HArray1OfReal> IGESBasic_HArray1OfHArray1OfReal::Value(
  const int num) const
{
  return occ::down_cast<TColStd_HArray1OfReal>(thelist.Value(num));
}
