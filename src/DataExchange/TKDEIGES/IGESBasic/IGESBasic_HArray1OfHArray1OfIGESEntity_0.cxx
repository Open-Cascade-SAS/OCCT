...
void IGESBasic_HArray1OfHArray1OfIGESEntity::SetValue(
  const int                      num,
  const occ::handle<IGESData_HArray1OfIGESEntity>& val)
{
  thelist.SetValue(num, val);
}

occ::handle<IGESData_HArray1OfIGESEntity> IGESBasic_HArray1OfHArray1OfIGESEntity::Value(
  const int num) const
{
  return occ::down_cast<IGESData_HArray1OfIGESEntity>(thelist.Value(num));
}
