...
bool IGESData_ParamReader::ReadInts(const IGESData_ParamCursor&       PC,
                                                const Message_Msg&                amsg,
                                                occ::handle<TColStd_HArray1OfInteger>& val,
                                                const int            index)
{
...
bool IGESData_ParamReader::ReadReals(const IGESData_ParamCursor& PC,
                                                 Message_Msg& /*amsg*/,
                                                 occ::handle<TColStd_HArray1OfReal>& val,
                                                 const int         index)
{
  if (!PrepareRead(PC, true))
    return false;
...
bool IGESData_ParamReader::ReadTexts(const IGESData_ParamCursor&              PC,
                                                 const Message_Msg&                       amsg,
                                                 occ::handle<Interface_HArray1OfHAsciiString>& val,
                                                 const int                   index)
{
  if (!PrepareRead(PC, true))
    return false;
...
bool IGESData_ParamReader::ReadEnts(const occ::handle<IGESData_IGESReaderData>& IR,
                                                const IGESData_ParamCursor&            PC,
                                                const Message_Msg&                     amsg,
                                                occ::handle<IGESData_HArray1OfIGESEntity>&  val,
                                                const int                 index)
{
  if (!PrepareRead(PC, true))
    return false;
  ...
  else
  {
    ...
    occ::handle<IGESData_HArray1OfIGESEntity> tab = new IGESData_HArray1OfIGESEntity(index, ind - 1);
    for (i = index; i < ind; i++)
