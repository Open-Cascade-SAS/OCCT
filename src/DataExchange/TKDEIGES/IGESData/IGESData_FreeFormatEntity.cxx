...
occ::handle<TCollection_HAsciiString> IGESData_FreeFormatEntity::ParamValue(
  const int num) const
{
  return UndefinedContent()->ParamValue(num);
}

occ::handle<TColStd_HSequenceOfInteger> IGESData_FreeFormatEntity::NegativePointers() const
{
  return thenegptrs;
}
...
void IGESData_FreeFormatEntity::AddEntities(const occ::handle<IGESData_HArray1OfIGESEntity>& ents)
{
  if (ents.IsNull())
  {
...
void IGESData_FreeFormatEntity::AddNegativePointers(const occ::handle<TColStd_HSequenceOfInteger>& list)
{
  if (thenegptrs.IsNull())
    thenegptrs = new TColStd_HSequenceOfInteger();
