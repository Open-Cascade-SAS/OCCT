...
public:
  ...
  Standard_EXPORT occ::handle<TColStd_HSequenceOfInteger> NegativePointers() const;
  ...
  //! by IGESWriter
  Standard_EXPORT void AddEntities(const occ::handle<IGESData_HArray1OfIGESEntity>& ents);

  //! Adds a list of Ranks of Parameters to be noted as Negative
  //! Pointers (this will be taken into account for Parameters
  //! which are Entities)
  Standard_EXPORT void AddNegativePointers(const occ::handle<TColStd_HSequenceOfInteger>& list);
...
private:
  occ::handle<TColStd_HSequenceOfInteger> thenegptrs;
};
