...
static bool Edit(const occ::handle<Standard_Transient>& ent, const bool UV)
{
  ...
  if (!bndy.IsNull())
  {
    ...
    if (nb == 0)
      return false;
    occ::handle<IGESData_HArray1OfIGESEntity>           arc3d = new IGESData_HArray1OfIGESEntity(1, nb);
    occ::handle<IGESBasic_HArray1OfHArray1OfIGESEntity> arcuv =
      new IGESBasic_HArray1OfHArray1OfIGESEntity(1, nb);
    occ::handle<TColStd_HArray1OfInteger> sens = new TColStd_HArray1OfInteger(1, nb);
    for (i = 1; i <= nb; i++)
    {
      sens->SetValue(i, bndy->Sense(i));
      occ::handle<IGESData_HArray1OfIGESEntity> cuv = bndy->ParameterCurves(i);
