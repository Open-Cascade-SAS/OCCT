...
static int iges_readline_file(void* theContext, char* theLine, int theLineSize)
{
  FILE* aFile = (FILE*)theContext;
  return fgets(theLine, theLineSize, aFile) == NULL ? 0 : 1;
}
...
int igesread (char* nomfic, int lesect[6], int modefnes)
{
  ...
  {
    aFile = fopen(nomfic, "rb");
  }
  if (aFile == NULL)
  {
    return -1;
  }
  int aResult = igesread_stream(aFile, iges_readline_file, lesect, modefnes);
  if (aFile != stdin)
  {
    fclose(aFile);
  }
...
int igesread_stream(void* theContext, IGES_ReadLineCallback theReadLine, int lesect[6], int modefnes)
{
  char ligne[100];
  int  numsec, numl;
  int  i;
  ...
  {
    numl++;
    i = iges_lire_stream(theContext, theReadLine, &numsec, ligne, modefnes);
    if (i <= 0 || i < i0)
    {
      if (i == 0)
      {
