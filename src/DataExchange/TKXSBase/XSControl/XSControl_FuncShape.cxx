...
static IFSelect_ReturnStatus XSControl_tpcompound(const occ::handle<IFSelect_SessionPilot>& pilot)
{
  ...
  {
    occ::handle<TColStd_HSequenceOfTransient> lise =
...
static IFSelect_ReturnStatus XSControl_traccess(const occ::handle<IFSelect_SessionPilot>& pilot)
{
  ...
  if (num == 0 || cascomp)
  {
    ...
    const occ::handle<TopTools_HSequenceOfShape>& list = TR->ShapeResultList(true);
                                                 ...
                                                 const char*               name)
{
  ...
  if (name[0] == '*' && (name[1] == '\0' || (name[1] == '*' && name[2] == '\0')))
  {
    ...
    occ::handle<TopTools_HSequenceOfShape> li = TransferBRep::Shapes(TP, (name[1] == '\0'));
    if (li.IsNull())
      return 0;
    list->Append(li);
    return li->Length();
