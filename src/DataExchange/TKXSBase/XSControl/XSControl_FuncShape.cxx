...
static IFSelect_ReturnStatus XSControl_tpdraw(const occ::handle<IFSelect_SessionPilot>& pilot)
{
  ...
  if (mode == 0)
  {
    if (argc < 2)
    {
      sout << "Donner au moins un NUMERO ou *" << '\n';
      return IFSelect_RetError;
    }
    if (arg1[0] == '*')
    {
  ...
  if (model.IsNull())
  {
    if (mode == 0)
    {
      sout << "No model, specify n0 of transfer item" << '\n';
      return IFSelect_RetError;
    }
  }
  if (mode == 0)
  ...
  for (i = n1; i <= n2; i++)
  {
    ...
    if (index == 0)
    {
      if (!tout)
      {
        sout << "Entite n0 " << num << " : non repertoriee" << '\n';
      }
      continue;
    }
    if (!binder->HasResult())
...
static IFSelect_ReturnStatus XSControl_tpcompound(const occ::handle<IFSelect_SessionPilot>& pilot)
{
  ...
  if (argc < 2)
  {
    sout << "Give a NAME for the Compound  + optional givelist, else roots are taken" << '\n';
    return IFSelect_RetError;
  }
  ...
  else
  {
    ...
    if (lise.IsNull())
    {
      sout << "Not a valid entity list : " << pilot->CommandPart(2) << '\n';
      return IFSelect_RetError;
    }
...
static IFSelect_ReturnStatus XSControl_traccess(const occ::handle<IFSelect_SessionPilot>& pilot)
{
  ...
  if (cassave)
  {
    sout << " save shapes -> current directory" << '\n';
  }
...
static IFSelect_ReturnStatus XSControl_fromshape(const occ::handle<IFSelect_SessionPilot>& pilot)
{
  ...
  if (!yena)
  {
    sout << "No transfer (either import or export) recorded" << '\n';
  }
...
static IFSelect_ReturnStatus XSControl_trimport(const occ::handle<IFSelect_SessionPilot>& pilot)
{
  ...
  if (modfic)
  {
    sout << " File to read : " << fnom << '\n';
  }
  else
  {
    sout << " Model taken from the session : " << fnom << '\n';
  }
...
static IFSelect_ReturnStatus XSControl_twrite(const occ::handle<IFSelect_SessionPilot>& pilot)
{
  ...
  for (int i = 1; i < argc; i++)
  {
    ...
    if (Shape.IsNull())
    {
      sout << "not a draw shape name:" << arg1 << '\n';
      continue;
    }
    sout << "Pour Shape : " << ai;
