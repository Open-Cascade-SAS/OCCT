...
occ::handle<TColStd_HSequenceOfTransient> XSControl_Reader::GiveList(const char* first,
                                                                const char* second)
{
  if (first && first[0] != '\0')
  {
...
occ::handle<TColStd_HSequenceOfTransient> XSControl_Reader::GiveList(
  const char*            first,
...
int XSControl_Reader::TransferList(const occ::handle<TColStd_HSequenceOfTransient>& list,
                                                const Message_ProgressRange& theProgress)
{
  if (list.IsNull())
    return 0;
...
void XSControl_Reader::GetStatsTransfer(const occ::handle<TColStd_HSequenceOfTransient>& list,
                                        int&                           nbMapped,
                                        int&                           nbWithResult,
