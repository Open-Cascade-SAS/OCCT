...
occ::handle<TColStd_HSequenceOfTransient> XSControl_TransferReader::RecordedList() const
{
  occ::handle<TColStd_HSequenceOfTransient> li = new TColStd_HSequenceOfTransient();
  if (myModel.IsNull())
    return li;
  ...
  const int            mode) const
{
  ...
  if (mode >= 2)
  {
    ...
    for (i = 1; i <= nb; i++)
    {
      ...
      if (rec.IsNull())
        return nulh;
      occ::handle<TColStd_HSequenceOfTransient> list = rec->Results(mode - 2);
...
occ::handle<TColStd_HSequenceOfTransient> XSControl_TransferReader::EntitiesFromShapeList(
  const occ::handle<TopTools_HSequenceOfShape>& res,
  const int                   mode) const
{
  occ::handle<TColStd_HSequenceOfTransient> lt = new TColStd_HSequenceOfTransient();
  if (res.IsNull())
    return lt;
...
occ::handle<TColStd_HSequenceOfTransient> XSControl_TransferReader::CheckedList(
  const occ::handle<Standard_Transient>& ent,
  const Interface_CheckStatus       withcheck,
  const bool            level) const
{
  occ::handle<TColStd_HSequenceOfTransient> res = new TColStd_HSequenceOfTransient();
  if (ent.IsNull())
    return res;
                                          ...
                                          const int mode) const
{
  ...
  occ::handle<TColStd_HSequenceOfTransient> list = RecordedList();
  int                     i, nb = list->Length();
...
occ::handle<TColStd_HSequenceOfTransient> XSControl_TransferReader::LastTransferList(
...
const occ::handle<TopTools_HSequenceOfShape>& XSControl_TransferReader::ShapeResultList(
  const bool rec)
{
  if (!rec)
  {
  ...
  else
  {
    ...
    occ::handle<TColStd_HSequenceOfTransient> li = RecordedList();
    myShapeResult                           = new TopTools_HSequenceOfShape();
    int i, nb = myModel->NbEntities();
    TopoDS_Shape     sh;
    for (i = 1; i <= nb; i++)
...
void XSControl_TransferReader::PrintStatsProcess(const occ::handle<Transfer_TransientProcess>& TP,
                                                 ...
                                                 const int                   mode)
{
  occ::handle<TColStd_HSequenceOfTransient> list; // null
