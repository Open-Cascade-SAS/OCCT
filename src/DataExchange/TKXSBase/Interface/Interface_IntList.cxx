...
Interface_IntList::Interface_IntList(const Interface_IntList& other, const bool copied)
{
  ...
  {
    int                 i;
    occ::handle<TColStd_HArray1OfInteger> ents = new TColStd_HArray1OfInteger(0, thenbe);
    ents->Init(0);
    for (i = 1; i <= thenbe; i++)
      ents->SetValue(i, theents->Value(i));
    occ::handle<TColStd_HArray1OfInteger> refs = new TColStd_HArray1OfInteger(0, thenbr);
    refs->Init(0);
    for (i = 1; i <= thenbr; i++)
...
void Interface_IntList::SetNbEntities(const int nbe)
{
  if (nbe <= theents->Upper())
    return;
  int                 i;
  occ::handle<TColStd_HArray1OfInteger> ents = new TColStd_HArray1OfInteger(0, nbe);
  ents->Init(0);
  for (i = 1; i <= thenbe; i++)
    ents->SetValue(i, theents->Value(i));
  theents = ents;
...
void Interface_IntList::Reservate(const int count)
{
  ...
  if (up < 2)
    up = 2;
  occ::handle<TColStd_HArray1OfInteger> refs = new TColStd_HArray1OfInteger(0, up);
  refs->Init(0);
...
bool Interface_IntList::IsRedefined(const int num) const
{
...
void Interface_IntList::AdjustSize(const int margin)
{
  ...
  else
  {
    ...
    if (up >= thenbr && up <= thenbr + margin)
      return;
    occ::handle<TColStd_HArray1OfInteger> refs = new TColStd_HArray1OfInteger(0, thenbr + margin);
    refs->Init(0);
    for (i = 1; i <= thenbr; i++)
      refs->SetValue(i, therefs->Value(i));
    therefs = refs;
