...
const occ::handle<TColStd_HArray1OfListOfInteger>& Interface_Graph::SharingTable() const
{
  return thesharings;
}
...
void Interface_Graph::Evaluate()
{
  ...
  occ::handle<TColStd_HSequenceOfTransient> aListOfEntities = new TColStd_HSequenceOfTransient();
  for (int i = 1; i <= anEntityNumber; i++)
  {
...
occ::handle<TColStd_HSequenceOfTransient> Interface_Graph::GetShareds(
  const occ::handle<Standard_Transient>& ent) const
{
  return Shareds(ent).Content();
}

occ::handle<TColStd_HSequenceOfTransient> Interface_Graph::GetSharings(
  const occ::handle<Standard_Transient>& ent) const
{
  int num = EntityNumber(ent);
  if (!num)
    return 0;
  // return occ::down_cast<TColStd_HSequenceOfTransient>(thesharings->Value(num));
  const NCollection_List<int>&         alist     = thesharings->Value(num);
  occ::handle<TColStd_HSequenceOfTransient> aSharings = new TColStd_HSequenceOfTransient;
                             ...
                             const Interface_Graph&            G)
{
  ...
  occ::handle<TColStd_HSequenceOfTransient> list = G.GetSharings(ent);
  if (list.IsNull())
    return;
