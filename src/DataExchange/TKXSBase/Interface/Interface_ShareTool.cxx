...
bool Interface_ShareTool::IsShared(const occ::handle<Standard_Transient>& ent) const
{
  ...
  occ::handle<TColStd_HSequenceOfTransient> list     = thegraph.GetShareds(ent);
...
{
  Interface_Graph&                     thegraph = theHGraph->CGraph();
  occ::handle<TColStd_HSequenceOfTransient> list     = thegraph.GetSharings(ent);
  if (list.IsNull())
    return 0;
                                                  ...
                                                  const bool            rootlast) const
{
  ...
  occ::handle<TColStd_HArray1OfInteger> fl = new TColStd_HArray1OfInteger(0, nb);
  fl->Init(0);
  if (ent == model)
  {
  ...
  {
    occ::handle<TColStd_HSequenceOfTransient> sq = new TColStd_HSequenceOfTransient();
    sq->Append(ent);
    //    file type process
  ...
  occ::handle<TColStd_HArray1OfInteger> ord = new TColStd_HArray1OfInteger(0, nb);
  ord->Init(0);
  for (i = 1; i <= nb; i++)
  {
    n0 = fl->Value(i);
