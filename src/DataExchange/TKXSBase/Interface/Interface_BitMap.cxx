...
void Interface_BitMap::Reservate(const int moreflags)
{
  ...
  occ::handle<TColStd_HArray1OfInteger> flags = new TColStd_HArray1OfInteger(0, nbw);
...
void Interface_BitMap::SetLength(const int nbitems)
{
  ...
  if (nbw == thenbwords)
    return;
  occ::handle<TColStd_HArray1OfInteger> flags = new TColStd_HArray1OfInteger(0, nbw * (thenbflags + 1));
  if (nbw > thenbwords)
    flags->Init(0);
