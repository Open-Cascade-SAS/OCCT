...
occ::handle<IFSelect_ListEditor> IFSelect_EditForm::ListEditor(const int num) const
{
  ...
  occ::handle<TColStd_HSequenceOfHAsciiString> lis = theeditor->ListValue(this, num);
...
void IFSelect_EditForm::LoadList(const int                         num,
                                 const occ::handle<TColStd_HSequenceOfHAsciiString>& list)
{
  int n = RankFromNumber(num);
  if (n <= 0 || n > theorigs.Upper())
    return;
...
occ::handle<TColStd_HSequenceOfHAsciiString> IFSelect_EditForm::OriginalList(
  const int num) const
{
  int                        n = RankFromNumber(num);
  occ::handle<TColStd_HSequenceOfHAsciiString> list;
  if (theorigs.Upper() == 0)
    return theeditor->ListValue(this, num);
  else
    return occ::down_cast<TColStd_HSequenceOfHAsciiString>(theorigs.Value(n));
}
...
occ::handle<TColStd_HSequenceOfHAsciiString> IFSelect_EditForm::EditedList(
  const int num) const
{
  if (themodifs.Upper() == 0)
  ...
  if (!IsModified(num))
    return OriginalList(num);
  int n = RankFromNumber(num);
  return occ::down_cast<TColStd_HSequenceOfHAsciiString>(themodifs.Value(n));
}
                                               ...
                                               const bool             enforce)
{
  ...
  if (!edited->IsTouched())
    return false;
  occ::handle<TColStd_HSequenceOfHAsciiString> newlist = edited->EditedValues();
...
bool IFSelect_EditForm::TouchList(
  const int                         num,
  const occ::handle<TColStd_HSequenceOfHAsciiString>& newlist)
{
  if (themodifs.Upper() == 0)
    return false;
  int tnum = RankFromNumber(num);
...
static void PrintList(const occ::handle<TColStd_HSequenceOfHAsciiString>& list,
                      Standard_OStream&                              S,
                      const bool                         alsolist)
{
  if (list.IsNull())
                                    ...
                                    const bool alsolist) const
{
  ...
  for (iv = 1; iv <= nbv; iv++)
  {
    ...
    else
    {
      ...
      if (theeditor->IsList(jv))
      {
        occ::handle<TColStd_HSequenceOfHAsciiString> list = OriginalList(jv);
