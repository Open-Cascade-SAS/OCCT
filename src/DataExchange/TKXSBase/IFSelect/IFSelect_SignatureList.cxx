...
void IFSelect_SignatureList::Add(const occ::handle<Standard_Transient>& ent, const char* sign)
{
  ...
  if (thelistat)
  {
    occ::handle<TColStd_HSequenceOfTransient> alist;
    if (thediclist.Contains(sign))
      alist = occ::down_cast<TColStd_HSequenceOfTransient>(thediclist.FindFromKey(sign));
    else
    {
...
occ::handle<TColStd_HSequenceOfHAsciiString> IFSelect_SignatureList::List(
...
occ::handle<TColStd_HSequenceOfTransient> IFSelect_SignatureList::Entities(
  const char* sign) const
{
  ...
  if (thediclist.FindFromKey(sign, aTList))
    list = occ::down_cast<TColStd_HSequenceOfTransient>(aTList);
  else
                                       ...
                                       const IFSelect_PrintCount               mod) const
{
  ...
  int nbtot = 0, nbsign = 0;
  NCollection_IndexedDataMap<TCollection_AsciiString, occ::handle<Standard_Transient>>::Iterator iter(
    thediclist);
  for (; iter.More(); iter.Next())
  {
...
void IFSelect_SignatureList::PrintSum(Standard_OStream& S) const
{
  ...
  int nbtot = 0, nbsign = 0, maxent = 0, nbval = 0, nbve = 0, minval = 0, maxval = 0,
                   totval = 0;
  for (; iter.More(); iter.Next())
  {
