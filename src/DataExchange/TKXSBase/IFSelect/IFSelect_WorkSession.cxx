...
void IFSelect_WorkSession::ClearData(const int mode)
{
  switch (mode)
  {
    ...
    case 4: {
      ...
      occ::handle<TColStd_HSequenceOfInteger> list = ItemIdents(STANDARD_TYPE(IFSelect_SelectPointed));
      int                   nb   = list->Length();
      int                   i; // svv #1
      for (i = 1; i <= nb; i++)
      {
...
occ::handle<TColStd_HSequenceOfTransient> IFSelect_WorkSession::Shareds(
...
occ::handle<TColStd_HSequenceOfTransient> IFSelect_WorkSession::Sharings(
  const occ::handle<Standard_Transient>& ent)
{
  occ::handle<TColStd_HSequenceOfTransient> list;
  if (!ComputeGraph())
...
occ::handle<TColStd_HSequenceOfInteger> IFSelect_WorkSession::ItemIdents(
...
occ::handle<TColStd_HSequenceOfHAsciiString> IFSelect_WorkSession::ItemNames(
  const occ::handle<Standard_Type>& type) const
{
  occ::handle<TColStd_HSequenceOfHAsciiString> list = new TColStd_HSequenceOfHAsciiString();
...
occ::handle<TColStd_HSequenceOfHAsciiString> IFSelect_WorkSession::ItemNamesForLabel(
...
occ::handle<TColStd_HSequenceOfTransient> IFSelect_WorkSession::SelectionResult(
  const occ::handle<IFSelect_Selection>& sel) const
{
  occ::handle<TColStd_HSequenceOfTransient> res;
  if (errhand)
  {
    errhand = false;
    try
...
occ::handle<TColStd_HSequenceOfTransient> IFSelect_WorkSession::SelectionResultFromList(
  ...
  const occ::handle<TColStd_HSequenceOfTransient>& list) const
{
  ...
  occ::handle<TColStd_HSequenceOfTransient> res = SelectionResult(sel);
...
occ::handle<TColStd_HSequenceOfInteger> IFSelect_WorkSession::AppliedDispatches() const
{
  occ::handle<TColStd_HSequenceOfInteger> list = new TColStd_HSequenceOfInteger();
...
occ::handle<TColStd_HSequenceOfInteger> IFSelect_WorkSession::FinalModifierIdents(
...
{
  occ::handle<TColStd_HSequenceOfInteger> list = FinalModifierIdents(true);
  int                   nb   = list->Length();
  int                   i; // svv #1
  for (i = 1; i <= nb; i++)
                                                       ...
                                                       const bool            keep)
{
  ...
  SetModel(newmod, false);
  //  UPDATE of SelectPointed
  occ::handle<TColStd_HSequenceOfInteger> pts = ItemIdents(STANDARD_TYPE(IFSelect_SelectPointed));
  nb                                     = pts->Length();
  for (i = 1; i <= nb; i++)
  {
...
occ::handle<TColStd_HSequenceOfHAsciiString> IFSelect_WorkSession::SentFiles() const
{
                                                   ...
                                                   const occ::handle<Standard_Transient>& entson) const
{
  ...
  occ::handle<TColStd_HSequenceOfTransient> list = thegraph->Graph().Sharings(entson).Content();
  if (list.IsNull())
    return -1;
  int i, nb = list->Length();
  for (i = 1; i <= nb; i++)
...
occ::handle<TColStd_HSequenceOfTransient> IFSelect_WorkSession::GiveList(
...
occ::handle<TColStd_HSequenceOfTransient> IFSelect_WorkSession::GiveListFromList(
  const char*            selname,
  const occ::handle<Standard_Transient>& ent) const
{
  occ::handle<TColStd_HSequenceOfTransient> list;
...
occ::handle<TColStd_HSequenceOfTransient> IFSelect_WorkSession::GiveListCombined(
                                             ...
                                             Standard_OStream&                 S)
{
  ...
  occ::handle<TColStd_HSequenceOfTransient> list = Sharings(ent);
  if (list.IsNull())
    S << "  Root" << std::endl;
  else
  {
