...
public:
  ...
  Standard_EXPORT occ::handle<TColStd_HSequenceOfTransient> MultipleResult() const;

  //! Defines a Binding with a Multiple Result, given as a Sequence
  //! Error if a Unique Result has yet been defined
  Standard_EXPORT void SetMultipleResult(const occ::handle<TColStd_HSequenceOfTransient>& mulres);

  DEFINE_STANDARD_RTTIEXT(Transfer_MultipleBinder, Transfer_Binder)

private:
  occ::handle<TColStd_HSequenceOfTransient> themulres;
};
