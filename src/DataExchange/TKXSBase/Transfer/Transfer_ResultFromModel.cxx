...
occ::handle<TColStd_HSequenceOfTransient> Transfer_ResultFromModel::Results(
  const int level) const
{
  int                     i, nb;
  occ::handle<TColStd_HSequenceOfTransient> list = new TColStd_HSequenceOfTransient();
  if (level > 1)
  {
...
occ::handle<TColStd_HSequenceOfTransient> Transfer_ResultFromModel::TransferredList(
  const int level) const
{
  int                     i, nb;
  occ::handle<TColStd_HSequenceOfTransient> list = new TColStd_HSequenceOfTransient();
  occ::handle<TColStd_HSequenceOfTransient> res  = Results(level);
  nb                                        = res->Length();
  for (i = 1; i <= nb; i++)
  {
...
occ::handle<TColStd_HSequenceOfTransient> Transfer_ResultFromModel::CheckedList(
  const Interface_CheckStatus check,
  const bool      result) const
{
  int                     i, nb;
  occ::handle<TColStd_HSequenceOfTransient> list = new TColStd_HSequenceOfTransient();
  occ::handle<TColStd_HSequenceOfTransient> res  = Results(2);
  nb                                        = res->Length();
  for (i = 1; i <= nb; i++)
  {
    DeclareAndCast(Transfer_ResultFromTransient, unres, res->Value(i));
