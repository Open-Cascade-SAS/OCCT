...
static void ShapeAppend(const occ::handle<Transfer_Binder>&           binder,
                        const occ::handle<TopTools_HSequenceOfShape>& shapes)
{
  if (binder.IsNull())
    return;
...
occ::handle<TopTools_HSequenceOfShape> TransferBRep::Shapes(const occ::handle<Transfer_TransientProcess>& TP,
                                                       const bool roots)
{
  occ::handle<TopTools_HSequenceOfShape> shapes;
  if (TP.IsNull())
    return shapes;
...
occ::handle<TopTools_HSequenceOfShape> TransferBRep::Shapes(
...
occ::handle<TColStd_HSequenceOfTransient> TransferBRep::Checked(const Interface_CheckIterator& chl,
                                                           const bool alsoshapes)
{
  occ::handle<TColStd_HSequenceOfTransient> ls = new TColStd_HSequenceOfTransient();
  for (chl.Start(); chl.More(); chl.Next())
  {
    const occ::handle<Interface_Check>& ach = chl.Value();
    if (ach->NbFails() + ach->NbWarnings() == 0)
...
occ::handle<TopTools_HSequenceOfShape> TransferBRep::CheckedShapes(const Interface_CheckIterator& chl)
{
  occ::handle<TopTools_HSequenceOfShape> ls = new TopTools_HSequenceOfShape();
  for (chl.Start(); chl.More(); chl.Next())
  {
    const occ::handle<Interface_Check>& ach = chl.Value();
    if (ach->NbFails() + ach->NbWarnings() == 0)
