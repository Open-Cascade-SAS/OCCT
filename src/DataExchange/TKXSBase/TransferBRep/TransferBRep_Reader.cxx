...
void TransferBRep_Reader::TransferList(const occ::handle<TColStd_HSequenceOfTransient>& list,
                                       const Message_ProgressRange&                theProgress)
{
  if (!BeginTransfer())
    return;
...
occ::handle<TopTools_HSequenceOfShape> TransferBRep_Reader::Shapes() const
{
  return theShapes;
}
...
occ::handle<TColStd_HSequenceOfTransient> TransferBRep_Reader::Transients() const
{
  return theTransi;
}
