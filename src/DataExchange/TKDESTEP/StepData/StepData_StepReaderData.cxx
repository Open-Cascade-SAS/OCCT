...
void StepData_StepReaderData::cleanText(const occ::handle<TCollection_HAsciiString>& theVal) const
{
  ...
  for (int aStringInd = 1; aStringInd <= aResStringSize; ++aStringInd)
  {
    ...
    if (aChar == '\\' && aStringInd <= aResStringSize - 3) // can contains the control directive
    {
      ...
      {
        char aResChar =
          static_cast<char>(aResString.Value(aStringInd + 3) | 0x80);
                                                      ...
                                                      int&        val) const
{
  ...
  if (nump > 0 && nump <= NbParams(num))
  {
    ...
    {
      val =
        static_cast<int>(std::round(Interface_FileReaderData::Fastof(FP.CValue())));
...
void StepData_StepReaderData::SetEntityNumbers(const bool withmap)
{
  ...
  occ::handle<TColStd_HArray1OfInteger> indx;             // for EXPORT (if any)
  ...
  occ::handle<TColStd_HArray1OfInteger> subpile   =     // ... saves memory ...
  ...
  for (num = 1; num <= nbdirec; num++)
  {
    ...
    if (inds(nr) != num)
    {
      if (nbsubpil >= maxsubpil)
      {
        ...
        occ::handle<TColStd_HArray1OfInteger> newsubpil = new TColStd_HArray1OfInteger(1, maxsubpil);
