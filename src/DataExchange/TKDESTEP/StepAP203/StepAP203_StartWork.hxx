...
public:
  ...
  Standard_EXPORT void Init(const occ::handle<StepBasic_Action>& aActionAssignment_AssignedAction,
                            const occ::handle<StepAP203_HArray1OfWorkItem>& aItems);

  //! Returns field Items
  Standard_EXPORT occ::handle<StepAP203_HArray1OfWorkItem> Items() const;

  //! Set field Items
  Standard_EXPORT void SetItems(const occ::handle<StepAP203_HArray1OfWorkItem>& Items);

  DEFINE_STANDARD_RTTIEXT(StepAP203_StartWork, StepBasic_ActionAssignment)

private:
  occ::handle<StepAP203_HArray1OfWorkItem> theItems;
};
