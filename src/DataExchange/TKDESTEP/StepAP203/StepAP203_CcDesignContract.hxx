...
public:
  ...
  Standard_EXPORT void Init(const occ::handle<StepBasic_Contract>& aContractAssignment_AssignedContract,
                            const occ::handle<StepAP203_HArray1OfContractedItem>& aItems);

  //! Returns field Items
  Standard_EXPORT occ::handle<StepAP203_HArray1OfContractedItem> Items() const;

  //! Set field Items
  Standard_EXPORT void SetItems(const occ::handle<StepAP203_HArray1OfContractedItem>& Items);

  DEFINE_STANDARD_RTTIEXT(StepAP203_CcDesignContract, StepBasic_ContractAssignment)

private:
  occ::handle<StepAP203_HArray1OfContractedItem> theItems;
};
