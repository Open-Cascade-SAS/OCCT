...
public:
  ...
  Standard_EXPORT occ::handle<StepGeom_GeomRepContextAndGlobUnitAssCtxAndGlobUncertaintyAssCtx> Value() const;
  
  //! Computes the length, plane angle and solid angle conversion
  //! factor  .  Returns a status, 0 if OK
  Standard_EXPORT int ComputeFactors (const occ::handle<StepRepr_GlobalUnitAssignedContext>& aContext,
                                                   const StepData_Factors& theLocalFactors = StepData_Factors());
  ...
  Standard_EXPORT int ComputeTolerance (const occ::handle<StepRepr_GlobalUncertaintyAssignedContext>& aContext);
...
private:
  ...
  Standard_EXPORT bool SiUnitNameFactor (const occ::handle<StepBasic_SiUnit>& aSiUnit, double& val) const;
