  ...
  const occ::handle<StepVisual_StyledItem>&                  Override)
{
  ...
  occ::handle<StepVisual_HArray1OfPresentationStyleAssignment> Styles =
  ...
  occ::handle<StepData_StepModel>&                                             theStepModel)
{
  ...
  occ::handle<StepRepr_HArray1OfRepresentationItem> elems =
...
bool STEPConstruct_Styles::LoadInvisStyles(
  occ::handle<TColStd_HSequenceOfTransient>& theInvStyles) const
{
  occ::handle<Interface_InterfaceModel> model         = Model();
  int                 nb            = model->NbEntities();
  ...
  const bool                   isForNAUO) const
{
  ...
  if (!SurfCol.IsNull())
  {
    ...
    occ::handle<StepVisual_HArray1OfFillStyleSelect> FASSs =
    ...
    occ::handle<StepVisual_HArray1OfSurfaceStyleElementSelect> SSESs;
    if (aSSRWP.IsNull())
    {
  ...
  // general part
  occ::handle<StepVisual_HArray1OfPresentationStyleSelect> PSSs =
