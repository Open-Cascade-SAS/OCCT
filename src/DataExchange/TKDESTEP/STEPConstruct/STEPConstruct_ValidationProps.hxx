...
public:
  ...
  Standard_EXPORT bool AddProp (const TopoDS_Shape& Shape, const occ::handle<StepRepr_RepresentationItem>& Prop, const char* const Descr, const bool instance = false);
  
  //! General method for adding (writing) a validation property
  //! for shape which should be already mapped on writing itself.
  //! It takes target and Context entities which correspond to shape
  //! Returns True if success, False in case of fail
  Standard_EXPORT bool AddProp (const StepRepr_CharacterizedDefinition& target, const occ::handle<StepRepr_RepresentationContext>& Context, const occ::handle<StepRepr_RepresentationItem>& Prop, const char* const Descr);
  ...
  Standard_EXPORT bool AddCentroid (const TopoDS_Shape& Shape, const gp_Pnt& Pnt, const bool instance = false);
  
  //! Finds target STEP entity to which validation props should
  //! be assigned, and corresponding context, starting from shape
  //! Returns True if success, False in case of fail
  Standard_EXPORT bool FindTarget (const TopoDS_Shape& S, StepRepr_CharacterizedDefinition& target, occ::handle<StepRepr_RepresentationContext>& Context, const bool instance = false);
  
  //! Searches for entities of the type PropertyDefinitionRepresentation
  //! in the model and fills the sequence by them
  Standard_EXPORT bool LoadProps (TColStd_SequenceOfTransient& seq) const;
  
  //! Returns CDSR associated with given PpD or NULL if not found
  //! (when, try GetPropSDR)
  Standard_EXPORT occ::handle<StepRepr_NextAssemblyUsageOccurrence> GetPropNAUO (const occ::handle<StepRepr_PropertyDefinition>& PD) const;
  
  //! Returns SDR associated with given PpD or NULL if not found
  //! (when, try GetPropCDSR)
  Standard_EXPORT occ::handle<StepBasic_ProductDefinition> GetPropPD (const occ::handle<StepRepr_PropertyDefinition>& PD) const;
  
  //! Returns Shape associated with given SDR or Null Shape
  //! if not found
  Standard_EXPORT TopoDS_Shape GetPropShape (const occ::handle<StepBasic_ProductDefinition>& ProdDef) const;
  ...
  //! is area or volume
  Standard_EXPORT bool GetPropReal (const occ::handle<StepRepr_RepresentationItem>& item,
                                                double& Val,
                                                bool& isArea,
                                                const StepData_Factors& theLocalFactors = StepData_Factors()) const;
  
  //! Returns value of Centriod property (or False if it is not)
  Standard_EXPORT bool GetPropPnt (const occ::handle<StepRepr_RepresentationItem>& item,
