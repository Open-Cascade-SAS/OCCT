...
public:
  ...
  Standard_EXPORT void Init(const occ::handle<TCollection_HAsciiString>& aRepresentationItem_Name,
                            const occ::handle<StepShape_HArray1OfConnectedEdgeSet>& aEbwmBoundary);

  //! Returns field EbwmBoundary
  Standard_EXPORT occ::handle<StepShape_HArray1OfConnectedEdgeSet> EbwmBoundary() const;

  //! Set field EbwmBoundary
  Standard_EXPORT void SetEbwmBoundary(
    const occ::handle<StepShape_HArray1OfConnectedEdgeSet>& EbwmBoundary);

  DEFINE_STANDARD_RTTIEXT(StepShape_EdgeBasedWireframeModel, StepGeom_GeometricRepresentationItem)

private:
  occ::handle<StepShape_HArray1OfConnectedEdgeSet> theEbwmBoundary;
};
