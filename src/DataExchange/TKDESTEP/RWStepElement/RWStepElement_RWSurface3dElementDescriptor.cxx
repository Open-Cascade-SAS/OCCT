  ...
  const occ::handle<StepElement_Surface3dElementDescriptor>& ent) const
{
  ...
  occ::handle<StepElement_HArray1OfHSequenceOfSurfaceElementPurposeMember> aPurpose;
  int                                                    sub3 = 0;
  if (data->ReadSubList(num, 3, "purpose", ach, sub3))
  {
    ...
    {
      occ::handle<StepElement_HSequenceOfSurfaceElementPurposeMember> HSSEPM =
  ...
  const occ::handle<StepElement_Surface3dElementDescriptor>& ent) const
{
  ...
  for (int i2 = 1; i2 <= ent->Purpose()->Length(); i2++)
  {
    SW.NewLine(false);
    SW.OpenSub();
    occ::handle<StepElement_HSequenceOfSurfaceElementPurposeMember> HSSEPM = ent->Purpose()->Value(i2);
  ...
  Interface_EntityIterator&) const
{
  ...
  /*  CKY  17JUN04 : content is made of STRINGS or ENUMS , no entity !
    for (int i1=1; i1 <= ent->Purpose()->Length(); i1++ ) {
      occ::handle<StepElement_HSequenceOfSurfaceElementPurposeMember> HSSEPM = ent->Purpose()->Value(i1);
