...
public:
  ...
  Standard_EXPORT void Init(const occ::handle<TCollection_HAsciiString>&                        aName,
                            const occ::handle<StepVisual_HArray1OfPresentationStyleAssignment>& aStyles,
                            const occ::handle<Standard_Transient>&                              aItem,
                            const occ::handle<StepVisual_StyledItem>&                  aOverRiddenStyle,
                            const occ::handle<StepVisual_HArray1OfStyleContextSelect>& aStyleContext);

  Standard_EXPORT void SetStyleContext(
    const occ::handle<StepVisual_HArray1OfStyleContextSelect>& aStyleContext);

  Standard_EXPORT occ::handle<StepVisual_HArray1OfStyleContextSelect> StyleContext() const;
...
private:
  occ::handle<StepVisual_HArray1OfStyleContextSelect> styleContext;
};
