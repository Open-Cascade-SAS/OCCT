...
public:
  ...
  Standard_EXPORT void Init(const occ::handle<TCollection_HAsciiString>& theRepresentationItem_Name,
                            const occ::handle<StepVisual_HArray1OfTessellatedEdgeOrVertex>& theItems,
                            const bool                 theHasGeometricModelLink,
                            const StepVisual_PathOrCompositeCurve& theGeometricModelLink);

  //! Returns field Items
  Standard_EXPORT occ::handle<StepVisual_HArray1OfTessellatedEdgeOrVertex> Items() const;
...
private:
  occ::handle<StepVisual_HArray1OfTessellatedEdgeOrVertex> myItems;
