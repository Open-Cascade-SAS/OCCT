...
static void SetNodes(const occ::handle<Poly_Triangulation>& theMesh,
                     occ::handle<TColgp_HArray1OfXYZ>&      theNodes,
                     const int            theNumPnindex,
                     occ::handle<TColStd_HArray1OfInteger>& thePnindices,
...
// ============================================================================
static void SetTriangles(const occ::handle<Poly_Triangulation>&         theMesh,
                         const occ::handle<TColStd_HArray2OfInteger>    theTriangles,
                         const int                    theTrianStripsNum,
                         const occ::handle<TColStd_HArray1OfTransient>& theTrianStrips,
                         const int                    theTrianFansNum,
                         const occ::handle<TColStd_HArray1OfTransient>& theTrianFans)
{
  ...
  else
  {
    ...
    for (int aTrianStripIndex = 1; aTrianStripIndex <= theTrianStripsNum;
         ++aTrianStripIndex)
    {
      occ::handle<TColStd_HArray1OfInteger> aTriangleStrip =
        occ::down_cast<TColStd_HArray1OfInteger>(theTrianStrips->Value(aTrianStripIndex));
    ...
    for (int aTrianFanIndex = 1; aTrianFanIndex <= theTrianFansNum; ++aTrianFanIndex)
    {
      occ::handle<TColStd_HArray1OfInteger> aTriangleFan =
        occ::down_cast<TColStd_HArray1OfInteger>(theTrianFans->Value(aTrianFanIndex));
...
static void GetSimpleFaceElements(Type                              theFace,
                                  ...
                                  occ::handle<TColStd_HArray2OfInteger>& theTriangles,
                                  int&                 thePnIndNb,
                                  int&                 theNormNb,
                                  int&                 theTriNb,
                                  occ::handle<TColStd_HArray1OfInteger>& thePnindices)
...
static void GetComplexFaceElements(Type                                theFace,
                                   occ::handle<TColgp_HArray1OfXYZ>&        theNodes,
                                   occ::handle<TColStd_HArray2OfReal>&      theNormals,
                                   occ::handle<TColStd_HArray1OfTransient>& theTriangleStrips,
                                   occ::handle<TColStd_HArray1OfTransient>& theTriangleFans,
                                   int&                   thePnIndNb,
                                   int&                   theNormNb,
                                   int&                   theTriStripsNb,
                                   int&                   theTriFansNb,
  ...
  const StepData_Factors&                   theLocalFactors)
{
  ...
  occ::handle<TColgp_HArray1OfXYZ>      aNodes;
  occ::handle<TColStd_HArray2OfReal>    aNormals;
  occ::handle<TColStd_HArray2OfInteger> aTriangles;
  int                 aNumPnindex = 0;
  int                 aNormNum    = 0;
  int                 aTrianNum   = 0;
  occ::handle<TColStd_HArray1OfInteger> aPnindices;

  occ::handle<TColStd_HArray1OfTransient> aTriaStrips;
  occ::handle<TColStd_HArray1OfTransient> aTriaFans;
  int                   aTrianStripsNum = 0;
  int                   aTrianFansNum   = 0;

  if (theTI->IsKind(STANDARD_TYPE(StepVisual_TriangulatedFace)))
  {
    occ::handle<StepVisual_TriangulatedFace> aTF = occ::down_cast<StepVisual_TriangulatedFace>(theTI);
  ...
  else
  {
    ...
    for (int aTrianStripIndex = 1; aTrianStripIndex <= aTrianStripsNum;
         ++aTrianStripIndex)
    {
      occ::handle<TColStd_HArray1OfInteger> aTriangleStrip =
        occ::down_cast<TColStd_HArray1OfInteger>(aTriaStrips->Value(aTrianStripIndex));
      for (int anIndex = 3; anIndex <= aTriangleStrip->Length(); anIndex += 2)
      {
        if (aTriangleStrip->Value(anIndex) != aTriangleStrip->Value(anIndex - 2)
            && aTriangleStrip->Value(anIndex) != aTriangleStrip->Value(anIndex - 1))
    ...
    {
      occ::handle<TColStd_HArray1OfInteger> aTriangleFan =
        occ::down_cast<TColStd_HArray1OfInteger>(aTriaFans->Value(aTrianFanIndex));
                                      ...
                                      const StepData_Factors&              theLocalFactors)
{
  ...
  occ::handle<Geom_Surface> aGeomSurface = StepToGeom::MakeSurface(aStepGeomSurface, theLocalFactors);
  ...
  {
    occ::handle<Geom_Surface> periodicSurf = ShapeAlgo::AlgoContainer()->ConvertToPeriodic(aGeomSurface);
  ...
  for (int aBoundIndex = 1; aBoundIndex <= theFaceSurface->NbBounds(); ++aBoundIndex)
  {
    ...
    if (aFaceLoop->IsKind(STANDARD_TYPE(StepShape_VertexLoop)))
    {
      ...
      occ::handle<StepShape_VertexLoop> aVertexLoop = occ::down_cast<StepShape_VertexLoop>(aFaceLoop);
