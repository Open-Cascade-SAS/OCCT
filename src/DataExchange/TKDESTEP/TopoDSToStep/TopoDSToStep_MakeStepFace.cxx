                                     ...
                                     const StepData_Factors&               theLocalFactors)
{
  ...
  try
  {
    ...
    if (!aTool.Faceted() && aTool.PCurveMode() != 0)
    {
      ...
      for (; Ex.More(); Ex.Next())
      {
        ...
        if (aTool.IsBound(E))
        {
          ...
          occ::handle<StepGeom_HArray1OfPcurveOrSurface> aGeom = C1pms->AssociatedGeometry();
          if (aGeom.IsNull())
    ...
    if (nbWires)
    {
      occ::handle<StepShape_HArray1OfFaceBound> aBounds = new StepShape_HArray1OfFaceBound(1, nbWires);
