  ...
  const occ::handle<StepVisual_TessellatedCurveSet>& ent) const
{
  ...
  int                                          nsub2;
  NCollection_Handle<NCollection_Vector<occ::handle<TColStd_HSequenceOfInteger>>> aCurves =
  ...
  if (data->ReadSubList(num, 3, "curves", ach, nsub2))
  {
    ...
    for (int i = 1; i <= nb2; i++)
    {
      occ::handle<TColStd_HSequenceOfInteger> aCurve = new TColStd_HSequenceOfInteger;
      int                   nsub3;
  ...
  const occ::handle<StepVisual_TessellatedCurveSet>& ent) const
{
  ...
  {
    occ::handle<TColStd_HSequenceOfInteger> aCurve = ent->Curves()->Value(curveIt);
    SW.OpenSub();
