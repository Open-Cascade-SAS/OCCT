...
bool  STEPControl_ActorRead::Recognize
  (const occ::handle<Standard_Transient>& start)
{
  if (start.IsNull()) return false;

  if (start->IsKind(STANDARD_TYPE(StepBasic_ProductDefinition))) return true;

  if (start->IsKind(STANDARD_TYPE(StepRepr_NextAssemblyUsageOccurrence))) return true;
  ...
  if(!aProdMode)
    if(start->IsKind(STANDARD_TYPE(StepShape_ShapeDefinitionRepresentation))) return true;
  ...
  if (start->IsKind(STANDARD_TYPE(StepShape_FacetedBrep))) return true;
  if (start->IsKind(STANDARD_TYPE(StepShape_BrepWithVoids))) return true;
  if (start->IsKind(STANDARD_TYPE(StepShape_ManifoldSolidBrep))) return true;
  if (start->IsKind(STANDARD_TYPE(StepShape_ShellBasedSurfaceModel))) return true;
  if (start->IsKind(STANDARD_TYPE(StepShape_FacetedBrepAndBrepWithVoids))) return true;
  if (start->IsKind(STANDARD_TYPE(StepShape_GeometricSet))) return true;
  if (start->IsKind(STANDARD_TYPE(StepRepr_MappedItem))) return true;
  if (start->IsKind(STANDARD_TYPE(StepShape_FaceSurface))) return true;
  if (start->IsKind(STANDARD_TYPE(StepShape_EdgeBasedWireframeModel))) return true;
  if (start->IsKind(STANDARD_TYPE(StepShape_FaceBasedSurfaceModel))) return true;
  if (aCanReadTessGeom && start->IsKind(STANDARD_TYPE(StepVisual_TessellatedFace))) return true;
  if (aCanReadTessGeom && start->IsKind(STANDARD_TYPE(StepVisual_TessellatedSurfaceSet))) return true;
  if (aCanReadTessGeom && start->IsKind(STANDARD_TYPE(StepVisual_TessellatedShell))) return true;
  if (aCanReadTessGeom && start->IsKind(STANDARD_TYPE(StepVisual_TessellatedSolid))) return true;
  if (aCanReadTessGeom && start->IsKind(STANDARD_TYPE(StepVisual_TessellatedShapeRepresentation))) return true;
...
//  On prend son contenu
    ...
    if (Recognize(und->Rep1()) || Recognize(und->Rep2())) return true;
    return false;
  }
...
{  
  ...
  occ::handle<StepData_StepModel> aStepModel = Handle(StepData_StepModel)::DownCast ( TP->Model() );
...
static void getListSDR(const occ::handle<StepRepr_ShapeAspect>& sa,
                       occ::handle<TColStd_HSequenceOfTransient>& listSDR,
                       const occ::handle<Transfer_TransientProcess>& TP)
{
  const Interface_Graph& graph = TP->Graph();
...
static void getSDR(const occ::handle<StepRepr_ProductDefinitionShape>& PDS,
                   occ::handle<TColStd_HSequenceOfTransient>& listSDR,
                   occ::handle<TColStd_HSequenceOfTransient>& listNAUO,
                   occ::handle<TColStd_HSequenceOfTransient>& listSDRAspect,
                   const occ::handle<Transfer_TransientProcess>& TP)
{
  ...
  for (subs4.Start(); subs4.More(); subs4.Next()) {
    ...
    if ( ! sdr.IsNull() ) {
      ...
      if ( !rep.IsNull())  {
        ...
        if ( ICS == 1 ) // translate all SDRs
          ...
          int iDiff = Abs ( FindShapeReprType(rep) - ICS );
    ...
    occ::handle<StepRepr_ShapeAspect> sa = 
      occ::down_cast<StepRepr_ShapeAspect>(subs4.Value());
    if ( ! sa.IsNull() ) {
...

{
  ...
  occ::handle<TColStd_HSequenceOfTransient> listSDR = new TColStd_HSequenceOfTransient;
  occ::handle<TColStd_HSequenceOfTransient> listNAUO = new TColStd_HSequenceOfTransient;
  occ::handle<TColStd_HSequenceOfTransient> listSDRAspect = new TColStd_HSequenceOfTransient;
  const Interface_Graph& graph = TP->Graph();
  ...
  int readSRR = aStepModel->InternalParameters.ReadRelationship;
  
  int readConstructiveGeomRR = aStepModel->InternalParameters.ReadConstrRelation;
  // Flag indicating whether SDRs associated with the product`s main SDR
  ...
  for(int i=1; i <= listSDR->Length() && PS.More(); i++) {
    ...
    occ::handle<StepShape_ShapeRepresentation> rep =  occ::down_cast<StepShape_ShapeRepresentation>(sdr->UsedRepresentation());
    if ( rep.IsNull() )
      continue;
                    ...
                    const Message_ProgressRange& theProgress)
{
 occ::handle<TransferBRep_ShapeBinder> shbinder;
  ...
  if(IsDepend) {
    ...
    if(!PD.IsNull()) {
      binder = TP->Find(PD);
      if (binder.IsNull()) binder = TransferEntity(PD, TP, theLocalFactors, false, aPS.Next());
      theResult = TransferBRep::ShapeResult(binder);
      if (!theResult.IsNull()) {
        if (iatrsf) {
    ...
    const Message_ProgressRange& theProgress)
{
  ...
  for (int i = 1; i <= nb && aPS.More(); i ++)
  {
    ...
    if (theUseTrsf)
    {
      if (anitem->IsKind(STANDARD_TYPE(StepGeom_Axis2Placement3d)))
      {
        ...
        if (!subs3.More())
        {
          occ::handle<StepGeom_Axis2Placement3d> aCS = occ::down_cast<StepGeom_Axis2Placement3d>(anitem);
  ...
  if (!isManifold) {
    ...
    occ::handle<Standard_Transient> info;
                    ...
                    const Message_ProgressRange& theProgress)
{
  ...
  occ::handle<Transfer_Binder> binder;
  bool isBound = false;
  if (!TP->IsBound(rep)) binder = TransferEntity(rep, TP, theLocalFactors, isBound, false, theProgress);
    ...
    const Message_ProgressRange& theProgress)
{
  ...
  for (int i = 1; i <= 2 && aPS.More(); i++)
  {
    ...
    occ::handle<StepShape_ShapeRepresentation> anitem = occ::down_cast<StepShape_ShapeRepresentation>(anitemt);
    occ::handle<Transfer_Binder> binder;
    bool isBound = false;
    ...
    const StepData_Factors& theLocalFactors)
{
  ...
  for (int i = 1; i <= 2; i ++) 
  {
    occ::handle<StepRepr_ConstructiveGeometryRepresentation> aCRepr = 
      occ::down_cast<StepRepr_ConstructiveGeometryRepresentation>(i == 1 ? theCGRR->Rep1() : theCGRR->Rep2() );
    if(aCRepr.IsNull())
      continue;
    ...
    for( ; j <= aCRepr->NbItems(); j++ )
    {
      ...
      if( !aStepAxis.IsNull())
      {
        occ::handle<Geom_Axis2Placement> anAxis = StepToGeom::MakeAxis2Placement (aStepAxis, aLocalFactors);
        if(anAxis.IsNull())
          continue;
                                             ...
                                             const occ::handle<Transfer_TransientProcess>& TP)
{
  ...
  for (subs7.Start(); ! PDSA.IsNull() && subs7.More(); subs7.Next()) {
    ...
    for (subs8.Start(); subs8.More(); subs8.Next()) {
      ...
      if ( i > repA->NbItems() ) IsSDRaspect=false;
    }
  }
  return IsSDRaspect;
}
                    ...
                    const Message_ProgressRange& theProgress)
{
  ...
  occ::handle<StepShape_ShapeRepresentation> maprep =  occ::handle<StepShape_ShapeRepresentation>::
    DownCast(mapit->MappingSource()->MappedRepresentation());
  bool isBound = false;
  Message_ProgressScope aPSRoot(theProgress, NULL, 2);
  occ::handle<Transfer_Binder> binder = TP->Find(maprep);
  if (binder.IsNull())    binder = TransferEntity(maprep, TP, theLocalFactors, isBound, false, theProgress);
  ...
  // translate possible shapes related by SRRs, which corresponds to
  // way of writing hybrid models in AP203 since 1998, and AP209
  int aReadSRR = Interface_Static::IVal("read.step.shape.relationship");
  int aReadConstructiveGeomRR = Interface_Static::IVal("read.step.constructivegeom.relationship");
  if (aReadSRR)
  {
    ...
    TopoDS_Shape aNewResult = TransferRelatedSRR(TP, maprep, false, aReadConstructiveGeomRR, theLocalFactors, aCund, aPS);
    if (!aNewResult.IsNull())
    {
      aResult = aNewResult;
    }
...
void  STEPControl_ActorRead::ResetUnits (occ::handle<StepData_StepModel>& theModel, StepData_Factors& theLocalFactors)
{
...
{
  Trsf = gp_Trsf();  // reinit
  if ( Origin.IsNull() || Target.IsNull() ) return false;
                                                      ...
                                                      const StepData_Factors& theLocalFactors)
{
  ...
  if ( Ax1.IsNull() || Ax2.IsNull() ) return false;
...
//=======================================================================
TopoDS_Shape STEPControl_ActorRead::TransferRelatedSRR(const occ::handle<Transfer_TransientProcess>& theTP,
                                                       const occ::handle<StepShape_ShapeRepresentation>& theRep,
                                                       const bool theUseTrsf,
                                                       const int theReadConstructiveGeomRR,
                                                       const StepData_Factors& theLocalFactors,
