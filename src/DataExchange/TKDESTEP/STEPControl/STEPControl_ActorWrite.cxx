...
static bool hasGeometry(const TopoDS_Shape& theShape)
{
  ...
  else if (aType == TopAbs_EDGE)
  {
    ...
    while (itrc.More())
    {
      ...
      bool aHasGeometry              = (CR->IsCurve3D() && !CR->Curve3D().IsNull())
...
occ::handle<StepShape_NonManifoldSurfaceShapeRepresentation> STEPControl_ActorWrite::getNMSSRForGroup(
  const occ::handle<TopTools_HSequenceOfShape>& shapeGroup,
...
{
  occ::handle<ShapeProcess_ShapeContext> aContext = occ::down_cast<ShapeProcess_ShapeContext>(theInfo);
  if (aContext.IsNull())
    return;

  const NCollection_DataMap<TopoDS_Shape, TopoDS_Shape, TopTools_ShapeMapHasher>&           aMap = aContext->Map();
  NCollection_DataMap<TopoDS_Shape, TopoDS_Shape, TopTools_ShapeMapHasher>::Iterator aShapeShapeIt(aMap);
...
occ::handle<Transfer_Binder> STEPControl_ActorWrite::Transfer(const occ::handle<Transfer_Finder>& start,
                                                         const occ::handle<Transfer_FinderProcess>& FP,
                                                         const Message_ProgressRange& theProgress)
{
  ...
  occ::handle<TColStd_HSequenceOfTransient> roots   = myContext.GetRootsForPart(SDRTool);
...
static bool transferVertex(const occ::handle<Transfer_FinderProcess>&          FP,
                                       occ::handle<StepShape_HArray1OfGeometricSetSelect>& aGSS,
                                       ...
                                       const StepData_Factors& theLocalFactors)
{
  ...
  occ::handle<StepShape_VertexPoint> aVP = occ::down_cast<StepShape_VertexPoint>(aTool.Find(aShVrtx));
  if (aVP.IsNull())
    return IsDone;
  ...
  const Message_ProgressRange&                           theProgress)
{
  ...
  if (isNMMode && !GroupMode() && theShape.ShapeType() == TopAbs_COMPOUND)
  {
    ...
    occ::handle<TopTools_HSequenceOfShape> RepItemSeq = new TopTools_HSequenceOfShape();
    // Non-manifold group to pass into TransferShape with each shape from RepItemSeq
    occ::handle<TopTools_HSequenceOfShape> NonManifoldGroup = new TopTools_HSequenceOfShape();
  ...
  occ::handle<TColStd_HSequenceOfTransient> ItemSeq = new TColStd_HSequenceOfTransient();
  ...
  if (ap == 3 && nbs > 1)
  {
    ...
    {
      occ::handle<StepRepr_HArray1OfRepresentationItem> axis =
    ...
    for (; j <= items->Length(); j++)
    {
      ...
      occ::handle<StepRepr_HArray1OfRepresentationItem> repr1 =
  ...
  else
  {
    ...
    if (nC > 0)
    {
      occ::handle<StepRepr_HArray1OfRepresentationItem> itemsTess =
    ...
    else
    {
      ...
      occ::handle<StepRepr_HArray1OfRepresentationItem> oldItems = shapeRep->Items();
      occ::handle<StepRepr_HArray1OfRepresentationItem> newItems =
  ...
  const Message_ProgressRange&                           theProgress)
{
  ...
  {
    occ::handle<TransferBRep_ShapeMapper> subs = TransferBRep::ShapeMapper(FP, RepItemSeq->Value(i));
  ...
  const Message_ProgressRange&                           theProgress)
{
  ...
  occ::handle<TColStd_HSequenceOfTransient> roots = myContext.GetRootsForAssemblyLink(mkitem);
