...
public:
  ...
  Standard_EXPORT occ::handle<Transfer_Binder> TransferSubShape
                   (const occ::handle<Transfer_Finder>& start,
                    ...
                    const occ::handle<TopTools_HSequenceOfShape>& shapeGroup = NULL, 
...
private:
  ...
  //! (ssv; 13.11.2010)
  Standard_EXPORT occ::handle<StepShape_NonManifoldSurfaceShapeRepresentation> getNMSSRForGroup (const occ::handle<TopTools_HSequenceOfShape>& shapeGroup, const occ::handle<Transfer_FinderProcess>& FP, bool& isNMSSRCreated) const;
  
  //! bind already written shared faces to STEP entity for non-manifold
  Standard_EXPORT void mergeInfoForNM(const occ::handle<Transfer_FinderProcess>& theFP, const occ::handle<Standard_Transient> &theInfo) const;
