                                                  ...
                                                  const Message_ProgressRange& theProgress)
{
  ...
  const occ::handle<Transfer_TransientProcess> &TP = reader.WS()->TransferReader()->TransientProcess();
  int nb = aModel->NbEntities();

  occ::handle<TColStd_HSequenceOfTransient> SeqPDS = new TColStd_HSequenceOfTransient;
  ...
  for (i = 1; i <= ExtRefs.NbExternRefs() && aPSE.More(); i++)
  {
    ...
    if (!format.IsNull()) {
      static occ::handle<TCollection_HAsciiString> ap203 = new TCollection_HAsciiString("STEP AP203");
      static occ::handle<TCollection_HAsciiString> ap214 = new TCollection_HAsciiString("STEP AP214");
      if (!format->IsSameString(ap203, false) &&
        !format->IsSameString(ap214, false)) {
#ifdef OCCT_DEBUG
  ...
  const STEPCAFControl_DataMapOfPDExternFile &PDFileMap)
{
  ...
  for (it.Initialize(S); it.More() && !isAssembly; it.Next(), nbComponents++) {
    ...
    TopLoc_Location loc;
    Sub0.Location(loc);
    if (NewShapesMap.Contains(Sub0)) isAssembly = true;
  }
...
static void SetAssemblyComponentStyle(const occ::handle<Transfer_TransientProcess> &theTP,
                                      const occ::handle<XCAFDoc_ColorTool>& theCTool, 
                                      ...
                                      const StepData_Factors& theLocalFactors)
{
  ...
  occ::handle<StepVisual_Colour> aSurfCol, aBoundCol, aCurveCol, aRenderCol;
  double aRenderTransp;
  ...
  occ::handle<StepShape_ShapeRepresentation> aRepr = occ::down_cast<StepShape_ShapeRepresentation>(theStyle->ItemAP242 ().Value ());
  if (aRepr.IsNull())
    return;
  ...
  {
    occ::handle<StepRepr_ShapeRepresentationRelationshipWithTransformation> aRelation = Handle(StepRepr_ShapeRepresentationRelationshipWithTransformation)::DownCast (aSubs.Value ());
    ...
    {
      occ::handle<StepGeom_Axis2Placement3d> anAxp1 = occ::down_cast<StepGeom_Axis2Placement3d>(anItemTransf->TransformItem1 ());
      occ::handle<StepGeom_Axis2Placement3d> anAxp2 = occ::down_cast<StepGeom_Axis2Placement3d>(anItemTransf->TransformItem2 ());

      if(!anAxp1.IsNull() && !anAxp2.IsNull())
      {
        occ::handle<Geom_Axis2Placement> anOrig = StepToGeom::MakeAxis2Placement (anAxp1, theLocalFactors);
        occ::handle<Geom_Axis2Placement> aTarg = StepToGeom::MakeAxis2Placement (anAxp2, theLocalFactors);
...
static void SetStyle(const occ::handle<XSControl_WorkSession> &theWS, 
                     const XCAFDoc_DataMapOfShapeLabel& theMap, 
                     const occ::handle<XCAFDoc_ColorTool>& theCTool, 
                     const occ::handle<XCAFDoc_ShapeTool>& theSTool, 
                     const STEPConstruct_Styles& theStyles, 
                     const occ::handle<TColStd_HSequenceOfTransient>& theHSeqOfInvisStyle, 
                     ...
                     const StepData_Factors& theLocalFactors)
{
  ...
  if (occ::handle<StepVisual_OverRidingStyledItem> anOverridingStyle = Handle(StepVisual_OverRidingStyledItem)::DownCast (theStyle))
  {
    SetStyle (theWS, theMap, theCTool, theSTool, theStyles, theHSeqOfInvisStyle, anOverridingStyle->OverRiddenStyle (), theLocalFactors);
    if (occ::handle<StepVisual_ContextDependentOverRidingStyledItem> anAssemblyComponentStyle = Handle(StepVisual_ContextDependentOverRidingStyledItem)::DownCast (theStyle))
    {
                                    ...
                                    bool theIsRoot)
{
  ...
  for(; aSubs.More(); aSubs.Next())
  {
    occ::handle<StepVisual_OverRidingStyledItem> anOverRidingStyle = Handle(StepVisual_OverRidingStyledItem)::DownCast (aSubs.Value ());
                                                   ...
                                                   const StepData_Factors& theLocalFactors) const
{
  ...
  occ::handle<TColStd_HSequenceOfTransient> aHSeqOfInvisStyle = new TColStd_HSequenceOfTransient;
  Styles.LoadInvisStyles(aHSeqOfInvisStyle);

  occ::handle<XCAFDoc_ColorTool> CTool = XCAFDoc_DocumentTool::ColorTool(Doc->Main());
  if (CTool.IsNull()) return false;
  occ::handle<XCAFDoc_ShapeTool> STool = XCAFDoc_DocumentTool::ShapeTool(Doc->Main());
  if (STool.IsNull()) return false;
...
static TDF_Label GetLabelFromPD(const occ::handle<StepBasic_ProductDefinition> &PD,
  const occ::handle<XCAFDoc_ShapeTool> &STool,
...
TDF_Label STEPCAFControl_Reader::FindInstance(const occ::handle<StepRepr_NextAssemblyUsageOccurrence> &NAUO,
  const occ::handle<XCAFDoc_ShapeTool> &STool,
  ...
  for (int i = 1; i <= nb; i++) {
    ...
    if (enti->DynamicType() == tNAUO) {
      ...
      for (subs.Start(); subs.More(); subs.Next()) {
        ...
        occ::handle<StepBasic_ProductDefinitionRelationship> PDR = PDS->Definition().ProductDefinitionRelationship();
    ...
    if (enti->DynamicType() == tPD || enti->DynamicType() == tPDWAD) {
      ...
      occ::handle<StepBasic_Product> Prod = (!PD->Formation().IsNull() ? PD->Formation()->OfProduct() : NULL);
      if (Prod.IsNull())
  ...
  for (int i = 1; i <= props.Length(); i++) {
    ...
    for (subs.Start(); L.IsNull() && subs.More(); subs.Next()) {
      if (subs.Value()->IsKind(STANDARD_TYPE(StepRepr_ProductDefinitionShape))) {
        occ::handle<StepRepr_ProductDefinitionShape> PDS = occ::down_cast<StepRepr_ProductDefinitionShape>(subs.Value());
        if (PDS.IsNull()) continue;
                                                   ...
                                                   const occ::handle<TDocStd_Document>& Doc) const
{
  ...
  for (int i = 1; i <= nb; i++) {
    ...
    for (int j = 1; j <= SVPLA->NbAssignedItems(); j++) {
      ...
      TDF_Label shL;
      if (!STool->Search(S, shL, true, true, true)) continue;
...
static bool findNextSHUOlevel(const occ::handle<XSControl_WorkSession> &WS,
...
//purpose  : auxiliary
//=======================================================================
static TDF_Label setSHUOintoDoc(const occ::handle<XSControl_WorkSession> &WS,
  const occ::handle<StepRepr_SpecifiedHigherUsageOccurrence>& SHUO,
  const occ::handle<XCAFDoc_ShapeTool>& STool,
  const STEPCAFControl_DataMapOfPDExternFile &PDFileMap,
  const XCAFDoc_DataMapOfShapeLabel &ShapeLabelMap)
                                                  ...
                                                  const STEPCAFControl_DataMapOfPDExternFile& PDFileMap) const
{
  ...
  for (int i = 1; i <= nb; i++) {
    ...
    occ::handle<StepVisual_Colour> SurfCol, BoundCol, CurveCol, RenderCol;
    double RenderTransp;
    // check if it is component style
    ...
    for (subs.Start(); subs.More(); subs.Next()) {
      ...
      occ::handle<StepRepr_SpecifiedHigherUsageOccurrence> SHUO =
        occ::down_cast<StepRepr_SpecifiedHigherUsageOccurrence>(aCharDef.ProductDefinitionRelationship());
      if (SHUO.IsNull())
        continue;
...
//purpose  : 
//=======================================================================
static bool GetMassConversionFactor(const occ::handle<StepBasic_NamedUnit>& NU,
  double& afact)
{
  afact = 1.;
  if (!NU->IsKind(STANDARD_TYPE(StepBasic_ConversionBasedUnitAndMassUnit))) return false;
  occ::handle<StepBasic_ConversionBasedUnitAndMassUnit> CBUMU =
    occ::down_cast<StepBasic_ConversionBasedUnitAndMassUnit>(NU);
  occ::handle<StepBasic_MeasureWithUnit> MWUCBU = CBUMU->ConversionFactor();
...
occ::handle<Poly_Triangulation> createMesh(const occ::handle<StepVisual_ComplexTriangulatedSurfaceSet>& theTriangulatedSufaceSet,
                                      const double theFact)
{
  occ::handle<StepVisual_CoordinatesList> aCoords = theTriangulatedSufaceSet->Coordinates();
  occ::handle<TColgp_HArray1OfXYZ> aNodes = aCoords->Points();
  occ::handle<TColStd_HArray1OfTransient> aTriaStrips = theTriangulatedSufaceSet->TriangleStrips();
  occ::handle<TColStd_HArray1OfTransient> aTriaFans = theTriangulatedSufaceSet->TriangleFans();
  ...
  {
    occ::handle<TColStd_HArray1OfInteger> aTriangleStrip = occ::down_cast<TColStd_HArray1OfInteger>(aTriaStrips->Value(i));
  ...
  {
    occ::handle<TColStd_HArray1OfInteger> aTriangleFan = occ::down_cast<TColStd_HArray1OfInteger>(aTriaFans->Value(i));
    aNbTriaFans += aTriangleFan->Length() - 2;
  }

  occ::handle<Poly_Triangulation> aMesh = new Poly_Triangulation(aNbPairs, aNbTriaStrips + aNbTriaFans, aHasUVNodes, aHasNormals);
  ...
  if (aHasNormals)
  {
    occ::handle<TColStd_HArray2OfReal> aNormals = theTriangulatedSufaceSet->Normals();
    gp_XYZ aNorm;
                                     ...
                                     const StepData_Factors& theLocalFactors)
{
  ...
  occ::handle<StepVisual_AnnotationOccurrence> anAO;
  ...
  {
    anAO = occ::down_cast<StepVisual_AnnotationOccurrence>(thePresentEntity);
    if (!anAO.IsNull())
    {
  ...
  for (int i = 0; i < anAnnotations.Length(); i++)
  {
    ...
    else
    {
      ...
      if (aRepresentationItem.IsNull())
        continue;
      occ::handle<StepVisual_TessellatedGeometricSet> aTessSet = occ::down_cast<StepVisual_TessellatedGeometricSet>(aRepresentationItem);
      if (aTessSet.IsNull())
        continue;
      gp_Trsf aTransf;
      if (aTessSet->IsKind(STANDARD_TYPE(StepVisual_RepositionedTessellatedGeometricSet)))
      {
        ...
        occ::handle<Geom_Axis2Placement> aLocation = StepToGeom::MakeAxis2Placement(aRTGS->Location(), theLocalFactors);
        if (!aLocation.IsNull())
        {
      ...
      for (int j = 1; j <= aNbItems; j++)
      {
        ...
        {
          occ::handle<StepVisual_TessellatedCurveSet> aTessCurve = occ::down_cast<StepVisual_TessellatedCurveSet>(aTessItem);
          ...
          occ::handle<TColgp_HArray1OfXYZ> aPoints = aCoordList->Points();
          ...
          {
            occ::handle<TColStd_HSequenceOfInteger> anIndexes = aCurves->Value(k);
            TopoDS_Wire aCurW;
            aB.MakeWire(aCurW);
        ...
        {
          occ::handle<StepVisual_ComplexTriangulatedSurfaceSet> aTessSurfSet = occ::down_cast<StepVisual_ComplexTriangulatedSurfaceSet>(aTessItem);
  ...
  const StepData_Factors& theLocalFactors)
{
  ...
  if (isHasPlane) {
    if (theDimObject->IsKind(STANDARD_TYPE(XCAFDimTolObjects_DimensionObject))) {
      ...
      occ::handle<TColgp_HArray1OfPnt> aPnts = new TColgp_HArray1OfPnt(1, 1);
  ...
  const StepData_Factors& theLocalFactors)
{
  ...
  for (Interface_EntityIterator anIt = aGraph.Sharings(theGDT); aSDR.IsNull() && anIt.More(); anIt.Next()) {
    ...
    if (!aDCR.IsNull())
      aSDR = !aDCR.IsNull() ? aDCR->Representation() : occ::down_cast<StepShape_ShapeDimensionRepresentation>(anEnt);
  }
  if (!aSDR.IsNull())
  {
    XSAlgo::AlgoContainer()->PrepareForTransfer();
  ...
  if (theGDT->IsKind(STANDARD_TYPE(StepShape_DimensionalSize))) {
    ...
    occ::handle<StepRepr_DerivedShapeAspect> aDSA = occ::down_cast<StepRepr_DerivedShapeAspect>(aDim->AppliesTo());
    if (aDSA.IsNull())
      return;
    ...
    if (aGISU.IsNull() || aGISU->NbIdentifiedItem() == 0)
      return;
    occ::handle<StepGeom_CartesianPoint> aPoint = occ::down_cast<StepGeom_CartesianPoint>(aGISU->IdentifiedItem()->Value(1));
    if (aPoint.IsNull()) {
      // try Axis2Placement3d.location instead of CartesianPoint
      occ::handle<StepGeom_Axis2Placement3d> anA2P3D =
        occ::down_cast<StepGeom_Axis2Placement3d>(aGISU->IdentifiedItem()->Value(1));
      if (anA2P3D.IsNull())
        return;
    ...
    // retrieve derived geometry
    occ::handle<StepShape_DimensionalLocation> aDim = occ::down_cast<StepShape_DimensionalLocation>(theGDT);
    occ::handle<StepRepr_DerivedShapeAspect> aDSA1 = occ::down_cast<StepRepr_DerivedShapeAspect>(aDim->RelatingShapeAspect());
    occ::handle<StepRepr_DerivedShapeAspect> aDSA2 = occ::down_cast<StepRepr_DerivedShapeAspect>(aDim->RelatedShapeAspect());
    ...
    if (!aGISU1.IsNull() && aGISU1->NbIdentifiedItem() > 0) {
      occ::handle<StepGeom_CartesianPoint> aPoint = occ::down_cast<StepGeom_CartesianPoint>(aGISU1->IdentifiedItem()->Value(1));
      if (aPoint.IsNull()) {
        // try Axis2Placement3d.location instead of CartesianPoint
        occ::handle<StepGeom_Axis2Placement3d> anA2P3D =
          occ::down_cast<StepGeom_Axis2Placement3d>(aGISU1->IdentifiedItem()->Value(1));
        if (!anA2P3D.IsNull())
          aPoint = anA2P3D->Location();
      }
      if (!aPoint.IsNull()) {
    ...
    if (!aGISU2.IsNull() && aGISU2->NbIdentifiedItem() > 0) {
      occ::handle<StepGeom_CartesianPoint> aPoint = occ::down_cast<StepGeom_CartesianPoint>(aGISU2->IdentifiedItem()->Value(1));
      if (aPoint.IsNull()) {
        // try Axis2Placement3d.location instead of CartesianPoint
        occ::handle<StepGeom_Axis2Placement3d> anA2P3D =
          occ::down_cast<StepGeom_Axis2Placement3d>(aGISU2->IdentifiedItem()->Value(1));
        if (!anA2P3D.IsNull())
          aPoint = anA2P3D->Location();
      }
      if (!aPoint.IsNull()) {
...
{
  if (GTWDR.IsNull()) return false;
  occ::handle<StepDimTol_HArray1OfDatumSystemOrReference> aHADSOR = GTWDR->DatumSystemAP242();
  if (aHADSOR.IsNull())
  {
    return false;
  }
  ...
  for(NCollection_List<occ::handle<StepDimTol_Datum>>::Iterator anIt(aDatumList); anIt.More(); anIt.Next())
  {
    ...
    for (subs4.Start(); subs4.More(); subs4.Next()) {
      ...
      TDF_Label shL;
      if (!STool->Search(aSh, shL, true, true, true)) continue;
...
bool STEPCAFControl_Reader::setDatumToXCAF(const occ::handle<StepDimTol_Datum>& theDat,
  ...
  const StepData_Factors& theLocalFactors)
{
  ...
  for (anIterD.Start(); anIterD.More(); anIterD.Next()) {
    occ::handle<StepRepr_ShapeAspectRelationship> aSAR = occ::down_cast<StepRepr_ShapeAspectRelationship>(anIterD.Value());
    if (aSAR.IsNull() || aSAR->RelatingShapeAspect().IsNull())
      continue;
    collectShapeAspect(aSAR->RelatingShapeAspect(), theWS, aSAs);
    occ::handle<StepDimTol_DatumFeature> aDF = occ::down_cast<StepDimTol_DatumFeature>(aSAR->RelatingShapeAspect());
  ...
  for (int i = 1; i <= aSAs.Length(); i++) {
    ...
    for (; anIter.More(); anIter.Next()) {
      occ::handle<StepAP242_GeometricItemSpecificUsage> aGISU = occ::down_cast<StepAP242_GeometricItemSpecificUsage>(anIter.Value());
      if (aGISU.IsNull())
        continue;
  ...
  for (int i = 1; i <= aSAs.Length(); i++) {
    occ::handle<StepDimTol_PlacedDatumTargetFeature> aDT = occ::down_cast<StepDimTol_PlacedDatumTargetFeature>(aSAs.Value(i));
    if (aDT.IsNull())
      continue;
    ...
    if (!aRelationship.IsNull()) {
      ...
      for (; aSAIter.More(); aSAIter.Next()) {
        occ::handle<StepAP242_GeometricItemSpecificUsage> aGISU = occ::down_cast<StepAP242_GeometricItemSpecificUsage>(aSAIter.Value());
        if (aGISU.IsNull())
          continue;
        for (int j = 1; j <= aGISU->NbIdentifiedItem(); j++) {
          TDF_Label aShapeL = getShapeLabel(aGISU->IdentifiedItemValue(j), theWS, aSTool);
    ...
    if (aType != XCAFDimTolObjects_DatumTargetType_Area && !isValidDT) {
      ...
      for (aDTIter.Start(); aDTIter.More(); aDTIter.Next()) {
        occ::handle<StepAP242_GeometricItemSpecificUsage> aGISU = occ::down_cast<StepAP242_GeometricItemSpecificUsage>(aDTIter.Value());
        if (aGISU.IsNull())
          continue;
        for (int j = 1; j <= aGISU->NbIdentifiedItem(); j++) {
          TDF_Label aShapeL = getShapeLabel(aGISU->IdentifiedItemValue(j), theWS, aSTool);
          ...
          if (!aSRWP.IsNull()) {
            ...
            for (int j = aSRWP->Items()->Lower(); j <= aSRWP->Items()->Upper(); j++)
            {
              ...
              if (aSRWP->ItemsValue(j)->IsKind(STANDARD_TYPE(StepGeom_Axis2Placement3d)))
              {
                ...
                occ::handle<Geom_Axis2Placement> anAxis = StepToGeom::MakeAxis2Placement(anAx, aLocalFactors);
  ...
  for (anIter.Start(); anIter.More(); anIter.Next()) {
    ...
    if (anAtr->IsKind(STANDARD_TYPE(StepDimTol_DatumSystem)))
    {
        ...
        if (anAtrDS->IsKind(STANDARD_TYPE(StepAP242_GeometricItemSpecificUsage)))
        {
          ...
          if (aAxGISUI->IdentifiedItemValue(1)->IsKind(STANDARD_TYPE(StepGeom_Axis2Placement3d)))
          {
            ...
            if (theGDTL.FindAttribute(XCAFDoc_GeomTolerance::GetID(), aTol))
            {
              ...
              occ::handle<TColStd_HArray1OfReal> aDirArr = anAx->Axis()->DirectionRatios();
              occ::handle<TColStd_HArray1OfReal> aDirRArr = anAx->RefDirection()->DirectionRatios();
              occ::handle<TColStd_HArray1OfReal> aLocArr = anAx->Location()->Coordinates();
              gp_Dir aDir;
              gp_Dir aDirR;
              gp_Pnt aPnt;
      ...
      if (aDS->NbConstituents() > 0)
      {
        ...
        occ::handle<StepDimTol_HArray1OfDatumReferenceCompartment> aDRCA = aDS->Constituents();
        if (!aDRCA.IsNull())
        {
          for (int i = aDRCA->Lower(); i <= aDRCA->Upper(); i++)
          {
            ...
            //gete modifiers
            occ::handle<StepDimTol_HArray1OfDatumReferenceModifier> aModif = aDRC->Modifiers();
            ...
            if (!aModif.IsNull())
            {
              for (int m = aModif->Lower(); m <= aModif->Upper(); m++)
              {
                ...
                else if (aModif->Value(m).CaseNumber() == 1)
                {
                  ...
                  double aVal = aModif->Value(m).DatumReferenceModifierWithValue()->ModifierValue()->ValueComponent();
            ...
            for (anIterDRC.Start(); anIterDRC.More(); anIterDRC.Next()) {
              ...
              {
                occ::handle<StepDimTol_Datum> aD = occ::down_cast<StepDimTol_Datum>(anIterDRC.Value());
              ...
              else if (anIterDRC.Value()->IsKind(STANDARD_TYPE(StepDimTol_DatumReferenceElement)))
              {
                ...
                occ::handle<StepDimTol_HArray1OfDatumReferenceModifier> aModifE = aDRE->Modifiers();
                if (!aModifE.IsNull())
                {
                  for (int k = aModifE->Lower(); k <= aModifE->Upper(); k++)
                  {
                    ...
                    else if (aModifE->Value(k).CaseNumber() == 1)
                    {
                      ...
                      double aVal = aModifE->Value(k).DatumReferenceModifierWithValue()->ModifierValue()->ValueComponent();
                ...
                for (anIterDRE.Start(); anIterDRE.More(); anIterDRE.Next()) {
                  ...
                  {
                    occ::handle<StepDimTol_Datum> aD = occ::down_cast<StepDimTol_Datum>(anIterDRE.Value());
                    setDatumToXCAF(aD, theGDTL, aPositionCounter, aXCAFModifiers, aXCAFModifWithVal, aModifValue, theDoc, theWS, theLocalFactors);
                  }
                }
              }
...
//purpose  : 
//=======================================================================
TDF_Label STEPCAFControl_Reader::createGDTObjectInXCAF(const occ::handle<Standard_Transient>& theEnt,
  const occ::handle<TDocStd_Document>& theDoc,
  const occ::handle<XSControl_WorkSession>& theWS,
  const StepData_Factors& theLocalFactors)
{
  ...
  if (theEnt->IsKind(STANDARD_TYPE(StepDimTol_GeometricTolerance))) {
    occ::handle<StepDimTol_GeometricTolerance> aGeomTol = occ::down_cast<StepDimTol_GeometricTolerance>(theEnt);
  ...
  if (theEnt->IsKind(STANDARD_TYPE(StepShape_DimensionalLocation))) {
    occ::handle<StepShape_DimensionalLocation> aDim = occ::down_cast<StepShape_DimensionalLocation>(theEnt);
  ...
  for (anIter.Start(); anIter.More(); anIter.Next()) {
      ...
      anAtr->IsKind(STANDARD_TYPE(StepShape_DimensionalSize)))
    {
      ...
      {
        occ::handle<StepRepr_ShapeAspect> aSA = occ::down_cast<StepRepr_ShapeAspect>(anIterDim.Value());
        if (!aSA.IsNull()) {
        ...
        if (!PropD.IsNull())//for old version
        {
          ...
          if (theEnt->IsKind(STANDARD_TYPE(StepShape_DimensionalSize))) {
            ...
            for (subs4.Start(); subs4.More(); subs4.Next()) {
              ...
              if (!DimCharR.IsNull()) {
                occ::handle<StepShape_ShapeDimensionRepresentation> SDimR = DimCharR->Representation();
                if (!SDimR.IsNull() && SDimR->NbItems() > 0) {
                  ...
                  if (!VR.IsNull()) {
                    ...
                    //occ::handle<StepRepr_HArray1OfRepresentationItem> HARI;
                    //if(CID.CaseMem(CIDM)==1)
                    //  HARI = CID.ListRepresentationItem();
                    //if(CID.CaseMem(CIDM)==2)
                    //  HARI = CID.SetRepresentationItem();
                    occ::handle<StepRepr_HArray1OfRepresentationItem> HARI = VR->ItemElement();
            ...
            if (aSh.IsNull()) continue;
            TDF_Label shL;
            if (!aSTool->Search(aSh, shL, true, true, true)) continue;
            occ::handle<TColStd_HArray1OfReal> arr = new TColStd_HArray1OfReal(1, 2);
            arr->SetValue(1, dim1);
            arr->SetValue(2, dim2);
          ...
          // read tolerances and datums
            ...
            //double dim = GT->Magnitude()->ValueComponent();
            occ::handle<StepBasic_MeasureWithUnit> dim3 = GT->Magnitude();
            ...
            occ::handle<TColStd_HArray1OfReal> arr = new TColStd_HArray1OfReal(1, 1);
            arr->SetValue(1, dim);
  ...
  const StepData_Factors& theLocalFactors)
{
  ...
  for (anIterDim.Start(); anIterDim.More(); anIterDim.Next()) {
    ...
    if (!aDCR.IsNull()) {
      ...
      if (!aSDR.IsNull()) {
        occ::handle<StepRepr_HArray1OfRepresentationItem> aHARI = aSDR->Items();
        ...
        if (!aHARI.IsNull())
        {
          for (int nr = aHARI->Lower(); nr <= aHARI->Upper(); nr++)
          {
            ...
            if (aDRI->IsKind(STANDARD_TYPE(StepRepr_ReprItemAndMeasureWithUnit))) {
              ...
              occ::handle<StepShape_QualifiedRepresentationItem> aQRI = aMWU->GetQualifiedRepresentationItem();
              if (aQRI->Qualifiers()->Length() == 0) {
                aDim1 = aVal;
                continue;
              }
              occ::handle<StepShape_TypeQualifier> aValueType = aQRI->Qualifiers()->Value(1).TypeQualifier();
    ...
    else if (!aPMT.IsNull())
    {
        ...
        //! 2 -> LimitsAndFits from StepShape
      {
        ...
        if(aUpperBound->IsKind(STANDARD_TYPE(StepBasic_MeasureWithUnit)) )
          aMWU = occ::down_cast<StepBasic_MeasureWithUnit>(aUpperBound);
        else if(aUpperBound->IsKind(STANDARD_TYPE(StepRepr_ReprItemAndMeasureWithUnit)) )
        {
          occ::handle<StepRepr_ReprItemAndMeasureWithUnit> aReprMeasureItem = 
            occ::down_cast<StepRepr_ReprItemAndMeasureWithUnit>(aUpperBound);
        ...
        if(aLowerBound->IsKind(STANDARD_TYPE(StepBasic_MeasureWithUnit)) )
          aMWU = occ::down_cast<StepBasic_MeasureWithUnit>(aLowerBound);
        else if(aLowerBound->IsKind(STANDARD_TYPE(StepRepr_ReprItemAndMeasureWithUnit)) )
        {
          occ::handle<StepRepr_ReprItemAndMeasureWithUnit> aReprMeasureItem = 
            occ::down_cast<StepRepr_ReprItemAndMeasureWithUnit>(aLowerBound);
          aMWU = aReprMeasureItem->GetMeasureWithUnit();
  ...
  {
    occ::handle<TColStd_HArray1OfReal> anArr = new TColStd_HArray1OfReal(1, 2);
  ...
  {
    occ::handle<TColStd_HArray1OfReal> anArr = new TColStd_HArray1OfReal(1, 3);
    ...
    if (aType == XCAFDimTolObjects_DimensionType_Location_None)
    {
      ...
      if (!aPSA.IsNull())
      {
        ...
        if (aPGISU->NbIdentifiedItem() > 0) {
          aGRI = occ::down_cast<StepGeom_GeometricRepresentationItem>(aPGISU->IdentifiedItemValue(1));
        }
      ...
      else if (!anAP.IsNull())
      {
        if (anAP->Name()->String().IsEqual("orientation") && !anAP->Axis().IsNull())
        {
          ...
          occ::handle<TColStd_HArray1OfReal> aDirArr = anAP->RefDirection()->DirectionRatios();
          gp_Dir aDir;
          int aDirLower = aDirArr->Lower();
          if (!aDirArr.IsNull() && aDirArr->Length() > 2)
          {
  ...
  XCAFDimTolObjects_GeomToleranceType& theType)
{
  ...
  {
    occ::handle<StepDimTol_GeoTolAndGeoTolWthDatRef> anE = occ::down_cast<StepDimTol_GeoTolAndGeoTolWthDatRef>(theEnt);
  ...
  const StepData_Factors& theLocalFactors)
{
  ...
  occ::handle<StepDimTol_GeometricTolerance> aTolEnt = occ::down_cast<StepDimTol_GeometricTolerance>(theEnt);
  ...
  if (!aTolEnt->Magnitude().IsNull()) {
    //get value
    double aVal = aTolEnt->Magnitude()->ValueComponent();
  ...
  occ::handle<StepDimTol_HArray1OfGeometricToleranceModifier> aModifiers;
  ...
  {
    aModifiers = occ::handle<StepDimTol_GeoTolAndGeoTolWthDatRefAndGeoTolWthMod>
  ...
  {
    aModifiers = occ::handle<StepDimTol_GeoTolAndGeoTolWthMod>
      ::DownCast(aTolEnt)->GetGeometricToleranceWithModifiers()->Modifiers();
  }
  else if (aTolEnt->IsKind(STANDARD_TYPE(StepDimTol_GeoTolAndGeoTolWthMaxTol)))
  {
    aModifiers = occ::handle<StepDimTol_GeoTolAndGeoTolWthMaxTol>
      ::DownCast(aTolEnt)->GetGeometricToleranceWithModifiers()->Modifiers();
  }
  else if (aTolEnt->IsKind(STANDARD_TYPE(StepDimTol_GeoTolAndGeoTolWthDatRefAndGeoTolWthMaxTol)))
  {
    aModifiers = occ::handle<StepDimTol_GeoTolAndGeoTolWthDatRefAndGeoTolWthMaxTol>
      ::DownCast(aTolEnt)->GetGeometricToleranceWithModifiers()->Modifiers();
  }
  ...
  {
    occ::handle<StepDimTol_GeometricToleranceWithMaximumTolerance> aMax = occ::down_cast<StepDimTol_GeometricToleranceWithMaximumTolerance>(aTolEnt);
                                                 ...
                                                 const StepData_Factors& theLocalFactors)
{
  ...
  occ::handle<XCAFDoc_ShapeTool> aSTool = XCAFDoc_DocumentTool::ShapeTool(theDoc->Main());
  occ::handle<XCAFDoc_DimTolTool> aDGTTool = XCAFDoc_DocumentTool::DimTolTool(theDoc->Main());
  if (aDGTTool.IsNull()) return false;
  ...
  for (int i = 1; i <= nb; i++) {
      ...
      anEnt->IsKind(STANDARD_TYPE(StepVisual_AnnotationOccurrence)))
    {
      ...
      if (!aDMIA.IsNull()) {
        ...
        if (!aDefinition.IsNull()) {
          ...
          for (Interface_EntityIterator anIter = aGraph.Sharings(aDefinition); anIter.More() && !isConnectedToGDT; anIter.Next()) {
            ...
            occ::handle<StepRepr_ShapeAspectRelationship> aSAR = occ::down_cast<StepRepr_ShapeAspectRelationship>(anIter.Value());
            if (!aSAR.IsNull()) {
      ...
      aDGTTool->Lock(aGDTL);
      occ::handle<XCAFDimTolObjects_DimensionObject> aDimObj = new XCAFDimTolObjects_DimensionObject();
      occ::handle<XCAFDoc_Dimension> aDim = XCAFDoc_Dimension::Set(aGDTL);
...
static occ::handle<StepShape_SolidModel> FindSolidForPDS(const occ::handle<StepRepr_ProductDefinitionShape> &PDS,
  const Interface_Graph &graph)
{
  occ::handle<StepShape_SolidModel> SM;
  ...
  for (subs.Start(); subs.More() && SM.IsNull(); subs.Next()) {
    ...
    if (SM.IsNull()) {
      ...
      for (subs1.Start(); subs1.More() && SM.IsNull(); subs1.Next()) {
        ...
        if (RR->Rep1() == SR) SR2 = occ::down_cast<StepShape_ShapeRepresentation>(RR->Rep2());
        else SR2 = occ::down_cast<StepShape_ShapeRepresentation>(RR->Rep1());
  ...
  TDF_LabelSequence& theShapes)
{
  ...
  for (; anIter.More(); anIter.Next()) {
    if (!anIter.Value()->IsKind(STANDARD_TYPE(StepRepr_RepresentationRelationship)))
      continue;
    occ::handle<StepRepr_RepresentationRelationship> aReprRelationship = occ::down_cast<StepRepr_RepresentationRelationship>(anIter.Value());
...
//purpose  :
//=======================================================================
occ::handle<TCollection_HAsciiString> buildClippingPlanes(const occ::handle<StepGeom_GeometricRepresentationItem>& theClippingCameraModel,
  ...
  const StepData_Factors& theLocalFactors)
{
  ...
  if (theClippingCameraModel->IsKind(STANDARD_TYPE(StepVisual_CameraModelD3MultiClipping))) {
    ...
    for (int i = 1; i <= aCameraModel->ShapeClipping()->Length(); i++) {
      aPlanes.Append(occ::down_cast<StepGeom_GeometricRepresentationItem>(aCameraModel->ShapeClipping()->Value(i).Value()));
    }
  }
                                                  ...
                                                  const StepData_Factors& theLocalFactors) const
{
  ...
  for (int i = 1; i <= nb; i++) {
    ...
    occ::handle<StepVisual_CameraModelD3> aCameraModel = occ::down_cast<StepVisual_CameraModelD3>(anEnt);
    ...
    occ::handle<Geom_Axis2Placement> anAxis = StepToGeom::MakeAxis2Placement(aCameraModel->ViewReferenceSystem(), aLocalFactors);
    ...
    anObj->SetType(aType);
    occ::handle<Geom_CartesianPoint> aPoint = StepToGeom::MakeCartesianPoint(aViewVolume->ProjectionPoint(), aLocalFactors);
    ...
    if (!aClippingCameraModel.IsNull()) {
      ...
      occ::handle<XCAFDoc_ClippingPlaneTool> aClippingPlaneTool = XCAFDoc_DocumentTool::ClippingPlaneTool(theDoc->Main());
    ...
    for (; anIter.More(); anIter.Next()) {
      if (anIter.Value()->IsKind(STANDARD_TYPE(StepRepr_MappedItem))) {
        occ::handle<StepRepr_MappedItem> anItem = occ::down_cast<StepRepr_MappedItem>(anIter.Value());
        if (occ::handle<StepRepr_Representation> aRepr = anItem->MappingSource()->MappedRepresentation())
        {
        ...
        occ::handle<StepVisual_AnnotationPlane> aPlane = occ::down_cast<StepVisual_AnnotationPlane>(anIter.Value());
  ...
  NCollection_Sequence<occ::handle<StepRepr_RepresentationItem>>& theItems)
{
  ...
  {
    occ::handle<StepRepr_RepresentationRelationship> aRelationship = occ::down_cast<StepRepr_RepresentationRelationship>(entIt.Value());
    if (aRelationship->Rep1() == theRepresentation)
    {
      occ::handle<StepShape_ShapeRepresentation>
        aRepr = occ::down_cast<StepShape_ShapeRepresentation>(aRelationship->Rep2());
      if (!aRepr.IsNull())
...
void STEPCAFControl_Reader::ExpandSubShapes(const occ::handle<XCAFDoc_ShapeTool>& ShapeTool,
  const STEPCAFControl_DataMapOfShapePD& ShapePDMap) const
{
  ...
  for (STEPCAFControl_DataMapIteratorOfDataMapOfShapePD it(ShapePDMap); it.More(); it.Next())
  {
    ...
    occ::handle<StepShape_ShapeRepresentation>
      aShapeRepr = occ::down_cast<StepShape_ShapeRepresentation>(aSDR->UsedRepresentation());
...
void STEPCAFControl_Reader::ExpandSBSM(TDF_Label& ShapeLab,
  ...
  const occ::handle<XCAFDoc_ShapeTool>& ShapeTool) const
{
  occ::handle<StepShape_ShellBasedSurfaceModel> aSBSM = occ::down_cast<StepShape_ShellBasedSurfaceModel>(Repr);

  // Access boundary shells
  occ::handle<StepShape_HArray1OfShell> aShells = aSBSM->SbsmBoundary();
  ...
  const occ::handle<XCAFDoc_ShapeTool>& ShapeTool) const
{
  ...
  // Access faces
  occ::handle<StepShape_HArray1OfFace> aFaces = Shell->CfsFaces();
  for (int f = aFaces->Lower(); f <= aFaces->Upper(); ++f)
  {
    ...
    // Access face bounds
    occ::handle<StepShape_HArray1OfFaceBound> aWires = aFace->Bounds();
    if (aWires.IsNull())
      continue;
    for (int w = aWires->Lower(); w <= aWires->Upper(); ++w)
    {
      ...
      occ::handle<StepShape_HArray1OfOrientedEdge> anEdges = anEdgeLoop->EdgeList();
                                                     ...
                                                     const StepData_Factors& theLocalFactors) const
{
  ...
  int aNb = aModel->NbEntities();
  STEPConstruct_Tool aTool(theWS);

  for (int anEntityInd = 1; anEntityInd <= aNb; ++anEntityInd)
  {
    occ::handle<Standard_Transient> anEntity = aModel->Value(anEntityInd);
    occ::handle<StepBasic_GeneralProperty> aGeneralProp = occ::down_cast<StepBasic_GeneralProperty>(anEntity);
    if (aGeneralProp.IsNull())
      continue;

    occ::handle<StepBasic_GeneralPropertyAssociation> aPropAssociation;
    NCollection_List< occ::handle<StepRepr_PropertyDefinition>> aPropDefinitionList;
    ...
    {
      aPropAssociation = occ::down_cast<StepBasic_GeneralPropertyAssociation>(aSharingListOfGP.Value());
    ...
    NCollection_List< occ::handle<StepRepr_PropertyDefinition>>::Iterator aPropDefIter(aPropDefinitionList);
    for ( ; aPropDefIter.More(); aPropDefIter.Next())
    {
      occ::handle<StepRepr_PropertyDefinition> aPropDefinition = aPropDefIter.Value();

      // check group of PropertyDefinition
      NCollection_List<occ::handle<StepRepr_PropertyDefinition>> aGroupedProperties;
        ...
        if (!aShapeAspect.IsNull())
        {
          ...
          for (aSharedListOfSA.Start(); aSharedListOfSA.More(); aSharedListOfSA.Next())
          {
            ...
            if (!anItemIdentUsage.IsNull())
            {
              ...
              {
                occ::handle<StepRepr_RepresentationItem> aReprItem = anItemIdentUsage->IdentifiedItemValue(anIndex);
                if (aReprItem.IsNull())
                  continue;
      ...
      for (TDF_LabelSequence::Iterator aLabelIt(aLabelSeq); aLabelIt.More(); aLabelIt.Next())
      {
        ...
        occ::handle<TDataStd_NamedData> anAttr;
        ...
        {
          NCollection_List<occ::handle<StepRepr_PropertyDefinition>>::Iterator aPropIt(aGroupedProperties);
...
//purpose  : 
//=======================================================================
bool STEPCAFControl_Reader::findReprItems(const occ::handle<XSControl_WorkSession>& theWS,
                                                      ...
                                                      NCollection_List<occ::handle<Transfer_Binder>>& theBinders) const
{
  ...
  occ::handle<Transfer_Binder> aBinder;
  const occ::handle<Transfer_TransientProcess>& aTP = theWS->TransferReader()->TransientProcess();
  occ::handle<StepRepr_HArray1OfRepresentationItem> aReprItems = aRepr->Items();
...
//purpose  : 
//=======================================================================
bool STEPCAFControl_Reader::fillAttributes(const occ::handle<XSControl_WorkSession>& theWS,
                                                       ...
                                                       occ::handle<TDataStd_NamedData>& theAttr) const
{
  ...
  for (aSharingListOfPD.Start(); aSharingListOfPD.More(); aSharingListOfPD.Next())
  {
    aPropDefRepr = occ::down_cast<StepRepr_PropertyDefinitionRepresentation>(aSharingListOfPD.Value());
    ...
    occ::handle<StepRepr_HArray1OfRepresentationItem> aReprItems = aUsedRepr->Items();
    if (!aReprItems.IsNull())
    {
      for (int anIndex = aReprItems->Lower(); anIndex <= aReprItems->Upper(); anIndex++)
      {
        ...
        {
          occ::handle<StepRepr_DescriptiveRepresentationItem> aDescrItem = occ::down_cast<StepRepr_DescriptiveRepresentationItem>(anItem);
        ...
        {
          occ::handle<StepRepr_MeasureRepresentationItem> aMeasureItem = occ::down_cast<StepRepr_MeasureRepresentationItem>(anItem);
        ...
        {
          occ::handle<StepRepr_ValueRepresentationItem> aValueItem = occ::down_cast<StepRepr_ValueRepresentationItem>(anItem);
          occ::handle<StepBasic_MeasureValueMember> aMeasureValueMem = aValueItem->ValueComponentMember();
        ...
        {
          occ::handle<StepRepr_IntegerRepresentationItem> anIntegerItem = occ::down_cast<StepRepr_IntegerRepresentationItem>(anItem);
        ...
        {
          occ::handle<StepRepr_RealRepresentationItem> aRealItem = occ::down_cast<StepRepr_RealRepresentationItem>(anItem);
        ...
        {
          occ::handle<StepRepr_BooleanRepresentationItem> aBoolItem = occ::down_cast<StepRepr_BooleanRepresentationItem>(anItem);
