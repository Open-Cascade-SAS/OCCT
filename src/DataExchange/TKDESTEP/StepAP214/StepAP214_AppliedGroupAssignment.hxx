...
public:
  ...
  Standard_EXPORT void Init(const occ::handle<StepBasic_Group>& aGroupAssignment_AssignedGroup,
                            const occ::handle<StepAP214_HArray1OfGroupItem>& aItems);

  //! Returns field Items
  Standard_EXPORT occ::handle<StepAP214_HArray1OfGroupItem> Items() const;

  //! Set field Items
  Standard_EXPORT void SetItems(const occ::handle<StepAP214_HArray1OfGroupItem>& Items);

  DEFINE_STANDARD_RTTIEXT(StepAP214_AppliedGroupAssignment, StepBasic_GroupAssignment)

private:
  occ::handle<StepAP214_HArray1OfGroupItem> theItems;
};
