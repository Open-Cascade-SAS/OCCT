...
Standard_Boolean DEGLTF_Provider::Read(ReadStreamMap&            theStreams,
                                        const Handle(TDocStd_Document)& theDocument,
                                        Handle(XSControl_WorkSession)&  theWS,
                                        const Message_ProgressRange&    theProgress)
{
                                        ...
                                        const Message_ProgressRange&    theProgress)
{
  ...
  {
    Message::SendFail() << "Error: DEGLTF_Provider stream map is empty";
    return Standard_False;
  }
  if (theStreams.Size() > 1)
  {
    Message::SendWarning() << "Warning: DEGLTF_Provider received " << theStreams.Size()
