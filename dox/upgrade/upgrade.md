...
The recommended method for building OCCT 7.x is CMake, see @ref build_occt_cmake.
The alternative solution (project files generated by OCCT legacy tool **genproj**) is no longer supported.
...
~~~~
  This change is not applied if the source or header file is recognized as containing the definition of Qt class with signals or slots, to avoid possible compilation errors of MOC files caused by inability of MOC to recognize macros (see https://doc.qt.io/qt-6/signalsandslots.html).
...
The upgrade script and sources of a specialized WOK version used for OCCT code upgrade can be found in WOK Git repository (no longer publicly available).

[Contact us](https://dev.opencascade.org/) if you need more help.
...
The anchor point specified for transformation persistence defines the window corner (or center in case of (0, 0) point).
